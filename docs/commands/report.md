...
| `--layer` | `[physical\|l1\|l2\|l3\|ipam\|overlay\|routing\|rack\|power\|identity\|netns\|security]` | every layer the inventory declares something for | Draw this layer on every page, instead of the ones the inventory has earned. Repeatable, and honoured verbatim: a layer with nothing in it is reported as empty rather than dropped. |

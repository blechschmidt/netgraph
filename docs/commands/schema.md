...
|---|---|---|---|
| `-k`, `--kind` | `[switch\|router\|hub\|computer\|server\|cable\|adapter\|tunnel\|patchpanel\|pdu\|user\|group\|template\|layout\|testsuite\|note\|area\|legend]` | — | Emit the schema for a single document kind instead of all of them. |

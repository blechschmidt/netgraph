...
Report the netgraph, Python and Graphviz versions in use, plus which YAML parser was
selected and what the runtime dependencies resolved to. `netgraph --version` prints the
...
```text
netgraph [GLOBAL OPTIONS] version [OPTIONS]
```
<!-- /generated -->
...
because nearly every interesting failure in netgraph is a failure of something *around* it:
...
* **The `netgraph` on `PATH` belongs to an environment nobody meant to be in.** The
...
[`netgraph render`](render.md) would use.
...
The first line is `netgraph <version>` and nothing else, so `netgraph --version | cut -d' ' -f2`
keeps working.

Graphviz has four states, and the line distinguishes them because they need different
answers: a version and a path (fine), `not found` (install it, or set `NETGRAPH_DOT` — the
`dot`, `mermaid` and `json` formats still work), or `unknown` with the reason `dot -V` gave,
which is usually a `NETGRAPH_DOT` pointing at something that is not Graphviz.
...
`schemaVersion` follows the same rule as the other JSON documents netgraph writes — adding a
...
* [`docs/getting-started.md`](../getting-started.md#on-windows-and-macos) — installing
  Graphviz, and `NETGRAPH_DOT` when it does not land on `PATH`.
...
* [`docs/testing.md`](../testing.md) — `NETGRAPH_YAML_LOADER`, the switch behind the
  `YAML parser` line.

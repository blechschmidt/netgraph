...
`netviz export` turns the inventory into files other tools consume: fourteen
...
ifupdown, FRR, WireGuard, and netviz's own vendor-neutral grammar for everything
...
```text
netviz [GLOBAL OPTIONS] export [OPTIONS] FORMAT
```
<!-- /generated -->
...
|---|---|---|
| [`netplan`](../export.md#device-configuration-the-seven-dialects) | `etc/netplan/10-netviz.yaml` for a `computer`, `server` or `router` | The 802.1Q configuration of a bridge port, and a VRF: netplan's `vrfs` section needs a numeric routing table and an inventory states a route distinguisher |
...
| [`interfaces`](../export.md#device-configuration-the-seven-dialects) | `interfaces.conf` — netviz's own vendor-neutral grammar, for **every** device | Nothing, ever. What it cannot do is be applied: no system reads it |
...
<!-- run: -->
```console
$ netviz -q -i examples/home-lab export hosts
# Generated by 'netviz export hosts' -- do not edit.
# netviz 0.0.2. Re-run the command to regenerate from the inventory.
...
```console
$ netviz -q -i examples/overlay export networkd --out build/config
```

Three rules apply to that directory, and they are narrower than the ones
[`netviz import`](import.md) uses, because regenerating a configuration is the
normal operation rather than an exceptional one. A file netviz generated is
**overwritten** without a flag, since its banner says so. A file netviz did not
...
`10-netviz.yaml` under the tree netviz loads is a document netviz tries to
...
```console
$ netviz -q -i examples/home-lab export netplan --name pc-desk | ssh pc-desk 'cat >/etc/netplan/10-netviz.yaml'
```
...
Every filter [`netviz render`](render.md) takes, `export` takes, and they mean
...
Reverse zones are regrouped from the prefixes [`netviz ipam`](ipam.md) sizes
...
<!-- run: rc=2 -->
```console
$ netviz -i examples/home-lab export hosts --port 9100
...
artefact, and `--force` additionally means "overwrite files netviz did not
...
<!-- run: rc=2 -->
```console
$ netviz -i examples/home-lab export hosts --out build/config
Usage: netviz export [OPTIONS] FORMAT
Try 'netviz export --help' for help.
...
| `--annotations`, `--no-annotations` | — | `--annotations` | Draw the notes, areas and legends the inventory declares for this view as native draw.io shapes, which the reader can edit and 'netviz import drawio' reads back. Turn them off for a file that should carry the topology alone. |
...
| `3` | The inventory could not be discovered or read at all, or the `--out` tree could not be written — the directory is a file, a write failed, or it holds files netviz did not generate and `--force` was not given. |

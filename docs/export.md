...
| `nftables` | `etc/nftables.conf` — one `table inet netgraph` with a set per zone, the three base chains and whatever NAT the translations need | Any device declaring `spec.firewall` | A rule using `invert`: nftables negates one expression at a time (`!=`) and `spec.firewall` inverts the whole selector set, so the rule would be written matching the opposite of what the document says |
...
The same seven dialects are read *back* by [`netgraph drift`](commands/drift.md),

...
- [`power`](#power)
- [`drawio`](#drawio)
- [Device configuration: the seven dialects](#device-configuration-the-seven-dialects)
...
Reverse zones are **not** re-derived here. [`netviz.subnets`](../src/netviz/subnets.py)
...
[`netviz ipam`](ipam.md) — and each prefix is rounded down to the nearest
...
| Variable | Contents |
|---|---|
| `netviz_element` | The fully-qualified name, e.g. `sites/north/access/sw-north-acc-01` |
| `netviz_name`, `netviz_kind`, `netviz_namespace` | Identity |
| `netviz_description`, `netviz_labels` | `metadata.description` and `metadata.labels` |
| `netviz_vendor`, `netviz_model`, `netviz_serial` | Hardware, when declared |
| `netviz_location` | `site`, `room`, `rack`, `position`, `height` from `metadata.location` |
| `netviz_addresses` | Every routable address, `10.0.0.1/24` form, canonically sorted |
| `netviz_interfaces` | One record per interface **in declaration order**: name, type, enabled, description, mac, mtu, addresses, VLAN mode and VLAN ids |
| `netviz_vlan_ids` | Every VLAN the element participates in (§9.3) |
| `netviz_vlans` | The device VLAN database — id, name, description |
...
> **Deliberately absent: the netviz version.** Every label becomes part of the
...
what lets [`netviz import drawio`](commands/import.md) tell a moved switch from
...
one somebody chose. Run [`netviz layout --write`](commands/layout.md) first and
...
netviz could write one the inventory was a document *beside* the truth rather
...
|---|---|---|---|
| `netplan` | `etc/netplan/10-netviz.yaml`, one document per host | `computer`, `server`, `router` | The 802.1Q configuration of a bridge port, and a VRF — netplan's `vrfs` section needs a numeric routing table and an inventory states a route distinguisher |
...
| `nftables` | `etc/nftables.conf` — one `table inet netviz` with a set per zone, the three base chains and whatever NAT the translations need | Any device declaring `spec.firewall` | A rule using `invert`: nftables negates one expression at a time (`!=`) and `spec.firewall` inverts the whole selector set, so the rule would be written matching the opposite of what the document says |
...
| `interfaces` | `interfaces.conf` — netviz's own vendor-neutral grammar | **Every** device | Nothing, ever. It is not somebody else's format; it is defined as whatever holds one device's interface configuration completely |
...
[`netviz drift`](commands/drift.md) reads back.
...
No inventory shipped with netviz refuses anything, so here is a constructed
...
|---|---|---|
| `netviz-dialect` | Which of the six wrote the file | [`drift`](commands/drift.md) and [`import`](commands/import.md), so neither needs `--from`; and `--out`, to tell a file netviz generated from one it did not |
| `netviz-element` | The fully-qualified name and kind of the device | `drift` and `import`, so neither needs `--host` |
| `netviz-source` | Every inventory document behind this file, relative to the root. Repeated when there is more than one — a router's own YAML *and* the tunnel document that gave it a peer | A person, editing |
...
tomorrow, so the rule is narrower than the one [`netviz import`](commands/import.md)
...
**A file netviz generated is overwritten**, no flag required. It carries the
...
The same seven dialects are read *back* by [`netviz drift`](commands/drift.md),
...
[`netviz/export/names.py`](../src/netviz/export/names.py), with every fold
recorded.
...
| `unresolved-endpoint` | A cable or tunnel whose endpoint does not resolve | Only reachable behind `--force`; `netviz validate` refuses it first |

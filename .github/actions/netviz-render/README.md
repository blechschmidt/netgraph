# `netviz-render` action

A composite action that installs netviz and Graphviz, runs `netviz render`
...
[`netviz-pages.yml`](../../workflows/netviz-pages.yml) does with it.
...
job, though the [reusable workflow](../../workflows/netviz-pages.yml) is
shorter:
...
| `output` | *(derived)* | Where to write the diagram; parent directories are created. Defaults to `${RUNNER_TEMP}/netviz.<extension>`. |
...
`args: --name sw*` reaches `netviz.exe` as whatever the workspace happened to
be holding. Put the filter in `netviz.toml` instead, where no shell touches
it:

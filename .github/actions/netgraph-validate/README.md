# `netgraph-validate` action

A composite action that installs netgraph, runs `netgraph validate` over an
...
| `output-file` | *(derived)* | Where to write the document; defaults to `${RUNNER_TEMP}/netgraph-validate.<format>`. Ignored by `github` and `text`, which write to the log. |
...
| Output | Meaning |
|---|---|
| `exit-code` | `netgraph validate`'s exit code: `0` clean, `1` findings, `2` usage. |

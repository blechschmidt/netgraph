# `netgraph-review` action

A composite action that reviews a change to a netgraph inventory: it produces
...
[`netgraph-review.yml`](../../workflows/netgraph-review.yml), which does the
...
jobs:
  review:
    ...
    steps:
      ...
      - id: netgraph
        uses: blechschmidt/netgraph/.github/actions/netgraph-review@v0.1.0
        with:
          inventory: inventory
      ...
      - uses: actions/upload-artifact@v4
        with:
          ...
          path: ${{ steps.netgraph.outputs.directory }}

      - uses: github/codeql-action/upload-sarif@v3
        with:
          sarif_file: ${{ steps.netgraph.outputs.sarif }}
          category: netgraph

      - run: cat "${{ steps.netgraph.outputs.comment }}" >>"$GITHUB_STEP_SUMMARY"
...
1. Installs Graphviz and netgraph, unless told not to.
2. Fetches `base` if the clone does not hold it. A `pull_request` checkout is
   ...
   `fetch-depth: 0`.
3. Draws `netgraph diff` in each of `formats`. Each drawing is allowed to fail:
   a missing Graphviz costs the review its picture and not its changeset.
4. Runs `netgraph review`, which loads both states, validates both, diffs them
   and writes five files into `output-directory`:

   | File | What |
   |---|---|
   | `comment.md` | The comment body. Its first line is the sticky marker. |
   | `plan.json` | The changeset, as `netgraph plan --json` writes it. Absent when the head does not load. |
   | `netgraph.sarif` | The head's findings, for `upload-sarif`. |
...
| `title` | `netgraph` | Heading of the comment, and the key of its sticky marker. Two inventories need two titles. |
| `layer` | *(empty)* | Which layer the diff is drawn at. One only — a diff compares one view. |
| `theme` | *(empty)* | Stylesheet for the diagram. |
| `diagram-args` | *(empty)* | Further `netgraph diff` flags, split on whitespace. |
...
| `artifact-name` | `netgraph-review` | What the bundle is called, for the link text. |
...
| `output-directory` | `${RUNNER_TEMP}/netgraph-review` | Where the files above go. |
...
| `install` | `true` | Install netgraph first. |
...
of [`netgraph-review.yml`](../../workflows/netgraph-review.yml) for why.

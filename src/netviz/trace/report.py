...
The JSON form is the same facts under the envelope every netviz document
...
alternating kinds; see :mod:`netviz.trace.model`.
...
from netviz.models import API_VERSION, format_bitrate
from netviz.render.graph import Layer, PatchView, TunnelView
from netviz.trace.model import Endpoint, Frontier, Link, TracedPath, TraceResult, Waypoint
...
#: the ``NetworkGraph`` of :mod:`netviz.render.jsonexport`.
...
def _tunnel(view: TunnelView) -> dict[str, Any]:
    ...
    ``netviz render -f json --layer overlay`` is for; repeating it per hop

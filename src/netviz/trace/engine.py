...
:func:`~netviz.render.graph.build_graph` already builds. Neither re-reads the
    ...
    *,
    ...
    Args:
        ...
        source: ``SRC`` as typed — an element, ``element:interface`` or an IP
            address (:func:`~netviz.trace.endpoints.resolve_endpoint`).
    ...
    Returns:
        A :class:`~netviz.trace.model.TraceResult`. Finding **no** path is a
...
def _same_element(
    ...
    ``netviz path sw1:port1 sw1:port3`` is a real question with a real answer:
...
def _l2_adjacency(graph: Graph) -> Mapping[str, tuple[_Hop, ...]]:
    ...
    :func:`~netviz.render.graph._tunnel_topology`), which is right for a
...
def _stacks_of(graph: Graph, endpoint: Endpoint) -> tuple[str, ...]:
    ...
    * ``srv-01`` pins nothing below the machine, so every stack of it does. That
      is the reading that keeps ``netviz path srv-01 sw-core`` working on a
...
def _l3_waypoint(node: Node, ingress: _End | None, egress: _End | None) -> Waypoint:
    ...
    (:attr:`~netviz.trace.model.Waypoint.netns`); ``kind`` likewise stays the

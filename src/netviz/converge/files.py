...
existing emitters in :mod:`netviz.export.config` are run **twice**:
...
* over the *observed* inventory -- the declaration with every observation folded
  into it, which is what :func:`netviz.plan.live.adopt` already builds for
  ``netviz plan --from-live`` -- giving what the device has now.
...
from netviz.export.config import ConfigSet, UnsupportedConfigError, generate
...
#: is absent because nothing reads it -- it is netviz's own projection, so a
...
class FileChange:
    ...
    #: Absolute path on the device: ``/etc/netplan/10-netviz.yaml``.
    path: str
...
def file_changes(
    ...
    Raises:
        UnsupportedConfigError: The dialect cannot express a device *as the
            ...
            is already made in :func:`netviz.export.config.generate`.
    """
...
def strip_banner(text: str) -> str:
    ...
    :func:`~netviz.export.config.header.config_header`, which names the

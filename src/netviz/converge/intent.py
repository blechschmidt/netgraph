...
:mod:`netviz.converge.commands` turn an intent into lines; the ordering, the
...
:mod:`netviz.converge.risk`. An intent is a statement of what would close a
...
from netviz.converge.model import Action, Provenance
...
class IntentKind(str, Enum):
    ...
    Bounded by what a capture can actually detect: ``netviz drift`` compares
...
#: Which :class:`~netviz.converge.model.Action` each kind reports as. Only
...
#: is up. Read by :mod:`netviz.converge.risk`; kept here so the two tables sit
...
class Intent:
    ...
    note: str = ""
    #: Overrides the sentence :func:`netviz.converge.commands.describe` would

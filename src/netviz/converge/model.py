...
``netviz drift`` says how the live network differs from the inventory.
...
exist yet. netviz opens no SSH session and never will from this command (see
...
:class:`Batch`
    ...
    radius that entails, from the :mod:`netviz.impact` engine. A transport
...
class Action(str, Enum):
    ...
    UPDATE = "update"
    DELETE = "delete"
    #: Nothing netviz can write a command for: a cable that must be moved, a
...
class Provenance:
    ...
    netviz's own opinion about a network, and this command does not have one:
    ...
    field: str | None = None
    #: The drift :class:`~netviz.drift.model.Direction` value.
    direction: str = ""
...
class Command:
    ...
    def script_lines(self) -> Iterator[str]:
        ...
        (``<<'NETVIZ_EOF'``) so the shell performs no substitution on the
        ...
        :func:`netviz.plan.live.adopt` -- a description, an SSID, a chassis
        ...
        ``netviz export config`` would write, byte for byte.
        """
        if self.kind == "exec":
            yield self.text
            return
...
class ConvergeChange:
    ...
    def actionable(self) -> bool:
        """Did netviz produce commands for this?"""
...
class Batch:
    ...
    #: :func:`netviz.impact.simulate`. Sorted, deduplicated.
    isolated: tuple[str, ...] = ()
...
class ConvergeError(NetvizError):
...
class DisruptiveChangeError(ConvergeError):
    ...
    failure mode :class:`~netviz.export.config.model.UnsupportedConfigError`
    already avoids.
    """

"""Building a :class:`~netviz.converge.model.ConvergePlan` from a capture.

Seven steps, in one place, so every consumer -- the CLI, a test, a future
transport -- gets the same plan from the same inputs:

1. read the capture exactly as ``netviz drift`` does, through the same
...
2. build the *observed* inventory, the declaration with every observation folded
   in (:func:`netviz.plan.live.adopt`), which is what the declarative dialects
   diff against;
3. turn every drift finding into intents (:mod:`netviz.converge.derive`);
4. classify each one against the device's management path
   (:mod:`netviz.converge.risk`);
5. render commands -- neutral lines, or per-dialect files
   (:mod:`netviz.converge.commands`, :mod:`netviz.converge.files`);
6. group into maintenance batches with the impact engine
   (:mod:`netviz.converge.batch`);
...
discipline :func:`netviz.export.config.generate` applies to a dialect that
...
from netviz.converge.batch import batches_for
...
from netviz.converge.intent import Intent, IntentKind, prerequisites_of
...
from netviz.plan.live import adopt
from netviz.render.graph import Graph, Layer, build_graph
...
#: The layer the plan is scoped by, matching ``netviz export config``: the
...
#: :data:`netviz.converge.intent.RANKS` except ``MANUAL``, because writing the
...
class ConvergeInputs:
    ...
    :func:`netviz.drift.check_drift` throws the draft away and returns only the
...
def _context(inventory: Inventory) -> ExportContext:
    ...
    The same one ``netviz export netplan`` builds, minus the CLI: one L1 graph,

"""Which changes can lock you out, and how netviz decides.
...
1. **The interface netviz would reach the box on.** That is
   :func:`netviz.export.context.management_address`, the same ranking that
   ...
   declared order. One definition, three consumers: if netviz would monitor
...
inventory -- has no management path netviz can name, and there an interface
...
from netviz.converge.intent import Intent, IntentKind
from netviz.converge.model import Risk
from netviz.export.context import management_address
from netviz.models import Device
from netviz.models.interface import Interface, VlanMode
from netviz.render.graph import Node
...
#:
#: ``mtu`` is deliberately absent: every stack netviz writes for changes it on
...
class ManagementPath:
    ...
    element: str
    #: The single interface netviz would reach the device on, or ``""`` when
...
def management_path(node: Node, device: Device) -> ManagementPath:
    ...
    Args:
        node: The device's node in the layer-1 graph, which is what
            :func:`~netviz.export.context.management_address` reads.

"""``netviz diff``: a changeset, drawn.

This module is deliberately thin, and deliberately the only place the two halves
meet. :mod:`netviz.plan` decides *what changed* and :mod:`netviz.render`
...
The result of :func:`draw` is an ordinary :class:`~netviz.render.graph.Graph`
and an ordinary :class:`~netviz.render.diffview.DiffOverlay`, so every
...
from netviz.plan.address import SIDECAR_TYPES
from netviz.plan.model import Action, Plan
from netviz.render.diffview import DiffOverlay, diff_overlay, union_graph
...
def draw(plan: Plan, before: Graph, after: Graph) -> Drawing:
    ...
    Args:
        plan: The changeset, from :func:`netviz.plan.diff`. It is the only
...
def updated_fields(plan: Plan) -> dict[str, tuple[str, ...]]:
    ...
    <netviz.plan.address.Address>`, because that is what a drawing keys
...
def renamed_addresses(plan: Plan) -> dict[str, str]:
    ...
    to :func:`~netviz.render.diffview.union_graph` collapses the pair into one

...
``netviz validate`` would give about the text on screen.** So the inventory is
loaded through an :class:`~netviz.loader.Overlay` carrying every unsaved
...
loaded on its own with :func:`~netviz.loader.load_stream` and the rules that
...
from netviz.config import Config, ValidationConfig, load_config
from netviz.diagnostics import Report, build_report
from netviz.loader import DocumentCache, Inventory, Overlay, load_stream, load_tree
from netviz.loader.tree import YAML_SUFFIXES
from netviz.lsp.text import Encoding, TextDocument
from netviz.lsp.uri import path_to_uri, relative_to, uri_to_path
...
class Workspace:
    ...
    def config(self) -> Config:
        """``netviz.toml`` as it stands, loaded once per server run."""

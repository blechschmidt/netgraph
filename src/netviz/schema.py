"""JSON Schema (2020-12) for netviz documents, generated from the models.

``netviz validate`` tells you about a typo *after* you have written the file.
...
**Prose.** :mod:`netviz.models.fielddocs` holds one description per field —
...
``netviz validate``; see ``docs/validation-rules.md``.
"""
...
from netviz.models.annotation import Area, Legend, Note
from netviz.models.base import NetvizModel
from netviz.models.device import Switch
from netviz.models.document import ELEMENT_MODELS, Element, element_model_for
...
from netviz.models.fielddocs import (
    DOCUMENTED_MODELS,
    FIELD_DOCS,
    KIND_NOTES,
    NONE,
...
from netviz.models.patchpanel import PORT_RANGE_PATTERN
from netviz.models.pdu import OUTLET_RANGE_PATTERN
...
from netviz.models.style import DASHES, ICON_NAME_PATTERN, NAMED_COLOURS, SHAPES
from netviz.models.template import INHERIT_KEY
...
_SIDECAR_MODELS: Final[dict[str, type[NetvizModel]]] = {
    LAYOUT_KIND: Layout,
...
#: needs a second object stays in :mod:`netviz.validate`.
...
def _apply_loader_sugar(definitions: dict[str, Any], *, kind: str | None, strict: bool) -> None:
    ...
    :mod:`netviz.loader`, so pydantic has never heard of either and cannot
    ...
    if not _split_interface(definitions) and strict:
        raise RuntimeError("netviz.schema: no definition Interface to add 'range' to")

    device_spec = definitions.get("DeviceSpec")
    if device_spec is None:
        if strict:
            raise RuntimeError(f"netviz.schema: no definition DeviceSpec to add {INHERIT_KEY!r}")
        return
...
def _split_interface(definitions: dict[str, Any]) -> bool:
    ...
    document but not a legal :class:`~netviz.models.interface.Interface`. The
...
def _document_sidecars(definitions: dict[str, Any]) -> None:
    ...
    element, so they take :class:`~netviz.models.element.ElementBase`'s
...
def _apply(
    ...
    *,
    ...
    if missing and strict:
        raise RuntimeError(
            f"netviz.schema: no such definition for {what} override: {', '.join(missing)}"
        )
...
def _apply_scalars(definitions: dict[str, Any], *, strict: bool) -> None:
    ...
    for (owner, key), replacement in _SCALAR_PROPERTIES.items():
        ...
        if target is None:
            if strict:
                raise RuntimeError(f"netviz.schema: no property {owner}.{key} to override")
            continue
...
def _apply_conditionals(definitions: dict[str, Any], *, strict: bool) -> None:
    for owner, rules in _CONDITIONALS.items():
        ...
        if definition is None:
            if strict:
                raise RuntimeError(f"netviz.schema: no definition {owner} to constrain")
            continue
...
_DOCUMENT_DESCRIPTION: Final = (
    f"One netviz element declared in YAML, `apiVersion: {API_VERSION}`. This schema is "

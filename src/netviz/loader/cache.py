...
the files on disk are the only state netviz trusts — but it is also the same
work every time, and the loops where it hurts are the ones where almost nothing
changed: ``netviz watch`` re-renders on a keystroke-sized edit, ``validate``
...
netviz version, the document ``apiVersion``, the YAML parser actually selected
...
netviz's own sources. That last one is what makes the cache safe to have
switched on while *developing* netviz: editing a validator changes the stamp,
...
(``netviz validate --format json``), because provenance *is* the YAML node
tree.
...
happened so ``netviz cache info`` and ``-v`` can say so.
"""
...
#: netviz would misread. It is part of :class:`Identity`, so a bump invalidates
...
CACHE_DIR_ENV_VAR: Final = "NETVIZ_CACHE_DIR"
...
DISABLE_ENV_VAR: Final = "NETVIZ_NO_CACHE"
...
class Identity:
    ...
    Two netvizs that agree on all of this will draw the same conclusion from
    ...
    *generation* directory entries are filed under, so a change here neither
    ...
    format_version: int
    #: The netviz distribution's own version.
    netviz: str
    ...
    #: A digest over netviz's own source files; see :func:`source_stamp`.
    source_stamp: str

    @classmethod
    def current(cls) -> Identity:
        """The identity of this interpreter, this netviz, this checkout."""
        return cls(
            format_version=FORMAT_VERSION,
            netviz=netviz.__version__,
    ...
    def generation(self) -> str:
        ...
        return f"{_slug(self.netviz)}-{self.fingerprint}"

    def describe(self) -> tuple[tuple[str, str], ...]:
        """Label/value pairs for ``netviz cache info``."""
        return (
...
def source_stamp() -> str:
    """A digest over the size and mtime of every netviz source file.

    The version number is the honest answer to "which netviz produced this
    ...
    """
    root = Path(netviz.__file__).resolve().parent
    digest = hashlib.sha256()
    try:
...
def resolve_cache_root(*, configured: Path | None = None, environ: Any = None) -> tuple[Path, str]:
    ...
    2. ``[cache] dir`` in the inventory's ``netviz.toml``, already resolved
       against that file by :mod:`netviz.config`.
    ...
    Returns:
        ...
        what ``netviz cache info`` prints — "the cache is not where I think it
    ...
    if configured is not None:
        return configured, "netviz.toml [cache] dir"
    xdg = (env.get("XDG_CACHE_HOME") or "").strip()
    if xdg:
        return Path(xdg).expanduser() / "netviz", "XDG_CACHE_HOME"
    if PLATFORM == "win32":  # pragma: no cover - platform fork
        local = (env.get("LOCALAPPDATA") or "").strip()
        base = Path(local) if local else Path.home() / "AppData" / "Local"
        return base / "netviz" / "Cache", "LOCALAPPDATA"
    if PLATFORM == "darwin":  # pragma: no cover - platform fork
        return Path.home() / "Library" / "Caches" / "netviz", "~/Library/Caches"
    return Path.home() / ".cache" / "netviz", "~/.cache"
...
class DocumentCache:
    ...
    load used. Nothing in netviz mutates a loaded element — only the importer
    ...
    def put(self, key: str, entry: CachedFile, *, path: Path | None = None) -> None:
        ...
        try:
            ...
            # An unwritable cache is a slow netviz, not a broken one. Stop
    ...
    def flush(self) -> None:
        ...
        ``netviz list`` into a directory walk.
        """
    ...
    def sweep(self) -> None:
        ...
        netviz that has since been upgraded — the oldest thing in the cache
...
def _decode(blob: bytes, *, key: str, path: Path, relative: str) -> CachedFile | None:
    ...
    lied, an older netviz may have written a shape this one does not know. The
    ...
    try:
        ...
        PydanticValidationError,  # an element this netviz does not accept
    ):
        return None
    try:
...
@dataclass(frozen=True, slots=True)
class CacheInfo:
    """What ``netviz cache info`` reports."""
    ...
    entries: int = 0
    used_bytes: int = 0
    #: Entries and bytes left behind by another netviz or another format.
    stale_entries: int = 0
    stale_bytes: int = 0

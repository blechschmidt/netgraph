...
from netviz.errors import SchemaIssue, format_path
from netviz.loader.provenance import Provenance, Site
...
class SourceLocation:
    ...
    line: int | None = None
    #: The document's redirect table (:mod:`netviz.loader.provenance`), kept so
...
class LoadError:
    ...
    def from_issue(
        ...
        *,
        ...
        """Build a record from one :class:`~netviz.errors.SchemaIssue`."""
        return cls(
...
class Inventory:
    ...
    #: ``netviz validate`` rather than only discovered to be broken by the
...
def subset(inventory: Inventory, names: Container[str]) -> Inventory:
    ...
    derivation with a filter threaded through it. ``netviz report`` builds a
    per-site address plan, VLAN table, cable schedule and power schedule by
    handing this to :mod:`netviz.listing`, :mod:`netviz.ipam` and
    :mod:`netviz.power` unchanged, which is what makes a site page and the
    ...
    :func:`~netviz.render.graph.build_graph` would report the far end as

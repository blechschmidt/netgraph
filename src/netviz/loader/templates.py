...
* **Every other list** — ``vlans``, ``members``, ``addresses``,
  ...
  netviz has no key for cannot be merged without inventing one, and a rule
...
device that inherited it. See :mod:`netviz.loader.provenance`.
"""
...
from netviz.errors import SchemaIssue, echo_value
from netviz.loader.documents import RawDocument
...
#: The one list netviz knows a key for, and therefore the one it merges.
    ...
    *,
    ...
    for key, value in inherited.items():
        ...
        if key == INTERFACES_KEY and isinstance(own, list) and isinstance(value, list):
            ...
            # A scalar, or a list netviz has no key for: the device replaces

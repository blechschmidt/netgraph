...
:mod:`netviz.loader.cache` short-circuit it: a file whose bytes have been seen
...
from netviz.errors import LoaderError, SchemaError, SchemaIssue
from netviz.loader.cache import CachedFile, CachedSlot, DocumentCache
...
from netviz.loader.ignore import IGNORE_FILE_NAME, IgnoreStack, parse_ignore_file
from netviz.loader.inventory import Inventory, LoadError, SourceLocation, qualify
from netviz.loader.provenance import FieldPath, Provenance, Site
from netviz.loader.templates import INHERIT_KEY, TemplateRegistry, resolved_spec
...
class Overlay:
    ...
    :mod:`netviz.edit` has to answer one question before it writes anything:
    ...
    *,
    ...
    Args:
        ...
        keep_provenance: Record, on every element's
            ...
            readable ``netviz validate`` formats need it, and they ask.
        cache: A store of already-parsed files
            (:mod:`netviz.loader.cache`), or ``None`` to parse everything.
...
def load_stream(text: str, *, name: str = STREAM_NAME, keep_provenance: bool = False) -> Inventory:
    ...
    recorded on :attr:`~netviz.loader.inventory.Inventory.errors` and the
...
def iter_inventory_files(
    ...
    loop, a broken ``.netvizignore`` — are appended to ``errors`` when a list
...
def _push_ignore_file(pending: _Pending, problems: list[LoadError]) -> IgnoreStack:
    """Add this directory's ``.netvizignore`` to the stack, if it has one."""
...
class _Builder:
    ...
    def _add_test_suite(self, document: RawDocument, entry: InventoryFile) -> None:
        ...
        and whether those names resolve is ``netviz test``'s question rather
    ...
    def _add_theme(self, document: RawDocument, entry: InventoryFile) -> None:
        ...
        colour nobody can spell should be a finding from ``netviz validate``
    ...
    def harvest(self, mark: _Mark) -> CachedFile | None:
        ...
        * A file declaring a ``kind: layout``, a ``kind: testsuite`` or one of
        ...
        per-file cache. They are counted, so ``netviz cache info`` can say how

"""Exclusion of inventory paths through ``.netvizignore`` files.
...
* A pattern containing a ``/`` anywhere but at the end is anchored to the
  directory holding the ``.netvizignore``; otherwise it matches a basename at
  any depth.
...
never descends into a pruned directory. A ``.netvizignore`` in a subdirectory
...
IGNORE_FILE_NAME: Final = ".netvizignore"
...
class IgnoreRule:
    """One pattern line of a ``.netvizignore``."""
...
@dataclass(frozen=True, slots=True)
class IgnoreRuleSet:
    """The rules of a single ``.netvizignore`` plus the subtree they govern."""
...
class IgnoreStack:
    ...
    Deeper files are evaluated last so that a nested ``.netvizignore`` can
...
def parse_ignore_file(path: Path, *, base: str) -> IgnoreRuleSet:
    """Read and compile a ``.netvizignore``.
...
def compile_rules(lines: Iterable[str]) -> list[IgnoreRule]:
    """Compile the pattern lines of a ``.netvizignore``, skipping comments."""

...
so :mod:`netviz.review.comment` never has to.
...
reported (:attr:`~netviz.review.model.FindingDelta.base_absent`) and the
...
from netviz.plan.model import Plan, StateRef
from netviz.plan.sources import MissingInventory, git_ref
from netviz.plan.state import state_digest
from netviz.review.model import Diagram, Review, compute_delta
...
def _side(stack: ExitStack, root: Path, spec: str) -> tuple[Inventory | None, StateRef]:
    """Load a folder or a git ref, the way ``netviz plan`` loads one.

"""One pull-request comment, rendered from a :class:`~netviz.review.model.Review`.
...
from netviz.plan.model import ACTION_SIGILS, Action, Change
from netviz.review.model import ACTION_LABELS, KindSummary, Review, Verdict
...
MARKER_PREFIX: Final = "<!-- netviz-review:"
...
def _element_details(review: Review, kinds: Sequence[KindSummary]) -> Iterator[str]:
    ...
    if dropped:
        yield (
            ...
            f"`plan.json` of the run's artifact, or run `netviz plan` locally."
        )
        yield ""
    yield "</details>"
...
def _footer(review: Review) -> Iterator[str]:
    version = f"netviz {review.version}" if review.version else "netviz"
    yield (
        f"<sub>{version} · this comment is edited in place on every push · "
        f"reproduce it with <code>netviz review --from {_plain(review.base)}</code></sub>"
    )

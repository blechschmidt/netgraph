...
:mod:`netviz.review.run` from things that read the world, and rendered by
:mod:`netviz.review.comment` without touching it again, so the two halves can
...
from netviz.diagnostics import Diagnostic, fingerprint
...
class FindingDelta:
    ...
    Identity is :func:`netviz.diagnostics.fingerprint` — rule, file, element,
...
class Review:
    ...
    def path_of(self, diagnostic: Diagnostic) -> str | None:
        ...
        The same join :class:`netviz.diagnostics.Report` does for SARIF, and
...
def summarise(review: Review) -> dict[str, Any]:
    ...
    return {
        ...
        "tool": {"name": "netviz", "version": review.version},

...
``netviz fmt``. The one thing worth knowing from outside is that it does not
share a parser with the rest of netviz:

* ``validate`` and ``render`` load through :mod:`netviz.loader.documents`,
...
it is written (:mod:`netviz.fmt.verify`), so the two cannot quietly disagree
about what a file says.
"""

from __future__ import annotations

from netviz.fmt.canonical import (
    INDENT,
    SEQUENCE_INDENT,
    SEQUENCE_OFFSET,
    WIDTH,

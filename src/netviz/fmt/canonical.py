...
This is the only place in netviz that parses YAML with anything other than
:mod:`netviz.loader.documents`. It has to be: the strict loader throws away
...
meaning survived (see :mod:`netviz.fmt.verify`).
...
2. **Normalise** it against the schema shape of its ``kind``
   (:mod:`netviz.fmt.order`): reorder keys, restyle scalars
   (:mod:`netviz.fmt.scalars`), and demote a flow collection that no longer
...
from netviz.fmt.order import MappingShape, SequenceShape, Shape, document_shape, order_keys
from netviz.fmt.scalars import (
    canonical_string,
    is_untouchable,
    plain,
    plain_survives,
...
def format_stream(text: str) -> str:
    ...
    try:
        ...
        # library. netviz's own loader resolves that same scalar as the string
    ...
    if not documents:
        ...
        # which is what ``netviz init --minimal`` writes. ruamel has nowhere
...
def _tidy(text: str) -> str:
    ...
    * exactly one trailing newline. An empty stream stays empty rather than
    ...
    by :func:`~netviz.fmt.verify.verify` refusing the result, so a description
    with a blank line in it made ``fmt`` refuse the file rather than corrupt
    it. :func:`~netviz.fmt.scalars.scalar_lines` says which lines those are.
    """
...
def set_scalar(container: CommentedMap | CommentedSeq, key: Any, value: str) -> None:
    ...
    :func:`~netviz.fmt.scalars.canonical_string`).
    """
...
def _flow_width(node: CommentedMap | CommentedSeq) -> int | None:
    ...
    A nested collection returns ``None``: netviz's schema has no such value,
...
def _render(value: Any) -> str:
    ...
    is the same one :func:`~netviz.fmt.scalars.canonical_string` will make,

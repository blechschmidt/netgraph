...
``netviz fmt`` rewrites files that are the source of truth for a network, so
...
* Each document is parsed with :mod:`netviz.loader.documents`, so a syntax
...
is a bug in netviz rather than in the user's file, so it says so.
"""
...
from netviz.fmt.scalars import scalar_lines
from netviz.loader.documents import YamlSyntaxError, parse_documents
from netviz.models import parse_document, parse_template
...
def meaning(text: str, *, name: str) -> list[str]:
    """One string per document, capturing everything netviz reads from it.
...
def comments(text: str) -> Counter[str]:
    ...
    :func:`~netviz.fmt.scalars.scalar_lines` is consulted rather than the text

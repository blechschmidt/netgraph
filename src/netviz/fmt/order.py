...
:class:`~netviz.models.interface.Interface` and the formatter places it
without being told.
...
from netviz.models.document import ELEMENT_MODELS
from netviz.models.element import ElementBase
from netviz.models.template import INHERIT_KEY, Template
...
_ANNOTATION_MODELS: Final[tuple[type[NetvizModel], ...]] = (Note, Area, Legend)
...
#: What :func:`document_shape` returns and :mod:`netviz.fmt.canonical` walks.
...
def document_shape(kind: object) -> Shape:
    ...
    that is not a kind netviz knows yields the bare envelope shape, so a
...
def _reachable_models() -> list[type[NetvizModel]]:
    ...
    found = [key for key in memo if isinstance(key, type) and issubclass(key, NetvizModel)]
    return [*roots, *found]
...
def _field_names(model: type[NetvizModel]) -> tuple[str, ...]:
    """The YAML keys of ``model``, in declaration order.

    Only ``apiVersion`` carries an alias (see ``netviz.models.base``), but
...
def _concrete_shape(annotation: Any, memo: dict[Any, Shape]) -> Shape:
    ...
    if origin is dict:
        ...
        return OPAQUE
    if isinstance(annotation, type) and issubclass(annotation, NetvizModel):
        return _model_shape(annotation, memo)
    return OPAQUE


def _model_shape(model: type[NetvizModel], memo: dict[Any, Shape]) -> Shape:

...
**A resolver disagreement.** netviz's loader is YAML 1.2 about booleans
(``netviz.loader.documents`` drops the 1.1 rule), so ``yes`` is the string
"yes" here and the boolean ``true`` in almost every other YAML reader. A value
whose plain form means one thing to netviz and another to PyYAML's stock
...
``1.0`` is already a float to every reader including netviz's, so it never
...
quotes to ``1:02`` — which netviz's own loader reads as the integer 62 —
would silently turn it into a string. So every rule above is gated on the plain
form already being a string to netviz: :func:`quote_style` refuses to quote
anything else. ``netviz fmt`` canonicalises documents; it does not repair
them, and a value the loader misreads is a job for ``netviz validate``.
...
from netviz.loader.documents import StrictSafeLoader, scan_tokens
from netviz.models.routing import ROUTE_DISTINGUISHER_PATTERN
...
class _StrictResolver(yaml.resolver.Resolver):
    """PyYAML's resolver carrying netviz's implicit-tag table.
    ...
    1.2 boolean rule ``netviz.loader.documents`` adds after class creation.
    """

    yaml_implicit_resolvers = _STRICT_LOADER.yaml_implicit_resolvers


#: How netviz reads a plain scalar, and how the rest of the world does.
...
def _tags(text: str) -> tuple[str, str]:
    """The implicit tags netviz and a stock YAML 1.1 reader give ``text``.
...
def is_untouchable(value: str) -> bool:
    """Is ``value`` a plain scalar netviz does not read as a string at all?

    ``1:02`` written bare is the integer 62 to netviz's loader, and ruamel
...
def quote_style(value: str) -> str | None:
    ...
    certain, whereas a plain one is only a string if netviz's resolver says so.
    """
    text = plain(value)
    ...
    if strict_tag != _STR_TAG:
        # Plain, ``text`` is not a string to netviz today (``1:02`` is 62).
    ...
    if permissive_tag != _STR_TAG:
        ...
        # where netviz sees a string: ``yes``, ``no``, ``on``, ``off``.
        return QUOTE
...
def plain_survives(text: str, *, in_flow: bool) -> bool:
    ...
    Asked of netviz's own parser rather than answered from the YAML grammar,
...
def canonical_string(value: str, *, in_flow: bool = False) -> str:
    ...
    See :func:`~netviz.fmt.canonical.set_scalar` for the assignment side of
    the same point.
...
def scalar_lines(text: str) -> frozenset[int]:
    ...
    they typed. The same question comes up in :mod:`netviz.fmt.verify`, where

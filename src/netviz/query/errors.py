...
<message>``, :meth:`netviz.loader.inventory.LoadError.location`), so a reader
who has seen one netviz diagnostic recognises this one. The caret line under
...
:func:`~netviz.errors.echo_value` applies to a rejected scalar, for the same
...
class QueryError(NetvizError):
    ...
    (:class:`~netviz.errors.ConfigurationError`), because a bad query is a bad

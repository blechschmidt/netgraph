...
The evaluator is a fold over the tree in :mod:`netviz.query.ast`, and it is
...
would be a lie. ``netviz query --interfaces`` prints exactly the positive
...
bounded by the graph. Attribute reads go through :class:`~netviz.query.facts.
...
from netviz.query.attributes import Attribute, Domain, ValueType, lookup
from netviz.query.errors import QueryError
from netviz.query.facts import (
    Facts,
    LinkEnd,
    element_values,
    interface_values,
...
from netviz.query.parser import Query
...
#: :data:`~netviz.query.parser.MAX_TERMS` of them and the editor re-evaluates
...
class QueryResult:
    ...
    the order every other netviz listing uses, and a result that reordered
    itself would not line up with ``netviz list`` beside it.
    """
    ...
    def interfaces(self) -> tuple[Witness, ...]:
        """The interface witnesses alone, for ``netviz query --interfaces``."""
...
def evaluate(query: Query, graph: Graph) -> QueryResult:
    ...
    Args:
        query: A query already parsed and checked by
            :func:`~netviz.query.parser.parse`.
...
def _compare(values: Sequence[str], expr: Comparison, attribute: Attribute) -> bool:
    ...
    :mod:`netviz.query.attributes` — this function is that paragraph.
    """
    if not values:
        return False

...
sixth field of the same :class:`~netviz.render.graph.FilterSpec`, and this
...
3. hand the resulting set of names to
   :func:`~netviz.render.graph.filter_graph`, which does the graph surgery it
...
from netviz.query.evaluate import evaluate
from netviz.query.parser import Query, parse
from netviz.render.graph import FilterSpec, Graph, filter_graph
...
def narrow(graph: Graph, spec: FilterSpec, *, source: str = "--select") -> Graph:
    ...
    The drop-in for :func:`~netviz.render.graph.filter_graph` at every call

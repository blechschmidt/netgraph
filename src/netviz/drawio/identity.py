...
``hash``
    A digest of the element's body (:func:`~netviz.plan.document.body_of`) as
...
``x`` / ``y`` / ``width`` / ``height`` / ``waypoints``
    Where the cell was when it left netviz, in netviz's own coordinates.
...
Nothing here reads or writes XML; :mod:`netviz.drawio.mxfile` does that, and
:mod:`netviz.drawio.reconcile` is what acts on the answers.
"""
...
#: which keeps the document well-formed; :mod:`netviz.drawio.mxfile` also
...
NAMESPACE_PREFIX: Final = "netviz"

#: The namespace the prefix is bound to. A URL that is netviz's, never
#: fetched: it is an identifier, in the XML tradition.
NAMESPACE_URI: Final = "https://netviz.dev/schema/drawio/v1"
...
#: Every attribute netviz writes, so a reader can tell one of its own from a
...
def qualified(attribute: str) -> str:
    """``kind`` → ``netviz:kind``, the spelling that goes in the file."""
    return f"{NAMESPACE_PREFIX}:{attribute}"


class CellRole(str, Enum):
    """What a netviz-authored cell is."""

...
:mod:`netviz.edit` operations without ever *guessing*. Everything here is
built on one rule: **identity comes from the attributes, never from the label
and never from the position**. A cell that still carries ``netviz:name``
...
behind, as faithfully as :mod:`netviz.drawio.markup` can — and one that is
...
(:attr:`~netviz.drawio.identity.Scope.COMPLETE`). Export a diagram narrowed by
...
A file netviz did not write gets the best-effort treatment: the kind is
...
:class:`~netviz.edit.session.EditSession`, and what the session's result
*means* is shown to the user as a :mod:`netviz.plan` changeset before a single
byte is written.
"""
...
from netviz.drawio.build import element_of
...
from netviz.drawio.model import Cell, Diagram, absolute_geometry
from netviz.drawio.notes import Level, Note
from netviz.drawio.styles import NOTE_SHAPE
from netviz.edit.cascade import placed_element, plan_cascade
...
from netviz.edit.rename import respelled_key
from netviz.layout.document import inline_entry
from netviz.layout.geometry import COORDINATE_PLACES, round_coordinate
from netviz.layout.resolve import resolve_key
from netviz.layout.seed import DEFAULT_LAYOUT_NAME
from netviz.loader.inventory import Inventory, namespace_of, short_name
from netviz.models.annotation import AREA_KIND, NOTE_KIND, Annotation
from netviz.models.element import KINDS
from netviz.models.interface import CABLEABLE_TYPES
from netviz.plan.document import body_of
from netviz.render.annotations import annotation_views, parse_markup
from netviz.render.graph import Graph, Node
...
#: are stored to :data:`~netviz.layout.geometry.COORDINATE_PLACES`, so
...
class ReconcileOptions:
    ...
    #: their cells as unmapped: they are netviz's own, whether or not this
...
class Reconciliation:
    ...
    annotated: int = 0
    #: Cells netviz could not map onto anything.
    unmapped: int = 0
        ...
        return Reconciliation(
            notes=(
                Note(
                    Level.ERROR,
                    "",
                    f"the diagram was written by a newer netviz (model version "
                    f"{diagram.version}, this one reads {MODEL_VERSION}); upgrade netviz "
...
class _State:
    ...
    #: Layout key to its new position, in netviz coordinates.
...
def _node_cell(state: _State, cell: Cell, cells: Mapping[str, Cell], seen: set[str]) -> None:
    """One vertex netviz wrote: a possible move, and a possible rename."""
    ...
    if not key:
        ...
        state.note(
            Level.WARNING,
            cell.id,
            "carries a netviz role but no node key, so netviz cannot tell what it "
...
def _move(state: _State, cell: Cell, cells: Mapping[str, Cell], key: str) -> None:
    ...
    The comparison is against the position netviz *stamped into the cell*,
    ...
    centre = state.diagram.frame.box_to_netviz(x, y, width, height)
    exported = _exported_position(cell)
    if exported is None:
        state.note(
            Level.WARNING,
            key,
            "netviz did not stamp a position on this cell, so there is nothing to compare "
...
def _link_cell(state: _State, cell: Cell, cells: Mapping[str, Cell], seen: set[str]) -> None:
    """One edge netviz wrote: a possible reroute, or a link left alone."""
...
def _reroute(state: _State, cell: Cell, key: str) -> None:
    ...
    current = tuple(frame.to_netviz(x, y) for x, y in cell.points)
...
def _foreign_cell(state: _State, cell: Cell, cells: Mapping[str, Cell]) -> None:
    """A cell netviz did not write, inside a file netviz did.
    ...
    if not cell.edge:
        ...
        state.note(
            Level.INFO,
            cell.id,
            f"is a cell netviz did not write ({_describe(cell)}); it was left in the "
...
def _endpoints(state: _State, cell: Cell, cells: Mapping[str, Cell]) -> tuple[str, str] | None:
    ...
    if source is None or target is None:
        ...
        state.note(
            Level.WARNING,
            cell.id,
            "is an edge drawn between something other than two netviz elements, so there "
    ...
    if any(port is None for port in ports):
        ...
        state.note(
            ...
            "one and re-import, or connect it with 'netviz edit connect'",
        )
        return None
...
def _annotation_cell(
    ...
    """One note or area netviz wrote: what the canvas now says about it.
    ...
    if not fqn:
        ...
        state.note(
            Level.WARNING,
            cell.id,
            f"is a {kind} cell with no name on it, so netviz cannot tell which document it "
            "stands for; it was left alone",
        )
        return
    seen.add((kind, fqn))
...
def _annotation_geometry(
    ...
    Compared against the box netviz stamped into the cell, exactly as a node
    is: an annotation whose box netviz *computed* — a zone following its
...
def _write_geometry(
    ...
    *,
    ...
    :class:`~netviz.edit.operations.SetAnnotation` documents as the first drag
...
def _note_text(state: _State, cell: Cell, fqn: str) -> None:
    ...
    if not stamped:
        state.note(
            Level.WARNING,
            fqn,
            "netviz did not stamp this note's source on the cell, so an edit to its text "
...
def _new_note(state: _State, cell: Cell, cells: Mapping[str, Cell]) -> bool:
    ...
    The one thing a draw.io user can *add* to a vertex that netviz is willing
...
def _drawn_annotations(graph: Graph) -> tuple[tuple[str, str], ...]:
    ...
    Resolved through :func:`~netviz.render.annotations.annotation_views` — the
...
def _doomed_keys(state: _State) -> frozenset[str]:
    ...
    :func:`~netviz.edit.cascade.plan_cascade` rather than the deletion list,
...
def _respell(
    ...
    That is the same way round :func:`netviz.edit.apply._rekey` decides it.
    """
    if not renamed:
        return dict(entries)
    placed: dict[str, Any] = {}
...
def _foreign(state: _State) -> Reconciliation:
    ...
    either a new element or noise, and netviz cannot tell which. What it
    *can* do is say what it saw and what it made of each cell, which is the
    difference between "netviz could not read my diagram" and "netviz read
    ...
    notes: list[Note] = [
        Note(
            Level.WARNING,
            "",
            "this file was not written by 'netviz export drawio', so it carries no "
            "identity attributes; nothing can be reconciled against the inventory and "
            "nothing will be changed. What netviz made of each cell follows",
        )
    ]
    unmapped = 0
        ...
        if kind is None:
            ...
            notes.append(
                Note(
                    ...
                    f"({_describe(cell)}); netviz does not guess",
                )
            )
            continue
        notes.append(

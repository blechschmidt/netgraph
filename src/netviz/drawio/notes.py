"""What netviz wants to say about a diagram, in either direction.
...
draw.io user added that netviz could not place, an element that changed under
...
is what lets ``netviz export drawio`` fold its notes into the ordinary export
manifest (:mod:`netviz.export.manifest`) without this package having to import
...
class Level(str, Enum):
    ...
    #: Something netviz did, worth saying out loud.
    INFO = "info"
    #: Something in the diagram netviz could not map, and did not guess at.
    WARNING = "warning"
...
class Note:
    """One thing netviz wants the operator to know about a diagram."""

    level: Level
    #: The cell, element or link it is about, in whatever spelling the reader
    #: will recognise: a cell id for something netviz cannot otherwise name,
    #: and the empty string for something about the diagram as a whole.
    subject: str
    message: str
    #: The :class:`~netviz.export.manifest.Reason` token this maps onto, for

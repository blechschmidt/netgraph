...
:func:`~netviz.render.annotations.parse_markup` defines. An mxGraph cell's
...
(:data:`~netviz.drawio.identity.ATTR_TEXT`) and compares the result with the
...
from netviz.render.annotations import Block, Span
...
#: :data:`~netviz.models.annotation.MAX_TEXT_LENGTH` characters when netviz
...
def markup_html(blocks: Sequence[Block]) -> str:
    ...
    Returns:
        ...
        :mod:`netviz.drawio.mxfile`. Empty for a note with no blocks, which
...
class _Reader(HTMLParser):
    ...
    def result(self) -> str:
        ...
        :func:`~netviz.render.annotations.parse_markup` joins consecutive

...
**Where each one goes.** An annotation is placed in netviz's coordinates,
...
* an **area** takes ``spec.geometry`` when it has one, and otherwise the hull of
  ...
  (:func:`~netviz.render.annotations.member_hull`) — so a zone follows the
  devices it names;
...
separate tuple from everything else, and :mod:`netviz.drawio.build` writes
them before the frames and the nodes. A zone written after its members would
cover them, and a reader would report that netviz exports blank boxes.

**Identity is the same identity.** Every cell carries the block
:mod:`netviz.drawio.identity` defines, with ``kind`` holding the document kind
and ``name`` its fully-qualified name, so ``netviz import drawio`` reconciles a
...
from netviz.drawio.markup import escape_html, markup_html
from netviz.drawio.model import ROOT_ID, Cell, Frame
from netviz.drawio.styles import (
    area_style,
    leader_style,
    legend_style,
    note_style,
...
from netviz.layout.geometry import Box
from netviz.loader.inventory import Inventory
from netviz.models.annotation import AREA_KIND, LEGEND_KIND, NOTE_KIND
...
#: the label printed under a node. The same offset :mod:`netviz.render.dot`
...
class _Boxed:
    """One annotation's rectangle in netviz coordinates, and where it came from."""
...
def place_annotations(
    ...
    """Give every annotation of ``views`` a rectangle in netviz coordinates.

    Args:
        views: The resolved annotations, from
            :func:`~netviz.render.annotations.annotation_views`.
...
def _note_boxes(
    ...
    note — :class:`~netviz.render.annotations.NoteView` never drops one.
    """
...
def _legend_box(legend: LegendView, drawing: Box | None) -> Box:
    ...
    covers whatever the arrangement put in that corner, and netviz has no way
...
def annotation_cells(
    ...
    Args:
        ...
        frame: The map from netviz's coordinates to the page's.
...
def _cell_id(kind: str, fqn: str) -> str:
    ...
    from netviz.drawio.build import cell_id

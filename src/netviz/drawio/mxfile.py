...
what netviz writes by default, because a diagram that is text is a diagram
...
Neither encoding is a container format netviz invented, so both are handled
conservatively:
...
* **No external entities, no DTD.** A ``<!DOCTYPE`` or ``<!ENTITY`` declaration
  ...
  attacks that every stdlib XML parser is open to. netviz is going to be
...
* **Forgiving about the namespace.** The ``netviz`` prefix is declared on the
...
from netviz.drawio.model import ROOT_ID, Cell, Diagram, Frame
from netviz.errors import NetvizError, clip_text
...
#: What goes in ``<mxfile agent="…">``: which netviz wrote the file, which is
#: the first question when a stale diagram turns up.
#:
#: Spelled here rather than imported from
#: :data:`netviz.export.header.GENERATOR`, which says the same words, because
...
#: module that reaches for :mod:`netviz.drawio` first cannot import it at all.
#: ``tests/test_drawio.py`` asserts the two strings are equal, so they cannot
#: drift apart in silence.
AGENT: Final = f"netviz {__version__}"


class DrawioFormatError(NetvizError):
    """The file is not a draw.io diagram netviz can read.
...
#: How the netviz attributes reach ElementTree once the prefix is bound.
...
def write_mxfile(diagram: Diagram, *, compress: bool = False) -> str:
    ...
    Args:
        ...
        compress: Write the deflate+base64 encoding draw.io's desktop app
            ...
            reviews and diffs, and netviz's whole argument is that a diagram
            should be readable.
...
def _diagram_id(diagram: Diagram) -> str:
    ...
    return f"netviz-{diagram.view or 'diagram'}"
...
def _cell_lines(cell: Cell, *, indent: int) -> Iterator[str]:
    ...
    Every cell netviz writes is wrapped, even one with a bare label: the
...
def _number(value: float) -> str:
    ...
    :func:`netviz.drawio.identity._number` uses them: ``%g`` would round a
...
def parse_mxfile(text: str, *, source: str = "<input>") -> Diagram:
    ...
    if _UNSAFE_DECLARATION.search(text):
        raise DrawioFormatError(
            f"{source} carries a document type or entity declaration; netviz does not "
...
def _ensure_namespace(text: str) -> str:
    """Bind the ``netviz`` prefix on the root element when nothing else has.
...
def _diagram_name(root: ElementTree.Element) -> str:
    ...
    return (element.get("name") or "netviz") if element is not None else "netviz"
...
def _model_element(root: ElementTree.Element, *, source: str) -> ElementTree.Element:
    ...
    if diagram is None:
        raise DrawioFormatError(
            ...
            "draw.io file netviz knows how to read"
        )
...
def _cell(element: ElementTree.Element, *, wrapper: ElementTree.Element | None) -> Cell:
    ...
    attributes = _netviz_attributes(carrier)
...
def _role(value: str | None) -> CellRole | None:
    """The declared role, or ``None`` for a cell netviz did not write."""
    if value is None:
        return None
    try:
...
def _netviz_attributes(element: ElementTree.Element) -> Mapping[str, str]:
    """The ``netviz:*`` attributes of one element, prefix stripped.

    Both spellings are accepted: the expanded ``{uri}kind`` a namespace-aware
    parser produces, and the literal ``netviz:kind`` that reaches us when an

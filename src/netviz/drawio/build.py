...
``netviz:placed="auto"`` and recorded in the manifest. The mark matters: an
invented position that comes home unchanged must not be written into the tree as
though somebody had chosen it.

**What identifies a cell.** Not the label. See :mod:`netviz.drawio.identity`.
...
from netviz.drawio.annotations import PlacedAnnotations, annotation_cells, place_annotations
...
from netviz.drawio.model import MARGIN, ROOT_ID, Cell, Diagram, Frame, cell_id
from netviz.drawio.notes import Level, Notes
from netviz.drawio.styles import edge_style, group_style, icon_data_uri, node_style
from netviz.layout.geometry import Box, Placement
...
from netviz.render.graph import Edge, Graph, Node
...
#: byte-stable. ``netviz layout --write`` is the way to get a real one.
...
#: The :class:`~netviz.export.manifest.Reason` tokens this builder's notes
...
class BuildOptions:
    ...
    #: The view being drawn, as :class:`~netviz.render.graph.Layer` spells it.
    view: str
    ...
    #: :attr:`~netviz.drawio.identity.Scope.PARTIAL`, and importing one never
    ...
    #: netviz's almost one for one, so a colour chosen in a manifest reaches
...
def build_diagram(
    ...
    *,
    ...
    Returns:
        A :class:`~netviz.drawio.model.Diagram` whose cells are in a fixed
...
def _annotations(
    ...
    """Every annotation of this view, boxed in netviz coordinates (§21).
...
@dataclass(frozen=True, slots=True)
class _Placed:
    """One node's box in netviz coordinates, and where the box came from."""
...
def _placements(graph: Graph, nodes: Sequence[Node], record: Notes) -> Mapping[str, _Placed]:
    ...
    if invented:
        record.add(
            ...
            "the export; run 'netviz layout --write' to make the arrangement part of the "

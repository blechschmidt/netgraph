"""``netviz export drawio`` — the inventory as an mxGraph diagram.
...
the whole reason it exists: netviz's argument is that a network diagram and a
...
* Every cell carries the identity attributes of
  :mod:`netviz.drawio.identity`, which is what makes ``netviz import
...
* Every icon is inlined as a data URI, so the file is one file — it can be
  mailed, and it will draw the same on a machine that has never seen netviz.
...
* Every annotation of §21 — a note, a zone, a key — is a **native mxGraph
  ...
  ``netviz import drawio`` reads the edit back into the document it came from.
...
from netviz.drawio.identity import ATTR_KIND, ATTR_NAME, CellRole, Scope
from netviz.drawio.model import Diagram
from netviz.drawio.mxfile import write_mxfile
from netviz.drawio.notes import Note, Notes
...
def _record(recorder: Recorder, notes: tuple[Note, ...]) -> None:
    ...
    The translation lives here rather than in :mod:`netviz.drawio` because

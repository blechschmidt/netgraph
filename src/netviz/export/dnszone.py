...
Prefixes are **not** re-derived here. :func:`netviz.subnets.subnets_of` is the
one implementation of "which prefixes exist, and who sits in them?", shared with
the layer-3 diagram, ``netviz list subnets`` and ``netviz ipam``; the reverse
zones are a regrouping of its output onto delegation boundaries
(:func:`~netviz.export.context.reverse_zone_of`) and nothing more.
...
(:mod:`netviz.export.names`) and every fold is recorded in the manifest.
"""

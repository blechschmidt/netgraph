...
output drops straight in::
    ...
    netviz export ansible-inventory -o inventory.json
...
``ansible_host``
    ...
    :func:`~netviz.export.context.management_address` — a management
...
from netviz.export.names import ansible_identifier
from netviz.models import Device
from netviz.render.graph import Layer, Node, PortView
...
def emit(context: ExportContext) -> str:
    ...
    document: dict[str, Any] = {
        ...
        "all": {
            ...
            "vars": {"netviz_generated_by": GENERATOR},
        },
    }
...
def _host_vars(node: Node, management: str) -> dict[str, Any]:
    ...
    Namespaced under ``netviz_`` apart from ``ansible_host``: a variable this
    ...
    variables: dict[str, Any] = {
        ...
        "netviz_element": node.fqn,
        "netviz_name": node.name,
        "netviz_kind": node.kind,
        "netviz_namespace": node.namespace,
    }
    if node.description:
        variables["netviz_description"] = node.description
    for key, value in (
        ("netviz_vendor", getattr(spec, "vendor", None)),
        ("netviz_model", getattr(spec, "model", None)),
        ("netviz_serial", getattr(spec, "serial", None)),
    ):
        if value:
            variables[key] = value
    if node.labels:
        variables["netviz_labels"] = dict(sorted(node.labels.items()))
    location = {
        key: value
        for key, value in (
            ("site", site),
    ...
    if location:
        variables["netviz_location"] = location

    variables["netviz_addresses"] = [address.cidr for address in element_addresses(node)]
    variables["netviz_interfaces"] = [_interface(port) for port in node.ports]
    variables["netviz_vlan_ids"] = sorted(node.vlans)
    vlans = _vlan_database(node)
    if vlans:
        variables["netviz_vlans"] = vlans
    return variables

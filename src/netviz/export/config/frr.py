...
``frr.conf`` and neither is written here. netviz does not know which release
...
from netviz.export.config.plan import DevicePlan, addresses_of, netns_limits
...
def declines(plan: DevicePlan) -> str:
    ...
    return (
        ...
        f"route. Its interfaces are 'netviz export netplan' or 'netviz export interfaces'"
    )
...
def _version_note() -> Iterator[str]:
    ...
    yield "! No 'frr version' or 'frr defaults' line is written here: netviz does not know"
...
def _record_interface(plan: DevicePlan, interface: Interface, recorder: Recorder) -> None:
    ...
    if interface.type in _STACKED:
        recorder.skip(
            ...
            f"built; create it with 'netviz export netplan' or 'netviz export networkd'",
        )
        ...
        recorder.skip(
            ...
            f"'netviz export wireguard' or 'netviz export netplan'. FRR routes over the "

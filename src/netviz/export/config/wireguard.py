...
interfaces are none of its business — ``netviz export netplan`` or
``netviz export interfaces`` describes those.
...
:mod:`netviz.export.config.netplan` narrows them: widening ``10.9.0.2/24`` to
...
(:class:`~netviz.export.config.model.Unsupported`), named with the tunnel
...
from netviz.export.config.plan import DevicePlan, TunnelPeer, TunnelPlan, addresses_of
from netviz.export.manifest import Reason, Recorder
from netviz.models.tunnel import TunnelAuth, TunnelType
...
#: :mod:`netviz.export.config.netplan` spells it so that a reader who has both
...
def declines(plan: DevicePlan) -> str:
    ...
    return (
        ...
        f"'netviz export netplan' or 'netviz export interfaces'"
    )
...
def _notes(tunnel: TunnelPlan) -> tuple[str, ...]:
    notes = [
        ...
        f"file holds {_PLACEHOLDER} where the keys go, because netviz stores no key",
...
def _interface(plan: DevicePlan, tunnel: TunnelPlan, recorder: Recorder) -> Iterator[str]:
    ...
    if mtu is not None:
        yield f"MTU = {mtu}"
    yield (
        f"PrivateKey = {_PLACEHOLDER}  # private key of {plan.name}; netviz holds no "

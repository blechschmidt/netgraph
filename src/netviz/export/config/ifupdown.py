...
— word for word the commands :mod:`netviz.export.routes` writes, because two
...
``# netviz holds no keys`` after a passphrase would become part of the
...
Three things are refused (:data:`~netviz.export.config.model.Unsupported`)
...
from netviz.export.config import netplan
...
from netviz.models.interface import Security
...
def _notes(plan: DevicePlan) -> tuple[str, ...]:
    ...
    if plan.forwards_ipv4 or plan.forwards_ipv6:
        ...
        notes.append("not in this file. See 'netviz export interfaces' for the declared state.")
    return tuple(notes)
    ...
    for interface in plan.interfaces:
        if interface.type is InterfaceType.LOOPBACK:
            recorder.skip(
                ...
                "netviz's; a second stanza for the loopback would fight the first, and a "
...
def _record_tunnel(plan: DevicePlan, interface: Interface, recorder: Recorder) -> None:
    ...
    if tunnel is not None and tunnel.type is TunnelType.WIREGUARD:
        recorder.skip(
            ...
            f"generate it with 'netviz export wireguard'",
        )
        return
    recorder.skip(
        subject,
...
def _unattached(route: StaticRoute, name: str | None) -> str:
    if route.blackhole and route.dev is None:
        ...
        return (
            ...
            f"command only from an interface coming up; 'netviz export routes' writes it"
        )
    if name is None:
        return (
...
def _route_arguments(plan: DevicePlan, route: StaticRoute) -> str:
    ...
    :mod:`netviz.export.routes`: one route described two ways by one program is
    ...
    if route.table is not None:
        ...
        # :mod:`netviz.export.routes` writes it: a name resolves only through

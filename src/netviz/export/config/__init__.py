...
Everything else netviz exports is *about* the network — a hosts file, a zone, a
pull list, a monitoring target. None of them is the network. The configuration a
device actually runs is, and until netviz could write one, the inventory was a
...
Seven dialects, each a pure function from a :class:`~netviz.export.config.plan.
DevicePlan` to a set of files:

===============  =============================================================
``netplan``      ``etc/netplan/10-netviz.yaml`` for a Linux host
...
``interfaces``   netviz's own vendor-neutral rendering, for everything else
===============  =============================================================
...
output. Where a dialect *requires* a value netviz does not hold — a WireGuard
...
:class:`~netviz.export.config.model.Unsupported` naming it, and the whole run
...
:mod:`netviz.importer` reads back: ``netviz drift`` takes these same six
...
from netviz.export.config import (
    frr,
    ifupdown,
    netplan,
    networkd,
...
from netviz.export.config.model import (
    ConfigFile,
    ConfigSet,
    DeviceConfig,
    Unsupported,
...
from netviz.export.config.plan import DevicePlan, device_plans
from netviz.export.context import ExportContext
...
CONFIG_DIALECTS: Final[Mapping[str, ConfigDialect]] = {
    ...
    "interfaces": ConfigDialect(
        ...
        lossy=(
            "nothing — it is netviz's own grammar and holds whatever the inventory states; "
...
def emit(dialect: str) -> Callable[[ExportContext], str]:
    ...
    :meth:`~netviz.export.config.model.ConfigSet.as_stream` writes a single

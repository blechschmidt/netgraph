...
Nothing else in :mod:`netviz.export` touches the filesystem: an emitter returns
...
not the one :func:`netviz.importer.write_files` uses. An imported tree is
...
**A file netviz generated is overwritten.** It carries the banner
(:func:`netviz.banner.parse_banner`), so there is no doubt.
...
from netviz.banner import DIALECT_KEY, parse_banner
from netviz.errors import NetvizError
from netviz.export.config.model import ConfigSet
...
class ConfigWriteError(NetvizError):
    ...
    operator's side both mean netviz could not do the file operation it was
    pointed at.
    """
...
def write_config(
    ...
    Args:
        config: The set to write, from
            :func:`~netviz.export.config.generate`.
        target: The output directory. Created, with its parents, when absent.
        force: Overwrite files netviz did not generate, and write inside the
            inventory tree.
    ...
    Raises:
        ConfigWriteError: ``target`` is inside the inventory, is not a directory,
            holds a file netviz did not write, or a write failed.
    """
    ...
    for relative, content in config.files():
        ...
        try:
            ...
            # LF on every platform; see netviz.fsio. A generated configuration
...
def stale_files(config: ConfigSet, target: Path) -> tuple[Path, ...]:
    """Files under ``target`` netviz generated that this run did not rewrite.
...
def _shown(path: Path) -> str:
    """A path as netviz prints one: forward slashes, whatever the platform.
    ...
    quoting it, which is the reasoning ``netviz fmt``, every finding and
    :func:`netviz.drift.report._root_text` already follow.
    """
    return path.as_posix()


def _refuse_foreign(config: ConfigSet, target: Path) -> None:
    """Refuse the write if it would clobber a file netviz did not write."""
    clashes = [
        relative
    ...
    raise ConfigWriteError(
        f"refusing to overwrite {len(clashes)} file(s) under {_shown(target)} that netviz "
...
def _banner_dialect(path: Path) -> str | None:
    ...
    certainly not something netviz wrote, which is exactly the answer the
    callers need.
    """
    try:

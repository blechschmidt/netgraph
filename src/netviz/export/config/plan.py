...
the inventory states, and the two places where netviz reasons rather than reads
...
dialect declares its own limits (:mod:`netviz.export.config.model`).
"""
...
from netviz.export.config.model import Unsupported
from netviz.export.context import ExportContext, elements_of
from netviz.loader.inventory import Inventory, namespace_of, short_name
from netviz.models import Device, Interface, InterfaceType, StaticRoute, Tunnel
...
def addresses_of(interface: Interface, *, routable_only: bool = False) -> tuple[str, ...]:
    ...
    ``netviz ipam`` apply, which is what a tunnel endpoint wants: a link-local
...
def netns_limits(plan: DevicePlan, dialect: str) -> Iterator[Unsupported]:
    ...
    That is exactly what :class:`~netviz.export.config.model.Unsupported` is
    for — the value is within the dialect's remit, the dialect has no syntax for
    it, and a file without it describes a different machine. ``netviz export
    ...
    for index, entry in enumerate(plan.device.spec.netns):
        yield Unsupported(
            ...
            detail=(
                ...
                f"this file would put the namespace's interfaces on the host. See 'netviz "

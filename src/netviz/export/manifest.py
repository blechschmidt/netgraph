...
**on stderr**, leaving stdout for the artefact itself::
    ...
    netviz export prometheus-sd -o targets.json 2> manifest.json
...
#: the ``NetworkGraph`` document :mod:`netviz.render.jsonexport` emits.
...
class Reason(str, Enum):
    ...
    #: (:func:`netviz.subnets.is_routable_address`).
    ...
    #: already dropped (:attr:`netviz.render.graph.Graph.dangling`). Only
    ...
    #: that is netviz's rather than the operator's, which matters because an

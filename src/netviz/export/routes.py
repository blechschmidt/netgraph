...
from netviz.models import Device, DeviceSpec, PolicyAction, PolicyRule
...
_FUNCTION_PREFIX: Final = "netviz_routes_"
...
def _table_words(
    ...
    A VRF is the exception: iproute2 resolves ``vrf blue`` itself, and netviz
...
def _dispatcher(scripts: Sequence[_Script]) -> Iterator[str]:
    ...
    for script in scripts:
        ...
        yield "        ;;"
    yield "    *)"
    yield "        echo \"netviz: no routing declared for '$target'\" >&2"
    yield "        exit 1"
    yield "        ;;"
    yield "esac"

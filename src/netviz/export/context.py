...
narrowed by the same :class:`~netviz.render.graph.FilterSpec` a render uses —
the format-specific options, and the :class:`~netviz.export.manifest.Recorder`
...
**"Which addresses identify this element on the network?"**
    ...
    :attr:`~netviz.render.graph.PortView.routable_addresses`, which is the
    same exclusion the layer-3 diagram and ``netviz ipam`` apply — one
...
from netviz.export.names import MAX_DNS_NAME, dns_name_parts
from netviz.loader.inventory import Inventory
from netviz.render.graph import Graph, Layer, Node, PortView
from netviz.render.icons import IconTheme
from netviz.render.theme import Theme
from netviz.subnets import IPNetwork
...
class Address:
    ...
    def packed(self) -> tuple[int, int]:
        ...
        :attr:`netviz.subnets.Subnet.sort_key`.
        """
...
class ExportOptions:
    ...
    # -- drawio ----------------------------------------------------------
    #: Which view the diagram draws, as :class:`~netviz.render.graph.Layer`
    ...
    #: netviz's style vocabulary line up almost one for one, so a colour a
    ...
    def at(self, layer: Layer) -> Graph:
        ...
        Raises:
            KeyError: The emitter asked for a layer it did not declare in its
                :attr:`~netviz.export.Exporter.layers`. A programming error,
                not a user one.
        """
...
def record_dangling(graph: Graph, recorder: Recorder) -> None:
    ...
    :attr:`~netviz.render.graph.Graph.dangling` already holds every cable and

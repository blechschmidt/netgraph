...
from netviz.export.context import ExportContext, location_of, record_dangling
from netviz.export.header import html_header
from netviz.export.manifest import Reason
from netviz.export.names import csv_cell, markdown_cell
from netviz.loader.inventory import namespace_of, short_name
from netviz.models import Cable
...
def schedule(context: ExportContext) -> tuple[Row, ...]:
    ...
    ``netviz report`` is this same list of runs in this same order — the whole
...
def _runs(spliced: Graph) -> Mapping[str, tuple[str, str]]:
    ...
    :class:`~netviz.render.graph.PatchView` remembers the segments it stands
...
def _record_missing(context: ExportContext, graph: Graph, emitted: Iterable[str]) -> None:
    ...
    * the graph could not resolve an endpoint — already recorded by
      :func:`~netviz.export.context.record_dangling`, and a broken inventory;
...
def _csv(rows: Sequence[Row]) -> str:
    ...
    Every cell additionally goes through :func:`~netviz.export.names.csv_cell`

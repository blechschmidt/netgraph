"""``netviz export`` — the inventory as operational artefacts.
...
``dns-zone``       An RFC 1035 forward zone and the reverse zones implied by
                   the prefixes :mod:`netviz.subnets` derives.
...
``networkd``,      per device: :mod:`netviz.export.config`. These are the only
...
**Scoped like a render.** The same :class:`~netviz.render.graph.FilterSpec`
...
records what it left out and why (:mod:`netviz.export.manifest`) and the CLI
...
:mod:`netviz.export.names`, with every fold recorded.
...
from netviz.export import ansible, cables, dnszone, drawio, hosts, power, prometheus, routes
...
from netviz.export.config import emit as config_emit
from netviz.export.config import generate as generate_config
from netviz.export.context import ExportContext, ExportOptions
from netviz.export.manifest import MANIFEST_KIND, Manifest, Reason, Recorder, Skip
from netviz.export.names import ansible_identifier, domain_name, is_domain_name, sanitise_label
from netviz.export.prometheus import is_assignable_label, is_label_name
...
class Exporter:
    ...
    #: device (:mod:`netviz.export.config`). ``--out DIR`` writes the tree;

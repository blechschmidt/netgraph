"""The tables behind ``netviz list``, and behind every report that shows one.
...
tables ``netviz report`` writes. A column added here appears in all of them,
...
:func:`~netviz.render.graph.build_graph`, tunnel stacks from
:func:`~netviz.render.graph.resolve_tunnels`, prefixes from
:func:`~netviz.subnets.subnets_of` and power from
:func:`~netviz.power.power_plan` — the same resolutions the diagrams draw. A
host on an untagged access port is a member of that VLAN in the table for the
same reason it is one in the picture.

Every function takes an :class:`~netviz.loader.inventory.Inventory` and nothing
else, so a *scoped* table — one site of a campus — is the same function over a
narrowed inventory (:func:`~netviz.loader.inventory.subset`) rather than a
...
from netviz.ipam import Utilisation, format_capacity, format_utilisation
from netviz.loader.inventory import Inventory, short_name
from netviz.models import GROUP_KIND, Adapter, Device, format_bitrate, format_watts
from netviz.power import format_utilisation_percent, power_plan
from netviz.render.graph import Layer, Node, build_graph, resolve_tunnels
...
def devices(inventory: Inventory, *, layers: Sequence[Layer] = (Layer.L1,)) -> Listing:
    ...
    (§15.2) and a PDU only appears in the power view. ``netviz list devices``
    asks for layer 1 — the topology a reader means by "the devices" — and
    ``netviz report``, which owes a page to every declared element, asks for
...
def tunnels(inventory: Inventory) -> Listing:
    ...
    The stack comes from :func:`~netviz.render.graph.resolve_tunnels`, the
...
def subnets(inventory: Inventory) -> Listing:
    ...
    The grouping is :func:`~netviz.subnets.subnets_of`, the same one
...
def power(inventory: Inventory) -> Listing:
    ...
    Shaped after the ``netviz ipam`` utilisation table, and for the same
...
def users(inventory: Inventory) -> Listing:
    ...
    groups (see :mod:`netviz.models.identity`) — which is exactly why it is
...
def utilisation(rows: Iterable[Utilisation], *, aggregated: bool = False) -> Listing:
    """The ``netviz ipam`` utilisation table: how full each prefix is.

    Not in :data:`LISTINGS`, because it is not a subject of ``netviz list``: it
    takes the rows :func:`netviz.ipam.build_report` derived rather than an
    ...
    The ``UTIL`` cell is plain text here. ``netviz ipam`` colours it by band
...
#: What ``netviz list`` accepts as its subject, in the order ``--help`` shows

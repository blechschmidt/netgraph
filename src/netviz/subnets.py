...
* ``netviz list subnets`` prints it,
* the layer-3 graph (:mod:`netviz.render.graph`) draws one node per prefix and
...
:data:`~netviz.models.GLOBAL_VRF`, which is what every address is in until
...
from netviz.models import GLOBAL_VRF, ROOT_NETNS, Adapter, Device, IPv4Address, IPv6Address
...
class AddressPlacement:
    ...
    #: :data:`~netviz.models.GLOBAL_VRF` for the global one.
    vrf: str = GLOBAL_VRF
    #: The network namespace the interface is in (§23.1), or
    #: :data:`~netviz.models.ROOT_NETNS` for the machine's initial one. Not
...
class Subnet:
    ...
    #: :data:`~netviz.models.GLOBAL_VRF` for the global one. Two subnets may

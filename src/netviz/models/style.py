...
an SVG attribute, and all three are text formats that netviz *generates*. A
...
#: not a colour, a shape netviz cannot draw, a font size nobody can read.
...
def _colour(value: Any) -> Any:
    ...
    wrote and what ``netviz fmt`` should leave in the file. Resolution to
...
#: the one the editor's inspector lists them in and the one ``netviz fmt``

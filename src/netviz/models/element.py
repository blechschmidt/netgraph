...
#: The thirteen *element* kinds defined by ``netviz.dev/v1alpha1`` (§3). Each
...
#: versioned on its own. See :mod:`netviz.models.layout`.
...
#: obvious place for it, is not an error. See :mod:`netviz.models.theme`.
...
#: ``netviz test``. See :mod:`netviz.models.testsuite`.
...
class ElementBase(NetvizModel):
    """Envelope of a netviz document.

    Subclasses narrow :attr:`kind` to a literal, which makes the union of all
    element models a discriminated union (see :mod:`netviz.models.document`).
    """
    ...
    def name(self) -> str:
        ...
        see :func:`netviz.loader.qualify`.
        """

...
* :mod:`netviz.schema` hangs them off the JSON Schema as ``description``, so
...
from netviz.models.adapter import AdapterSpec, UpstreamPort
...
from netviz.models.cable import CableSpec, InterfaceRef
from netviz.models.device import BridgeConfig, DeviceSpec, Forwarding, VlanDefinition
from netviz.models.element import ElementBase
from netviz.models.firewall import FirewallConfig, FirewallRule, NatRule, Zone
from netviz.models.identity import GroupSpec, UserSpec
...
from netviz.models.metadata import Location, Metadata
from netviz.models.netns import NetnsDefinition
from netviz.models.patchpanel import PatchPanelSpec
from netviz.models.pdu import PduSpec
from netviz.models.power import PoeConfig, PowerConfig, PowerDraw, PowerInput
...
from netviz.models.testsuite import Assertion, TestSuiteSpec
from netviz.models.tunnel import TunnelSpec
...
DOCUMENTED_MODELS: Final[tuple[type[NetvizModel], ...]] = (
    ElementBase,
    Metadata,
    Location,
    DeviceSpec,
...
KIND_NOTES: Final[dict[str, str]] = {
    ...
    "element: it carries no network facts and is never drawn as a node. See `netviz layout`.",
    "testsuite": "Named assertions about the network the other documents describe, graded by "
    "`netviz test`. Not an element: it declares no device and is never drawn.",
...
FIELD_DOCS: Final[dict[tuple[str, str], Doc]] = {
    ...
    ("Metadata", "labels"): Doc(
        ...
        "`netviz.dev/` prefix is reserved for the tool."
    ),
    ("Metadata", "annotations"): Doc(
        "Per-element input to the tooling, not selectable. `netviz/ignore` suppresses "
    ...
    ("StaticRoute", "prefix"): Doc(
        ...
        "destination with them set is a typo or a host route, and netviz will not guess which.",
    ...
    ("StaticRoute", "metric"): Doc(
        "Administrative distance or cost, as this device counts it. Documentation only: netviz "
    ...
    ("VlanConfig", "mode"): Doc(
        "Access or trunk. 802.1Q has neither concept; netviz expands the mode into a PVID, an "
    ...
    ("UserSpec", "email"): Doc(
        ...
        "directory without netviz having to model the directory."
    ),
    ("UserSpec", "uid"): Doc(
    ...
    ("Assertion", "from_"): Doc(
        ...
        "matching several of them. The spellings `netviz path` accepts."
    ),
    ...
    ("Assertion", "select"): Doc(
        "Which elements the claim is about, in `netviz render`'s filter vocabulary: "
...
def check_coverage() -> None:
    ...
    if problems:
        raise RuntimeError("netviz.models.fielddocs: " + "; ".join(problems))

...
``netviz fmt`` may rewrite a whole file, because rewriting the whole file *is*
...
byte for byte. A file written the way ``netviz fmt`` writes it, and a file
...
from netviz.edit.errors import RoundTripError
from netviz.fmt.canonical import INDENT, SEQUENCE_INDENT, SEQUENCE_OFFSET, format_stream
...
#: introduced and how ``netviz fmt`` writes them.
...
#: form ``netviz fmt`` writes; the second is what a sequence written flush
...
def _yaml(style: IndentStyle) -> YAML:
    ...
    Built per call for the reason :func:`netviz.fmt.canonical._yaml` gives:
    ...
    def _parse(self) -> None:
        if self.inline:
            raise RoundTripError(
                ...
                "cannot be edited without moving its first key; run 'netviz fmt' on the "
                "file first"
            )
...
def _is_canonical(text: str) -> bool:
    """Is ``text`` already what ``netviz fmt`` would write?"""
    try:
...
def _style_for(text: str) -> tuple[IndentStyle, Any, bool]:
    ...
    Raises:
        RoundTripError: The round-trip parser cannot read the document. Not the
            ...
            reaches ``float("-.")``. netviz's own loader reads that scalar as
            ...
            :func:`netviz.fmt.canonical.format_stream`.
    """
    first: Any = None
    for index, style in enumerate(STYLES):
        try:
            ...
            raise RoundTripError(
                ...
                f"both agree, and 'netviz fmt' reports the same thing."
            ) from exc
        if index == 0:
            first = data

...
a choice its author made: netviz keeps it. A short name stays short if a short
...
from netviz.edit.paths import FieldPath, format_field_path, get_field
from netviz.loader.inventory import namespace_of, qualify, short_name
from netviz.models import Adapter, Cable, Element, Group, Tunnel
...
def references_of(fqn: str, element: Element) -> Iterator[Reference]:
    ...
    so each endpoint's :attr:`~netviz.models.InterfaceRef.document_index` is
...
def dependents_of(
    ...
    Resolution is the loader's (:meth:`~netviz.loader.Inventory.lookup`): a
...
class NameIndex:
    ...
    :class:`~netviz.loader.Inventory` answers the same question, but only
    ...
    The algorithm is :meth:`~netviz.loader.Inventory.lookup`'s, deliberately:
    a reference netviz will resolve differently from the way this predicts is

...
:class:`~netviz.edit.operations.CopyElement` is one element. Everything larger
...
in the applier (:func:`netviz.edit.apply._copy`) and the *arithmetic* here,
which is the same split :mod:`netviz.edit.arrange` makes.
...
from netviz.edit.errors import EditError, OperationError
from netviz.edit.operations import CopyElement, CreateElement, Operation, SetGeometry
from netviz.edit.references import NameIndex, reference_text, references_of, rewrite_reference
from netviz.errors import SchemaError
from netviz.layout.document import inline_entry
from netviz.layout.geometry import DEFAULT_GRID, Placement
...
from netviz.models import API_VERSION, Cable, Element, Tunnel, parse_document
from netviz.models.scalars import ELEMENT_NAME_PATTERN
...
#: sniffing the shape: a payload from a newer netviz should be refused with a
#: sentence, not silently half-understood.
CLIPBOARD_FORMAT: Final = "netviz.dev/clipboard/v1"

#: How many elements one copy may produce. A guard rather than a policy — the
#: same one :mod:`netviz.edit.arrange` sets on an arrangement — because a
...
#: inventory both having it be a defect netviz reports". Everything else —
...
def _members_of(inventory: Inventory, namespace: str) -> tuple[str, ...] | None:
    ...
    :attr:`~netviz.loader.Inventory.namespaces`, which lists the namespaces
...
def _below(base: str, rest: str) -> str:
    ...
    Not :func:`~netviz.loader.inventory.qualify`, which joins a namespace to a
...
def _endpoints(source: _Source, *, index: NameIndex) -> tuple[str, ...]:
    ...
    Resolved the loader's way (:class:`~netviz.edit.references.NameIndex`), so
...
def _geometry_operations(
    ...
    *,
    ...
    because :class:`~netviz.edit.operations.SetGeometry` replaces a section
...
def clipboard_payload(
    ...
    another netviz window rebuilds exactly these elements, links included.
...
def paste_plan(
    ...
    *,
    ...
    Raises:
        EditError: The payload is not a netviz clipboard, or holds nothing.
    """
    documents = _payload_documents(payload)
    if not documents:
        raise EditError("the clipboard holds no netviz documents")
    ...
    for entry in documents:
        ...
        try:
            ...
            raise EditError(
                f"the clipboard holds a document netviz cannot read: {problems}"
            ) from exc
...
def _payload_documents(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    Raises:
        EditError: The payload is not a netviz clipboard of a version this
            netviz reads.
    """
    if not isinstance(payload, Mapping):  # pragma: no cover - typed at the boundary
        raise EditError("the clipboard does not hold a netviz fragment")
    fmt = payload.get("format")
    if fmt != CLIPBOARD_FORMAT:
        raise EditError(
            f"the clipboard holds {fmt!r}, which is not a netviz fragment ({CLIPBOARD_FORMAT})"
        )
    ...
    *,
    ...
    for reference in references_of(address, element):
        for old, new in mapping.items():
            ...
            # for a payload netviz wrote. It can for one somebody edited by

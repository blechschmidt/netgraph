...
``netviz edit`` has to map each one onto an exit status. A single
...
the loader's own :class:`~netviz.loader.LoadError` and the validator's
:class:`~netviz.validate.Finding`, which is what :class:`ValidationRefused`
carries.
"""
...
class ConflictError(EditError):
    ...
    a ``git checkout`` or a second netviz process landing in between is
...
class RoundTripError(EditError):
    ...
    the first key onto its own line. ``netviz fmt`` fixes it; until then the

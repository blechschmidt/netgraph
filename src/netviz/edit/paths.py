...
(:func:`netviz.errors.format_path` prints the same shape) and that it is
...
entries (:class:`~netviz.edit.operations.AddInterface`) say so in their name.
"""
...
def get_field(document: Any, path: Sequence[str | int]) -> Any:
    ...
    :class:`~netviz.edit.operations.SetField` decidable.
    """
    node = document
    for step in path:
        if isinstance(step, int):

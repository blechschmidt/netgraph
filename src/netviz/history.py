...
between ``git`` and a :class:`~netviz.plan.Plan`, and nothing above it.
...
:func:`Timeline.frame`
    ...
    between them, which is what ``netviz diff`` draws and what
    ``netviz log`` summarises.
...
message, because a repository that grew its ``netviz/`` directory at some
...
* :class:`Timeline` remembers the loaded :class:`~netviz.loader.Inventory` for
...
* :class:`FrameCache` remembers whatever a caller derived from a *pair* of
  trees — the rendered SVG, in ``netviz web``'s case — so scrubbing back over
...
from netviz.config import DEFAULT_MAX_REVISIONS
from netviz.errors import LoaderError, NetvizError
from netviz.loader import DocumentCache, Inventory, load_tree
...
from netviz.plan.model import Action, Change, StateRef
...
class HistoryError(NetvizError):
...
class Commit:
    ...
    def to_dict(self) -> dict[str, Any]:
        """The JSON form ``netviz log`` prints and the editor fetches."""
        return {
            "hash": self.hash,
            "abbrev": self.abbrev,
...
class Frame:
    ...
    def to_dict(self) -> dict[str, Any]:
        """The JSON form, for ``netviz log -F json`` and the editor."""
...
#: of ``netviz log`` wants "3 devices", not "3 device elements".
...
def summarise(plan: Plan) -> str:
    ...
    in which direction. Deliberately not :func:`~netviz.plan.summary_line`,
...
class FrameCache(Generic[_T]):
    ...
    asking: ``netviz log`` wants the changeset, the editor wants a rendered
...
class Timeline:
        ...
        if limit is None or limit > self.max_revisions:
            ...
            if total > self.max_revisions:
                raise HistoryError(
                    ...
                    f"raise 'max-revisions' in the [history] table of netviz.toml"
                )
    ...
    def _revision_argument(self, rev: str) -> str:
        ...
        here. ``netviz web`` takes revisions from a query string, and
        ``git log --output=<file>`` is a file this server would otherwise
        write on request; see :func:`~netviz.plan.sources.check_revision`.
        """
        try:

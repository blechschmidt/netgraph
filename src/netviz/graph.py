...
:mod:`netviz.render.graph` already resolves an inventory into a frozen
:class:`~netviz.render.graph.Graph` — the thing the renderers consume. That
...
The two are not alternatives, they are one pipeline::
    ...
    resolved = build_graph(inventory)        # netviz.render.graph
    g        = to_networkx(resolved)         # here
    g        = filter_graph(g, vlans=[10])
    print(stats(g))

Resolution happens exactly once, in :func:`~netviz.render.graph.build_graph`,
...
(:attr:`~netviz.render.graph.Edge.id`: the cable's fully-qualified name, or
...
:class:`~netviz.render.graph.PortView` records), ``vlans`` and ``node_type``.
...
:class:`~netviz.render.graph.TunnelView`. ``source``
...
#: and :data:`~netviz.render.graph.SUBNET_KIND` are all taken, so this cannot
...
def to_networkx(source: Inventory | ResolvedGraph, *, layer: Layer | None = None) -> nx.MultiGraph:
    ...
    Args:
        source: A tree loaded by :func:`~netviz.loader.load_tree`, or the
            :class:`~netviz.render.graph.Graph` that
            :func:`~netviz.render.graph.build_graph` produced from one.
    ...
    Returns:
        ...
        :attr:`~netviz.render.graph.Graph.dangling`).
...
def broadcast_domains(graph: nx.MultiGraph) -> tuple[BroadcastDomain, ...]:
    ...
    the :mod:`netviz.render.graph` module docstring). Candidates are then
    ...
    * a **cable** carries VLAN 10 when its ``vlans`` contains 10, i.e. when both
      ends agree on it (:func:`~netviz.render.graph._link_vlans`);
...
def _vlan_links(
    ...
    :func:`~netviz.render.graph._tunnel_vlans`), so it prunes itself out here.
    """
...
def stats(graph: nx.MultiGraph) -> GraphStats:
    ...
    :func:`~netviz.subnets.subnets_of` — ``127.0.0.1`` is not a subnet of this
    network.
    """
    vlans: set[int] = set()

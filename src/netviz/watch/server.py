...
The server exists so that ``netviz watch --serve`` can show a diagram that
...
The first and third are promises :mod:`netviz.httpserve` makes on behalf of
every local server netviz starts; what is here is the routing on top of them.
...
from netviz.watch.pipeline import LiveRender, Snapshot
...
#: Default port of ``netviz watch --serve``. The bind address comes from
#: :data:`netviz.httpserve.DEFAULT_HOST`, which is loopback.
...
<title>netviz — {html.escape(title)}</title>

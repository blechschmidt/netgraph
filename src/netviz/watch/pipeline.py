...
The cycle is the unit of work ``netviz watch`` repeats. It is deliberately
...
from netviz.errors import NetvizError, RenderError, format_path
from netviz.fsio import write_bytes_atomically
from netviz.loader import DocumentCache, Inventory, LoadError, load_tree
...
class Problem:
    ...
    #: as plain pairs rather than as :class:`netviz.fixes.Fix` objects so that
...
class RenderRequest:
    ...
    The per-inventory ``netviz.toml`` is *not* held here: it is re-read on
    ...
    #: see :func:`netviz.render.render_layers`.
    ...
    #: see :mod:`netviz.render.aggregate`.
    ...
    def config_root(self) -> Path:
        """Directory ``netviz.toml`` is looked for in."""
...
def run_cycle(request: RenderRequest) -> CycleResult:
    ...
    :class:`~netviz.errors.NetvizError` escaping this function would kill a
    ...
    except (NetvizError, UnknownElementError, OSError) as exc:
        return CycleResult(
...
def flatten_problems(
    ...
    *,
    ...
    Shared with :mod:`netviz.web.preview`: two front ends listing the same
...
def write_atomically(path: Path, payload: bytes) -> None:
    ...
    viewer, another netviz — may be reading it. Writing to a sibling temporary
    ...
    is refused rather than allowed, which :func:`netviz.fsio.replace_atomically`

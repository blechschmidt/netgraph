...
``netviz render`` takes twenty-odd flags that shape the picture rather than
...
selected with ``netviz render --profile poster``.
...
tuple for a repeatable option, an :class:`~netviz.render.icons.IconTheme` for
...
class _Where:
    ...
    #: ``"/path/to/netviz.toml: "``, or ``""`` when parsing a bare mapping.
    prefix: str = ""
...
def _icons(value: Any, where: _Where) -> Any:
    ...
    runs ``netviz`` from a parent folder must get the same icons.
    """
    from netviz.errors import RenderError  # local: keeps this module import-cheap
    from netviz.render.icons import BUNDLED_THEMES, NO_ICONS, icon_theme
...
def _theme(value: Any, where: _Where) -> Any:
    ...
    who runs ``netviz`` from a parent folder must get the same colours.
    """
    from netviz.errors import RenderError  # local: keeps this module import-cheap
    from netviz.render.theme import BUNDLED_THEMES, NO_THEME, load_theme
...
def _link_template(value: Any, where: _Where) -> Any:
    ...
    from netviz.render.links import LinkTemplate
...
def _formats() -> Sequence[str]:
    from netviz.render import FORMATS
...
def _node_kinds() -> Sequence[str]:
    from netviz.render import NODE_KINDS

    return NODE_KINDS


def _rankdirs() -> Sequence[str]:
    from netviz.render import RANKDIRS

    return RANKDIRS


def _routing_styles() -> Sequence[str]:
    from netviz.models.layout import ROUTING_STYLES
...
class Setting:
    ...
    parse: _Parse
    #: One line, for ``netviz config show`` and the documentation test.
    summary: str
...
#: ``netviz config show`` prints them: what to draw, then what to summarise,
...
class Origin(str, Enum):
    """Where a resolved value came from. Printed by ``netviz config show``."""
...
def describe_value(value: Any) -> str:
    """A settings value as one readable cell of ``netviz config show``."""
    if value is None:
        return "(unset)"
    if isinstance(value, bool):

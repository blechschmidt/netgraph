...
from the *inventory* and :func:`~netviz.graph.broadcast_domains` re-derives two
...
:func:`single_points` hands each layer to :func:`netviz.connectivity.analyse`,
...
:func:`~netviz.impact.graphs.unpowered` walks the feed graph separately and the
...
from netviz.connectivity import LINK, NODE, Cut, Graph, analyse, components, reachable
from netviz.expectations import Declaration, Expectation, declarations
from netviz.impact.graphs import LAYERS, POWER, LayerView, feed_sources, unpowered, views
...
from netviz.models import Adapter, Device, Element, Interface
from netviz.power import PowerPlan, power_plan
from netviz.trace import TracedPath, TraceError, TraceResult, trace
...
#: :attr:`~netviz.impact.model.ImpactReport.anchor_source` when the anchors
...
#: validation rules — ``netviz validate`` gates on them too — and this is the
...
def prune(inventory: Inventory, removed: Iterable[str]) -> Inventory:
    ...
    :func:`~netviz.render.graph.build_graph` already drops from the drawing —
...
def simulate(
    ...
    *,
    ...
    Args:
        ...
        wanted_layers: Which of :data:`~netviz.impact.graphs.LAYERS` to report.
...
def _check_paths(
    ...
    is what keeps ``netviz impact`` and ``netviz path`` from disagreeing
...
def single_points(
    ...
    *,
    ...
    Args:
        ...
        layer_views: The views to sweep, from :func:`~netviz.impact.graphs.views`.
...
def _spof(view: LayerView, cut: Cut) -> Spof:
    """One :class:`~netviz.connectivity.Cut`, told what it is."""
    if cut.kind == NODE:
        return Spof(
            layer=view.layer,
            kind=NODE,
...
def check_expectations(inventory: Inventory) -> tuple[Finding, ...]:
    ...
    Delegated to :func:`netviz.validate.validate` rather than reimplemented,
    exactly the way :mod:`netviz.ipam` delegates its own subset: a rule that
    two commands can disagree about is a rule nobody can act on, and ``netviz

...
``netviz import`` reads a live network and writes YAML. This inverts it: the
...
:mod:`netviz.importer.lldp`, :mod:`netviz.importer.iproute`,
:mod:`netviz.importer.csvlinks` and the seven configuration readers in
:mod:`netviz.importer.config` produce the same
:class:`~netviz.importer.draft.Draft` here as they do there — so a dialect
netviz can import is a dialect netviz can check against, with no second
parser to keep in step.

Those six close the loop the other way as well. :mod:`netviz.export.config`
writes the configuration a device would run, in the same seven dialects, so
``netviz export netplan`` and ``netviz drift --from netplan`` are two ends of
...
:mod:`netviz.drift.coverage`; everything absent that it does not vouch for
becomes an :class:`~netviz.drift.model.Unobserved`, which is reported in its
...
``vendor``, ``model``, ``serial``, ``location``
    No dialect netviz reads reports any of them as a field. LLDP's ``descr``
...
from netviz.drift.coverage import CAPABILITIES, Coverage, coverage_of
from netviz.drift.model import Change, Direction, DriftReport, Unobserved
from netviz.importer.draft import Draft, DraftCable, DraftDevice, DraftInterface, Endpoint
from netviz.loader.inventory import Inventory, namespace_of, short_name
from netviz.models import Adapter, Cable, Device, ElementBase, format_bitrate
from netviz.models.interface import Interface, VlanMode
...
#: ``ip`` cannot tell a wireless NIC from a wired one, so neither can netviz.
...
#: Kinds that are netviz's own abstraction rather than a thing on the wire. No
#: capture format has a word for any of them, so an observed ``kind`` is never
#: evidence against one. Mirrors :data:`netviz.plan.live._UNOBSERVABLE_KINDS`.
...
def compare(
    ...
    *,
    ...
    Returns:
        A :class:`~netviz.drift.model.DriftReport` whose two lists are sorted
        and whose :attr:`~netviz.drift.model.DriftReport.drifted` flag is true
...
class _Comparison:
    ...
    def _resolve_devices(self) -> None:
        ...
        Resolution goes through :meth:`~netviz.loader.inventory.Inventory.lookup`,
    ...
    def _compare_device_kind(self, fqn: str, declared: ElementBase, observed: DraftDevice) -> None:
        ...
        anything about (:meth:`~netviz.importer.draft.Draft.device`), so it is
        never evidence. An adapter, a patch panel, a PDU, a user and a group are
        netviz's own abstractions, which no capture format has a word for.
        """
    ...
    def _compare_vlans(
        ...
        :func:`~netviz.importer.iproute._infer_parent_trunks` writes the
        ...
        if absent:
            self.blind.append(
                Unobserved(
                    ...
                    reason=(
                        "no dialect netviz reads prints the VLAN set of a trunk; what was "
    ...
    def _compare_links(self) -> None:
        ...
        :attr:`~netviz.importer.draft.DraftCable.key` already is, once the
    ...
    def _link_blind_reason(self, key: tuple[Endpoint, Endpoint]) -> str:
        ...
        if not observing:
            return (
                ...
                f"nothing else netviz reads does — a configuration says what a box does "

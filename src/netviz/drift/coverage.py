...
The seven configuration dialects are the ones :mod:`netviz.export.config` writes,
read back (:mod:`netviz.importer.config`), and they split into two groups for
...
``ip -j addr show`` answers, and both are worth asking; ``netviz drift`` reports
...
carries; :mod:`netviz.importer.iproute` derives the minimum set implied by the
...
from netviz.importer.draft import Draft, DraftDevice
...
#: :func:`netviz.importer.dialect_of` returns, so ``auto`` never appears.
...
#: skipped by :mod:`netviz.importer.iproute` by design — it terminates no cable
...
class Coverage:
    ...
    def observes_trunk_vlans(self, device: str) -> bool:
        """Always false: no dialect netviz reads prints a port's VLAN set.
...
def coverage_of(draft: Draft) -> Coverage:
    ...
    :attr:`~netviz.importer.draft.DraftDevice.sources` names the inputs a
    device was seen in and :attr:`~netviz.importer.draft.Draft.dialects` maps

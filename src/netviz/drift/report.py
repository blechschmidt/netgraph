"""Rendering a :class:`~netviz.drift.model.DriftReport` for a human or a machine.
...
The document skeleton for the latter two lives in :mod:`netviz.diagnostics`, so
...
def _root_text(root: Path) -> str:
    ...
    :func:`~netviz.fsio.display_path` is where that rule and the forward
...
def _cases(report: DriftReport) -> Iterator[JUnitCase]:
    ...
    :attr:`~netviz.drift.model.DriftReport.elements` at all; it still needs a
    ...
    for name in sorted({*groups, *passing}):
        ...
        if group is None:
            yield JUnitCase(classname="netviz.drift.element", name=name)
            continue
        classname = f"netviz.drift.{group.kind or 'element'}"
        if group.changes:
            detail = "\n".join(

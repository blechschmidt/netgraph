...
it: :mod:`netviz.validate`, which grades the expectations as ``E047``, ``E048``
and ``W141`` so ``netviz validate`` gates a pull request on them, and
:mod:`netviz.impact`, which reports them alongside the failure simulation that
...
#: spellings are accepted for the same reason ``netviz/ignore`` accepts both:
#: the ``netviz.dev/`` prefix matches ``apiVersion`` and the bare one is what
#: people type. A document may set either; setting both merges them.
EXPECTATION_ANNOTATIONS: Final[tuple[str, ...]] = ("netviz/redundancy", "netviz.dev/redundancy")

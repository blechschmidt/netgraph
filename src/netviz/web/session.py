...
``netviz web`` used to hold a string. That is enough for a scratchpad and it
...
whole file the user retyped — becomes a list of :mod:`netviz.edit`
operations, applied through an :class:`~netviz.edit.EditSession`. This module
...
* **Disk to browser.** :class:`TreeWatcher` runs the same ``watchfiles`` watch
  ``netviz watch`` runs, over the same :class:`~netviz.watch.loop.InventoryFilter`,
  and bumps :attr:`EditingSession.revision`. Every bump — from the watcher or
  from a write of this session's own — is announced on
  :attr:`EditingSession.events` (:mod:`netviz.web.events`), naming the files
...
client is issued an id (:mod:`netviz.web.presence`), and the state payload
...
from netviz.config import DEFAULT_MAX_REVISIONS, Config, ValidationConfig, load_config
...
from netviz.edit.batch import describe as describe_batch
from netviz.edit.cascade import describe as describe_cascade
from netviz.edit.cascade import plan_cascade
from netviz.edit.tree import digest_of
from netviz.errors import NetvizError
from netviz.fixes import Fix, apply_fix, fixes_for, offers_for
from netviz.history import FrameCache, HistoryError, Timeline
from netviz.impact import LAYERS as IMPACT_LAYERS
from netviz.impact import ImpactError, ImpactReport, simulate
...
from netviz.plan import PlanSourceError
from netviz.plan import diff as diff_states
from netviz.plan.sources import git_ref
from netviz.query import QueryError
from netviz.query import select as run_query
from netviz.render import IconTheme
from netviz.render.graph import Layer, build_graph
from netviz.render.routes import RouteCache
from netviz.validate import Finding, validate
from netviz.watch.pipeline import Problem, Status, flatten_problems
from netviz.web.events import EVENT_NAMES, EVENTS_PATH, HEARTBEAT_SECONDS, EventBus
from netviz.web.presence import Client, Presence
from netviz.web.preview import (
    Preview,
    ViewOptions,
    clip_problems,
    render_diff,
...
class SessionError(NetvizError):
...
class Gesture:
    ...
    addresses: tuple[str, ...]
    #: The ``netviz edit …`` lines that would replay it.
    commands: tuple[str, ...]
...
def _diagnostics_payload(problems: Sequence[Problem]) -> dict[str, Any]:
    ...
    same way and counted the same way; see :data:`~netviz.web.preview.MAX_PROBLEMS`.
    """
...
class EditingSession:
    ...
    #: :class:`~netviz.render.routes.RouteCache`.
    ...
    def settings(self) -> ValidationConfig:
        """How this tree grades findings: its own ``netviz.toml``."""
    ...
    def graph(
        ...
        Args:
            ...
            known: The :func:`~netviz.web.preview.graph_digest` the caller
    ...
    def apply(
        ...
        *,
        ...
        """Apply a batch of :mod:`netviz.edit` operations, atomically.
        ...
        Raises:
            ReadOnly: This session does not write.
            SessionError: An operation is not one netviz has.
    ...
    def arrange(
        ...
        *,
        ...
        The tidying itself is :func:`~netviz.edit.arrange.arrange_operations`,
        ...
        Args:
            command: One of :data:`~netviz.edit.arrange.ARRANGEMENTS`.
        ...
        Raises:
            ReadOnly: This session does not write.
            SessionError: ``command`` is not one netviz has.
            Conflict: ``revision`` is not the current one.
    ...
    def reparent(
        ...
        *,
        ...
        ``netviz edit move`` calls a move are the same thing, and this is the
        one place that says so. :func:`~netviz.edit.containers.move_plan`
    ...
    # :mod:`netviz.edit.clipboard`, so the browser, ``netviz edit copy`` and
    ...
    def duplicate(
        ...
        *,
        ...
        ``Ctrl-D``. The plan is
        :func:`~netviz.edit.clipboard.copy_plan`'s, so what the browser writes
        and what ``netviz edit duplicate`` writes are the same operations.
    ...
    def paste(
        ...
        *,
        ...
        Raises:
            ReadOnly: This session does not write.
            Conflict: ``revision`` is not the current one.
            EditError: The payload is not a netviz fragment, or cannot be
    ...
    def fix(
        ...
        *,
        ...
        The repair goes through the same gate ``netviz validate --fix`` uses:
    ...
    def _commit(
        ...
        *,
        ...
        :class:`~netviz.edit.EditSession` does all of it, under the same
        ...
        :class:`~netviz.edit.Batch` puts the tree back before re-raising. That
    ...
    def changes(self) -> dict[str, Any]:
        ...
        ``commands`` is the whole session as a ``netviz edit`` script, in the
    ...
    def impact(self, failed: Sequence[str], *, layer: str = "l1") -> dict[str, Any]:
        ...
        Args:
            failed: Elements to remove, in the spellings
                :func:`~netviz.impact.resolve_element` takes. Usually one: the
                shape the pointer was on.
            layer: Which view to measure in, one of
                :data:`~netviz.impact.LAYERS`.
        ...
        Returns:
            ...
            needs and nothing it does not. The full analysis is ``netviz
    ...
    def cascade(self, addresses: Sequence[str]) -> dict[str, Any]:
        ...
        Returns:
            :meth:`~netviz.edit.cascade.CascadePlan.to_dict`, plus the
    ...
    def search(self, expression: str, *, layer: str = "l1") -> dict[str, Any]:
        ...
        Args:
            expression: The query, in the language of :mod:`netviz.query`.
    ...
    def history(self, *, limit: int | None = None) -> dict[str, Any]:
        ...
        time. What is refused is an explicit range: ``netviz log --from`` and
    ...
    def invalidate(
        ...
        anything outside the tree (``netviz.toml``, a file the loader skips)
    ...
    def stream_ended(self, client_id: str) -> None:
        ...
        streaming; :data:`~netviz.web.presence.PRESENCE_TTL` removes it if the
...
def _decode(payload: Sequence[Mapping[str, Any]]) -> tuple[Operation, ...]:
    ...
    Raises:
        SessionError: The body is not a list of operation objects, or one of
            them is not an operation netviz has.
    """
...
class TreeWatcher:
    ...
    The same watch ``netviz watch`` runs, over the same filter, for the same
    reason: an edit made in ``$EDITOR``, by ``git checkout`` or by a second
    netviz process is a change to the source of truth and the open browser has
    ...
    def start(self) -> TreeWatcher:
        if self._thread is None:
            self._thread = threading.Thread(target=self._run, name="netviz-web-watch", daemon=True)
            self._thread.start()
        return self

...
This is the whole of what ``netviz web`` does per keystroke-burst, and it is
the same pipeline the command line runs, in the same order, with two
differences that follow from being interactive:

* **A rejected inventory is still drawn.** ``netviz render`` refuses one
...
* **The picture comes back as a fragment, not a file.**
  :func:`~netviz.web.svgdoc.prepare` strips everything that could execute or
...
:class:`~netviz.web.session.EditingSession` has already loaded, so an editing
...
The result carries a :class:`~netviz.watch.pipeline.Status`, and a status
...
from netviz.diff import Drawing, draw
from netviz.errors import NetvizError, echo_value
from netviz.layout.geometry import Geometry
from netviz.loader import Inventory, load_stream
from netviz.plan import Plan
...
from netviz.render.aggregate import (
    AGGREGATE_ID_PREFIX,
    AggregateSpec,
    collapse_namespaces,
    collapse_targets,
)
from netviz.render.dot import cluster_keys, to_dot, to_image
from netviz.render.icons import BUNDLED_THEMES, NO_ICONS
from netviz.render.ids import element_ids
from netviz.render.jsonexport import annotations_payload
from netviz.render.routes import RouteCache, anchors_of, default_routing, fans_of, route_plan
from netviz.render.styles import StyleMap
...
from netviz.watch.pipeline import Problem, Status, flatten_problems
from netviz.web.svgdoc import prepare
...
#: the list, and ``netviz validate`` prints every one — but a cap on what is
...
#:
#: The kept ones are the most severe, because :func:`~netviz.watch.pipeline.
...
class RequestError(NetvizError):
...
class ViewOptions:
    ...
    The fields mirror the options ``netviz render`` takes, minus the ones
    ...
    #: ``netviz render --collapse`` folds them
    #: (:func:`~netviz.render.aggregate.collapse_namespaces`). The browser's,
    ...
    kinds: tuple[str, ...] = ()
    #: A selector query (:mod:`netviz.query`) the drawing is narrowed to, or
    ...
    #: :class:`~netviz.render.graph.FilterSpec` rather than the browser
    ...
    def render_options(self) -> RenderOptions:
        ...
        (:func:`netviz.web.svgdoc.prepare` strips any that survive).
        """
...
class Preview:
    ...
    svg: str | None = None
    #: Info-box records keyed by SVG element id; see :mod:`netviz.render.details`.
    ...
    #: ``netviz render -f json`` exports it, or ``None`` when the inventory
    #: stores none. A canvas that lets somebody drag a node needs to know where
    #: netviz thinks that node is, and whether the position is stored or was
    #: invented by the engine; see :mod:`netviz.layout.geometry`.
    geometry: Mapping[str, Any] | None = None
    #: The notes, areas and legends this pass drew (§21), in the form
    #: ``netviz render -f json`` publishes them, or ``None`` when the view
    ...
    #: in the form ``netviz render -f json`` publishes it, provenance
    ...
    #: keyed by node and edge id, exactly as ``netviz diff -f json`` publishes
...
def graph_digest(graph: Graph, options: RenderOptions) -> str:
    ...
    repeats inside :func:`~netviz.render.dot.to_image` when the picture *has*
...
def render_source(source: str, view: ViewOptions | None = None) -> Preview:
    ...
    :attr:`~netviz.watch.pipeline.Status.FAILED` with the installation hint
...
def render_inventory(
    ...
    *,
    ...
    (:mod:`netviz.web.session`) draws its tree without re-parsing it and
    without either face of ``netviz web`` growing its own renderer.

    Args:
        inventory: A loaded tree or stream, errors and all.
        view: Which graph to build and how to draw it.
        settings: Validation settings, normally the tree's own ``netviz.toml``
            with the page's ``strict`` toggle folded in. Defaults to netviz's
        ...
        fixes: The mechanical repairs on offer, keyed as
            :func:`~netviz.watch.pipeline.flatten_problems` keys them. Passed
        ...
        routes: A :class:`~netviz.render.routes.RouteCache` to draw orthogonal
    ...
    try:
        ...
        svg = prepare(payload)
    except (NetvizError, OSError) as exc:
        return Preview(
...
def _containers(
    ...
    *directly* — Graphviz has no reason to draw ``sites`` around three sites
    ...
    boxes is one :func:`~netviz.render.dot.cluster_keys` names, and only such
    ...
    (:func:`netviz.render.dot._frames`). A level in between is drawn round
...
def _styles(graph: Graph, options: RenderOptions, *, diff: bool = False) -> Mapping[str, Any]:
    ...
    :class:`~netviz.render.styles.StyleMap` back out of ``to_image`` — which
...
def render_diff(
    ...
    *,
    ...
    changed — :mod:`netviz.diff` joins the changeset to the two drawings, and
    the same code answers ``netviz diff`` on the command line.
    ...
    Args:
        ...
        plan: The changeset between them, from :func:`netviz.plan.diff`.
        view: Which graph to build and how to draw it.
...
def _geometry(
    ...
    ``netviz render -f json`` — points, ``y`` upwards, a position being the
    ...
    position) *and* the line netviz drew from it. The canvas puts a grab
    ...
    :mod:`netviz.layout.routing`. ``anchors`` is what it clips against, so
...
def _links(
    ...
    :mod:`netviz.layout.avoid` worked out on top of them so the line misses
    ...
    the JavaScript mirror of :mod:`netviz.layout.routing` exact: the canvas
...
def icon_choices(default: IconTheme | None = None) -> tuple[str, ...]:
    ...
    place so the two cannot drift: the themes that ship with netviz, plus
...
def _icon_choice(value: Any, default: IconTheme | None) -> IconTheme | None:
    ...
    if bundled is None:
        ...
        raise RequestError(
            ...
            "A theme of your own is a directory, and is chosen with 'netviz web --icons DIR' "
...
def _namespaces(value: Any) -> tuple[str, ...]:
    ...
    :func:`~netviz.render.aggregate.collapse_targets` also assumes.
    """
...
def _describe(exc: NetvizError | OSError) -> str:
    """A failure in the words the page should show.

    Only two things reach here: a netviz error — Graphviz missing, an SVG
    that would not parse — which already reads as a sentence, and a bare
    ``OSError`` from reading an icon file, which does not.
    """
    if isinstance(exc, OSError) and not isinstance(exc, NetvizError):

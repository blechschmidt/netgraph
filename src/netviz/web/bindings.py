...
``session``
    ``netviz web DIR``: there is a tree of files, not a scratchpad.
``write``
...
BINDINGS: Final[tuple[Binding, ...]] = (
    ...
    #
    # One language for the CLI, the renderer and this page (:mod:`netviz.query`).
    ...
    Binding(
        ...
        detail=(
            ...
            "same narrowing 'netviz render --select' does."
        ),
    ),
    Binding(
        id="search.clear",
    ...
    Binding(
        ...
        keys=("n",),
        detail="Asks for a kind and a name, and writes the document. 'netviz edit create'.",
        where="canvas",
        needs="write",
    ),
    Binding(
        ...
        section="Editing the inventory",
        keys=("c",),
        detail=("Cables the focused element to another, port to port. 'netviz edit connect'."),
        where="canvas",
        needs="write",
    ),
    Binding(
        ...
        detail=(
            ...
            "change. 'netviz edit delete --cascade' / 'disconnect'."
        ),
        where="canvas",
        needs="write",
    ),
    ...
    Binding(
        ...
        detail=(
            ...
            "selected. Paste it into another netviz window, or into a text "
            "editor to read it. 'netviz edit copy'."
        ),
        where="canvas",
        needs="session",
    ),
    ...
    Binding(
        ...
        detail=(
            ...
            "beside it. 'netviz edit duplicate'."
        ),
        where="canvas",
        needs="write",
    ),
    Binding(
        ...
        section="Editing the inventory",
        keys=("F2",),
        detail="Renames it and every reference to it. 'netviz edit rename'.",
        where="canvas",
        needs="write",
    ),
    Binding(
        ...
        detail=(
            ...
            "on the focused one when nothing is selected. 'netviz edit set'."
        ),
        where="canvas",
        needs="write",
    ),
    Binding(
        ...
        section="Editing the inventory",
        keys=(),
        detail="'netviz edit unset', across the whole selection as one change.",
        needs="write",
    ),
    Binding(
        id="element.move",
        ...
        keys=(),
        detail=(
            "Moves the selected documents into a different file, together. 'netviz edit move'."
        ),
        needs="write",
    ),
    Binding(
        ...
        detail=(
            ...
            "folder netviz would read is one holding a document, so an empty one "
    ...
    Binding(
        ...
        section="Editing the inventory",
        keys=(),
        detail="Removes a cable, leaving both devices. 'netviz edit disconnect'.",
        needs="write",
    ),
    # -- Routing a link ----------------------------------------------------
    #
    ...
    Binding(
        ...
        detail=(
            ...
            "view's default back. Honoured by 'netviz render' as well as here."
        ),
        where="canvas",
        needs="write",
    ),
    Binding(
        ...
        keys=("Shift-R",),
        detail=(
            "Writes the bends netviz computed to keep this link clear of the boxes it "
    ...
    Binding(
        ...
        section="Editing the inventory",
        keys=(),
        detail="'netviz edit remove-interface'.",
        needs="write",
    ),
    # -- Arranging the diagram ---------------------------------------------
    #
    ...
    # :mod:`netviz.edit.arrange` — one per layout document that loses an
    ...
    Binding(
        ...
        detail=(
            ...
            "sets in 'netviz.toml' ([editor] grid, 20 points by default)."
        ),
        needs="write",
    ),
    # -- The view ----------------------------------------------------------
    ...
    Binding(
        ...
        detail=(
            ...
            "element. The same folding 'netviz render --collapse' does. A view, not "
    ...
    Binding(
        ...
        detail=(
            ...
            "theme is the one 'netviz web --icons' named, or the set that ships "
            "with netviz; which of them exist is the command line's to say, "
    ...
    Binding(
        ...
        section="Files and history",
        keys=(),
        detail="The session as a 'netviz edit' script somebody else can review or run.",
        needs="session",
    ),
    Binding(
        id="timeline.toggle",
...
def payload() -> dict[str, Any]:
    ...
    them, and :data:`netviz.models.KINDS` is where they are declared — a page

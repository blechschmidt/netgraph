...
``netviz web`` opens a page with the stream in a text area on one side and the
...
:mod:`~netviz.web.preview`
    :func:`~netviz.web.preview.render_source`, the ``parse → validate →
...
:mod:`netviz.render.details`
    The info-box records, keyed by the id the drawn element carries. They are
    the JSON export, so what a hover says and what ``netviz render -f json``
    prints cannot drift apart — and they live in the renderer rather than here
    because ``netviz render -f svg`` puts the same records in its tooltips.
:mod:`~netviz.web.svgdoc`
...
:mod:`~netviz.web.server`
    :class:`~netviz.web.server.WebServer`, the routes over all of it,
    loopback-bound like every local server netviz starts.
:mod:`~netviz.web.events`
    :class:`~netviz.web.events.EventBus`, the push channel an editing session
...
:func:`~netviz.web.preview.render_source` takes a string and returns the
diagram, the records and the problems, and no HTTP is involved.
"""

from __future__ import annotations

from netviz.render.details import DETAIL_OPTIONS, build_details
from netviz.web.bindings import BINDINGS, SECTIONS, Binding
from netviz.web.events import EVENT_NAMES, EVENTS_PATH, Event, EventBus

...
real batch through :mod:`netviz.edit`, because a tour that only *described*
...
:class:`~netviz.web.session.EditingSession` over the copy; every route in
:mod:`netviz.web.server` selects it with ``?scratch=<token>`` and answers from
...
way :mod:`netviz.loader.tree` reads it — YAML suffixes, no component starting
with ``.`` or ``_`` — plus ``netviz.toml``, because the copy should render
...
mistyped ``netviz web /`` from filling a disk, and the refusal names what it
...
class Tours:
    ...
    def open(self, session: EditingSession, *, icons: IconTheme | None = None) -> Scratch:
        ...
        root = Path(mkdtemp(prefix="netviz-tour-"))
        try:
...
def copy_inventory(source: Path, destination: Path) -> int:
    ...
    path component starting with ``.`` or ``_``, plus ``netviz.toml`` at the

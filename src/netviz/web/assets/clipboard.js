 ...
 * See netviz/edit/clipboard.py, which decides all of that; nothing here knows
 ...
 *                          netviz window, another inventory, or a text editor
 ...
 *                          read or does not hold a netviz fragment.
 *
...
var netvizClipboard = (function () {
   ...
   * HTTP to something other than localhost, which is a `netviz web --host`
  ...
  /** JSON that is a netviz fragment, or null for anything else.
   *
  ...
  function anchor(at) {
    if (!at || !window.netvizNotes || !netvizNotes.graphPoint) { return null; }
    return netvizNotes.graphPoint(at.x, at.y);
  }
   ...
   */
  function targets() {
    return netvizSelect.targets().map(function (address) {
  ...
  function namespaceAt(at) {
    if (!at || !window.netvizContainers) { return ""; }
    return netvizContainers.namespaceAt(at);
  }
  ...
  function paste(at, namespace) {
    read().then(function (fragment) {
      if (!fragment) {
        host.refuse("the clipboard holds no netviz elements");
        return;
      }

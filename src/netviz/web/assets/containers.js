 ...
 * `netviz edit move`, and the file moves with it.
 *
 ...
 * Graphviz boxes is one `netviz.render.dot.cluster_keys` names, and only such
 ...
 * `collapse=<namespace>`, which is `netviz render --collapse` and
 * `netviz.render.aggregate.collapse_namespaces`. It writes nothing: how much
 ...
 * `netvizCull.boxOf` answers in, so nothing is converted to draw. A *write*
 * is in netviz's -- points, `y` upwards -- so the offset between the two is
 * measured off the nodes exactly as links.js and notes.js measure it.
 */
window.netvizContainers = (function () {
  "use strict";
   ...
   * `netviz render` writes to a file — where there is no header and the
  ...
  function hullOf(entry) {
    ...
    (entry.members || []).forEach(function (member) {
      ...
      var box = id && window.netvizCull.boxOf(id);
      if (!box) { return; }
  ...
  function beginDrop(record, event, entry) {
    ...
    if (entry) {
      ...
      addresses = window.netvizSelect.has(address)
        ? window.netvizSelect.addresses()
        : [address];
    }
    drag = {
      mode: "drop",
  ...
  /** Post the drop as `netviz edit move`, or say why it is not one.
   *
   ...
   * merge (netviz.edit.apply._merge_section): a `groups` holding one entry

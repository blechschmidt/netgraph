 ...
 * palette (see netviz/render/styles.py). A user looking at a navy switch needs
 ...
 * (netviz/web/preview.py): resolved values keyed by address, each with its
...
window.netvizStyle = (function () {
   ...
   */
  function subjects() {
    var chosen = window.netvizSelect.targets();
    var found = [];
  ...
  function paint() {
    ...
    var picked = window.netvizSelect.targets();
    var chosen = subjects();
  ...
  function defineCommands(K) {
    ...
    K.define("style.inspect", {
      run: function () {
        show(true);
        if (!window.netvizSelect.targets().length) {

 ...
 * is byte-for-byte what `netviz render` draws.
 *
 ...
 * document through the same comment-preserving path `netviz layout` uses, and
 ...
 * cursor. It is a faithful port of `netviz/layout/routing.py` -- same
 ...
 *
 * Coordinates. netviz's are points with `y` upwards (Graphviz's own); the
 ...
 */
window.netvizLinks = (function () {
  "use strict";

  /* ---------------------------------------------------- routing, mirrored */

  /** @see netviz.layout.routing.DEFAULT_NODE_SIZE */
  var DEFAULT_NODE_SIZE = [54, 36];
  /** @see netviz.layout.routing.CLEARANCE */
  var CLEARANCE = 1;
  /** @see netviz.layout.routing.LOOP_REACH */
  var LOOP_REACH = 40;
  /** @see netviz.layout.routing.FAN_GAP */
  var FAN_GAP = 14;
  /** @see netviz.layout.routing.EPSILON */
  var EPSILON = 1e-9;
  /** @see netviz.layout.routing.TOUCH */
  var TOUCH = 1e-3;
  ...
  /** Turn the bends netviz computed into bends this document declares.
   *
   * `routed` is what netviz.layout.avoid worked out so the line misses the
      ...
      ctx.refuse(
        frame.geometry && frame.geometry.links
          ...
          : "this diagram is not arranged yet, so a cable has nowhere to bend. " +
            "Run 'netviz layout --write' to place it"
      );
    }
    return false;
  }
  ...
  return {
     ...
     * through netviz.layout.routing and asserts the two agree. */
    routeOf: routeOf
  };
})();

 ...
 * `annotations` beside `geometry` -- the same payload `netviz render -f json`
 ...
 * netviz.render.annotations computes it, and never guessed from pixels.
 *
 ...
 * and it is written down in netviz.edit.operations.SetAnnotation.
 *
 ...
 *
 * Coordinates are links.js's: netviz's are points with `y` upwards, the SVG's
...
window.netvizNotes = (function () {
   ...
   * `clientX` to a netviz coordinate is this file's, not a second copy of it.
   */
   ...
   *
   * The browser's copy of netviz.render.annotations.member_hull, over the
   ...
   * rule, for the same reason, as netviz/drawio/reconcile.py's `_write_geometry`.
   *
  ...
  function openEditor(entry) {
    ...
    editing.entryHandle = window.netvizKeys.overlay(box, {
  ...
  function closeEditor() {
    ...
    var going = editing;
    editing = null;
    window.netvizKeys.closeOverlay(going.entryHandle);
  }

 ...
 * uses* -- netvizDetail's -- because a second description of an element is a
...
var netvizA11y = (function () {
  ...
  function outline(ids, meta) {
    ...
    ids.forEach(function (id) {
      ...
      button.addEventListener("click", function (event) {
        ...
        if ((event.shiftKey || event.ctrlKey || event.metaKey) && window.netvizSelect) {
          window.netvizSelect.toggle([window.netvizSelect.addressOf(record)]);
          return;
        }
  ...
  function group(id) {
    ...
    if (found && window.netvizCull) { window.netvizCull.materialise(id); }
    return found;
  }
   ...
   */
  function centre(id) {
    var known = window.netvizCull ? window.netvizCull.centreOf(id) : null;

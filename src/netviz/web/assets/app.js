 ...
 *
 *   stream    `netviz web` on a file, a pipe or nothing. The text lives in
 *             the browser and is posted to /api/render whenever it settles.
 *             Nothing is on disk; nothing is written.
 *   session   `netviz web DIR`. The server holds the inventory tree, and
 ...
 *
 * What an info box looks like is not here: it is netvizDetail, in
 * netviz/render/assets/detail.js, shared with the page `netviz render
...
(function () {
   ...
   *  whether it is folded. What makes a drop into one a `netviz edit move`:
  ...
  function options() {
    var request = {
      ...
      collapse: netvizContainers.collapsed(),
      select: netvizSearch.selector() || null,
  ...
  function query() {
    ...
    // the request, and in the cache key the request doubles as.
    var folded = netvizContainers.collapsed();
    ...
    var narrowing = netvizSearch.selector();
  ...
  function render() {
    ...
    var key = mode === "session" ? netvizSession.graphPath(query()) : null;
  ...
  function apply(result, key) {
    ...
    if (svg) {
      if (!reuse || key !== currentView) {
        ...
        netvizCull.index(el.viewport.firstElementChild, details);
        measureZoomCeiling();
      }
    ...
    netvizA11y.annotate(details, { view: el.layer.value });
    ...
    netvizLinks.annotate(el.viewport.firstElementChild, geometry, details);
    ...
    netvizNotes.annotate(el.viewport.firstElementChild, geometry, annotations, details);
    ...
    netvizContainers.annotate(el.viewport.firstElementChild, geometry, containers, details);
    ...
    if (mode === "session") { netvizSession.drew(result); }
  ...
  function showProblems(problems, dangling, omitted) {
    ...
    if (omitted > 0) {
      ...
      more.textContent = "and " + omitted + " more, not listed here. "
        + "Run 'netviz validate' for all of them.";
   ...
   * `netviz validate --fix` prints.
   */
  function fixButtons(problem) {
    var offered = problem.fixes || [];
    if (!offered.length || mode !== "session" || !netvizSession.isWritable()) { return null; }
    ...
    offered.forEach(function (offer) {
      ...
      button.addEventListener("click", function () {
        netvizSession.fix(problem, offered.length > 1 ? offer.key : null);
      });
   ...
   * line means anything. See netviz.loader.LoadError.location for the shape.
   */
  function navigation(location) {
    if (mode === "session") {
      return {
        title: "open " + location,
        go: function () {
          if (!netvizSession.locate(location)) { toast("no file at " + location, "error"); }
        }
      };
    }
  ...
  function toast(text, kind) {
    ...
    netvizA11y.announce(text, kind === "error");
  }
  ...
  function hitAt(target) {
    ...
    if (hit) { return hit; }
    var address = netvizLinks.linkAt(target);
  ...
  function inspectFocused() {
    ...
    if (mode === "session") {
      netvizSession.reveal(here.record.id);
      ...
      netvizSession.select(netvizSelect.size()
        ? netvizSelect.addresses()
        : [netvizSelect.addressOf(here.record)]);
      netvizA11y.select(here.element);
    }
    netvizA11y.announce("inspecting " + netvizA11y.label(here.record), false);
    return true;
  }
  ...
  function showFailure(next) {
    if (mode !== "session") {
      toast("failure mode needs a tree: start netviz with a directory", "error");
      return false;
    }
    ...
    if (!failure.on) {
      clearFailure();
      el.summary.textContent = lastStatus.concat(culling()).join("  ·  ");
      netvizA11y.announce("failure mode off", false);
      return true;
    }
    failure.element = null;
    sayFailure("failure mode: click an element to see what its loss would isolate");
    netvizA11y.announce("failure mode on; click an element", false);
    return true;
  }
  ...
  function paintFailure(result) {
    ...
    netvizA11y.announce(result.message, false);
  }
  ...
  /* How a record is drawn is netvizDetail's, in
   * netviz/render/assets/detail.js, and is shared with the self-contained
   * page `netviz render -f html` writes: the two front ends show the same
  ...
  function describe(record) {
    return netvizDetail.describe(record, {
  ...
  function measureZoomCeiling() {
    var natural = netvizCull.naturalScale(view.k);
  ...
  el.source.addEventListener("keydown", function (event) {
    ...
    if (mode === "session") { netvizSession.markDirty(); } else { schedule(); }
  });
  ...
  el.canvas.addEventListener("dblclick", function (event) {
    ...
    if (netvizNotes.editAt(event)) { event.preventDefault(); return; }
    if (netvizLinks.insert(event)) { event.preventDefault(); }
  });
  ...
  el.canvas.addEventListener("contextmenu", function (event) {
    ...
    if (netvizLinks.remove(event)) { event.preventDefault(); return; }
    if (netvizMenu.openAt(event)) { event.preventDefault(); }
  });
  ...
  el.canvas.addEventListener("click", function (event) {
    ...
    netvizLinks.select(netvizLinks.linkAt(event.target));
    ...
    // here too.
    var note = netvizNotes.at(event.target);
    netvizNotes.select(note ? note.id : null);
    if (note) {
    ...
    if (event.shiftKey || event.ctrlKey || event.metaKey) {
      ...
      if (picked) {
        netvizSelect.toggle([netvizSelect.addressOf(picked.record)]);
        netvizA11y.focus(picked.record.element, { quiet: true, scroll: false });
      }
      return;
    }
    var hit = recordAt(event.target);
    ...
    netvizSelect.set([netvizSelect.addressOf(hit.record)], { quiet: true });
    ...
    if (failure.on) { askImpact(netvizSelect.addressOf(hit.record)); return; }
    ...
    if (mode === "session") { netvizSession.reveal(netvizSelect.addressOf(hit.record)); }
    if (pinned === hit.record.element) { hideInfo(true); netvizA11y.select(null); return; }
    pinned = hit.record.element;
    showInfo(hit, event);
    el.info.classList.add("pinned");
    netvizA11y.select(hit.record.element);
  });
  ...
  (function draggable() {
    ...
    window.addEventListener("mousemove", function (event) {
      ...
      if (netvizNotes.dragging()) { netvizNotes.move(event); return; }
   ...
   * netviz.web.bindings declares. The keyboard, the palette and the shortcut
   ...
   * shows them greyed with "open a folder with 'netviz web DIR' for this"
   * rather than pretending netviz cannot do them.
   */
  function defineCommands() {
    ...
    K.define("menu.open", { run: function () { netvizMenu.openFocused(); } });
    K.define("dismiss", {
      run: function () {
        ...
        if (netvizSelect.size() && netvizSelect.clear()) { return; }
        if (netvizStyle.isOpen()) { netvizStyle.show(false); return; }
        if (!el.info.hidden) { hideInfo(true); netvizA11y.select(null); return; }
        if (failure.on) { showFailure(false); return; }
        if (mode === "session" && netvizSession.isScrubbing()) {
          netvizSession.showTimeline(false);
          return;
        }
        if (mode === "session" && netvizSession.isDiffing()) {
    ...
    K.define("validate", {
      run: function () {
        ...
        netvizA11y.announce(el.problemCounts.textContent || "nothing to report", false);
      }
    });
    ...
    K.define("node.move", {
      run: function (context) {
        ...
        if (direction) { netvizA11y.move(direction); }
      }
    });
    K.define("node.link", {
      run: function (context) { netvizA11y.cycleLink(context.chord === "Shift-l" ? -1 : 1); }
    });
    K.define("node.first", { run: function () { netvizA11y.first({ quiet: false }); } });
    K.define("node.last", { run: function () { netvizA11y.last({ quiet: false }); } });
    ...
    K.define("node.select", {
      run: function () {
        ...
        netvizSelect.set([netvizSelect.addressOf(here.record)], { quiet: true });
        inspectFocused();
      }
    });
    ...
    K.define("link.bend", { run: function () { pickLink(); return netvizLinks.bend(); } });
    K.define("link.straighten", {
      run: function () { pickLink(); return netvizLinks.straighten(); }
    });
    K.define("link.route", {
      run: function () {
        pickLink();
        if (!netvizLinks.hasLink()) { return netvizLinks.route(null); }
        K.prompt({
          ...
          confirm: "Route",
          onSubmit: function (values) { netvizLinks.route((values.style || "").trim()); }
        });
        return true;
      }
    });
    K.define("link.pin-route", {
      run: function () { pickLink(); return netvizLinks.pinRoute(); }
    });
    K.define("link.label.reset", {
      run: function () { pickLink(); return netvizLinks.resetLabel(); }
    });
     ...
     * not a device. */
    K.define("annotation.create", { run: function (context) { netvizNotes.create(context); } });
    K.define("annotation.edit", {
      run: function () { return netvizNotes.edit(); },
      enabled: function () { return netvizNotes.editable(); }
    });
     ...
     * registrations out of it to prove that every binding netviz declares has
    ...
    K.define("container.fold", {
      run: function (context) {
        ...
        if (wanted) { netvizContainers.select(wanted); }
        // With nothing picked, the focused element names the namespace: see
        // netvizContainers.toggle for why the keyboard needs that path.
        var focused = netvizA11y.focused();
        return netvizContainers.toggle(focused ? focused.record.id : "");
      }
    });
    ...
    K.define("view.fit", {
      run: function () { resetView(); netvizA11y.announce("diagram fitted", false); }
    });
    ...
    K.provide("elements", function () {
      return netvizA11y.elements().map(function (entry) {
        return {
          id: entry.element,
          title: netvizSelect.addressOf(entry.record) || entry.record.name || entry.element,
          detail: netvizA11y.label(entry.record),
          group: entry.record.type === "edge" ? "link" : "element",
          run: function () {
            el.canvas.focus();
            netvizA11y.focus(entry.element, { quiet: false });
            inspectFocused();
          }
        };
      });
  ...
  function pickLink() {
    ...
    if (here && here.record && here.record.type === "edge") {
      netvizLinks.select(String(here.record.id || ""));
    }
  }
  ...
  function toggle(control, said) {
    ...
    render();
    netvizA11y.announce(said + (control.checked ? " on" : " off"), false);
  }
  ...
  function stepLayer(delta) {
    ...
    render();
    netvizA11y.announce(options[next].textContent, false);
  }
  ...
  var bridge = {
    ...
    focusElement: function (address) {
      var found = netvizA11y.elements().filter(function (entry) {
        return netvizSelect.addressOf(entry.record) === address;
      })[0];
      if (!found) { return false; }
      netvizA11y.focus(found.element, { quiet: false });
      return true;
    }
  };
  ...
  var keyHost = {
    ...
    isWritable: function () { return mode === "session" && netvizSession.isWritable(); },
    hasFocus: function () { return !!netvizA11y.focused(); },
  ...
  netvizLinks.attach({
    writable: function () { return mode === "session" && netvizSession.isWritable(); },
    view: function () { return el.layer.value; },
    refuse: function (why) { toast(why, "error"); },
    write: function (operation, said) { netvizSession.ops([operation], said); }
  });
  ...
  netvizNotes.attach({
    ...
    write: function (operations, said) { return netvizSession.ops(operations, said); }
  });
   ...
   * server's; see netviz/edit/containers.py. */
  netvizContainers.attach({
    writable: function () { return mode === "session" && netvizSession.isWritable(); },
    view: function () { return el.layer.value; },
    ...
    reparent: function (addresses, namespace, said) {
      return netvizSession.reparent(addresses, namespace, said);
    }
  });
  ...
  netvizCull.attach({ el: el, culled: culled });
  netvizA11y.attach({ el: el, bringIntoView: bringIntoView });
  ...
  netvizSelect.attach({
    ...
    changed: function (addresses) {
      ...
      if (mode === "session") { netvizSession.select(addresses); }
      netvizStyle.refresh();
    }
  });
  // A resized canvas is a different viewport, so a different part of the
  // diagram has to be drawn.
  window.addEventListener("resize", function () { netvizCull.schedule(); });
  ...
  defineCommands();
  netvizSelect.defineCommands();
  netvizSearch.defineCommands();
  netvizStyle.attach({
    el: el,
    writable: function () { return mode === "session" && netvizSession.isWritable(); },
    ops: function (operations, said) { return netvizSession.ops(operations, said); },
    refuse: function (why) { toast(why, "error"); }
  });
  netvizStyle.defineCommands(netvizKeys);
  ...
  netvizClipboard.defineCommands({
    layer: function () { return el.layer.value; },
    post: function (path, body) { return netvizSession.request(path, body); },
    applied: function (result, said) { return netvizSession.adopt(result, said); },
    toast: toast,
    refuse: function (why) { toast(why, "error"); }
  });
  netvizSession.defineCommands(bridge);
  netvizTour.defineCommands(bridge);
  netvizKeys.attach(keyHost);
  ...
  fetch("/api/bindings", { cache: "no-store" })
    ...
    .then(function (payload) {
      netvizKeys.load(payload);
      el.commandsKey.textContent = netvizKeys.chordFor("palette");
      el.shortcutsKey.textContent = netvizKeys.chordFor("help");
    })
  ...
  function bootFace() {
    return fetch("/api/state", { cache: "no-store" })
      ...
      .then(function (state) {
        ...
        mode = "session";
        netvizSession.attach(bridge, state);
        render();
        return state;
      })
      ...
      // answered -- see netvizTour.boot.
      .then(function (state) { netvizTour.boot(mode, state); });
  }

 ...
 * runs a command through netvizKeys.run, under the id netviz.web.bindings
...
window.netvizMenu = (function () {
  ...
  function openAt(event) {
    ...
    var annotation = window.netvizNotes.at(event.target);
    if (annotation) {
      window.netvizNotes.select(annotation.id);
    ...
    if (!record) {
      var container = window.netvizContainers.at(event.target, event);
      if (container) {
        window.netvizContainers.select(container.namespace);
  ...
  function openFocused() {
    ...
    var annotation = window.netvizNotes.selection();
    if (annotation) {
    ...
    var container = window.netvizContainers.selection();
    if (container) {
  ...
  function show(hit, at) {
    ...
    if (record && target !== "selection") {
      ...
      if (element) { window.netvizA11y.focus(element, { quiet: true, scroll: false }); }
    }
    ...
    var entry = window.netvizKeys.overlay(layer, {
  ...
  function close() {
    ...
    var going = open;
    open = null;
    window.netvizKeys.closeOverlay(going.entry);
  }
  ...
  function targetFor(record) {
    var address = window.netvizSelect.addressOf(record);
    if (address && window.netvizSelect.size() > 1 && window.netvizSelect.has(address)) {
      return "selection";
    }
  ...
  function caption(record, annotation, target, container) {
    if (target === "selection") {
      var count = window.netvizSelect.size();
  ...
  function choose(button, spec) {
    ...
    close();
    window.netvizKeys.run(spec.id, context);
  }
   ...
   *
   * Kinds come from netviz.models.KINDS by way of /api/bindings rather than
  ...
  function toggleSub(button, spec) {
    ...
    shutSub();
    var kinds = window.netvizKeys.kinds();

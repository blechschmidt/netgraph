"""The HTTP face of ``netviz web``: a fixed set of routes over one session.
...
**The scratchpad** — ``netviz web`` on a file, a pipe or nothing — holds a
...
**The editing session** — ``netviz web DIR`` — holds a real inventory tree
(:mod:`netviz.web.session`) and exposes it::
    ...
    POST /api/ops             a batch of netviz.edit operations, applied
...
session over the copy (:mod:`netviz.web.tour`); every route above answers from
...
file routes go through :func:`~netviz.web.session.relative_path`, which
...
answer; :mod:`netviz.web.session` decides, and :mod:`netviz.edit` writes. No
...
:mod:`netviz.httpserve`'s, and is the same here as for ``netviz watch
--serve``.
...
from netviz.edit import ConflictError, EditError, ValidationRefused
from netviz.httpserve import (
    DEFAULT_HOST,
    BackgroundServer,
    LocalHandler,
    bind,
...
from netviz.web.tour import TOUR_END_PATH, TOUR_PATH, Scratch, Tours
...
#: Default port of ``netviz web``. One above the preview's, so a watch run
...
#: The keyboard bindings and the command list, from :mod:`netviz.web.bindings`.
...
#: :mod:`netviz.edit` will actually do rather than what the picture suggests.
...
#: Apply the mechanical repair for one diagnostic (:mod:`netviz.fixes`).
...
#: one to keep in files and open with ``netviz watch``; a browser textarea is
...
class _Handler(LocalHandler):
    ...
    #: The scratch copies the guided tour edits; see :mod:`netviz.web.tour`.
    tours: Tours
    ...
    def _events(self, session: EditingSession, *, body: bool) -> None:
        ...
        * **Beat.** An idle stream writes a comment every
          :data:`~netviz.web.events.HEARTBEAT_SECONDS`, which keeps a proxy
        ...
        if self.session is None:
            self._refuse(
                RequestError(
                    ...
                    "document-stream scratchpad; open a folder with 'netviz web DIR'"
                )
            )
            return
        try:
    ...
    def _clipboard(self, session: EditingSession, path: str) -> None:
        ...
        :class:`~netviz.web.session.Change`: a copy produces a *fragment* and
    ...
    def _presence(self, session: EditingSession) -> None:
        ...
        loopback bind and the ``Host`` check in :class:`~netviz.httpserve.LocalHandler`.
        """
        try:
            payload = self._read_json()
        except RequestError as exc:
    ...
    def _put(self, path: str) -> None:
        ...
        if session is None or not path.startswith(FILE_PREFIX):
            self.send_text(
                ...
                "open a folder with 'netviz web DIR --write' to edit files",
                allow="GET, HEAD, POST",
            )
            return
        try:
    ...
    def _icons_state(self) -> dict[str, Any]:
        ...
        :func:`~netviz.web.preview.icon_choices` will accept back, ``theme``
    ...
    def _refuse(self, exc: BaseException, *, body: bool = True) -> None:
        ...
        bug in netviz and is re-raised, which the base server turns into a 500
...
def _paths(query: Mapping[str, Sequence[str]]) -> list[str] | None:
    ...
    checked by :func:`~netviz.web.session.relative_path` before it becomes a
...
def _client_id(query: Mapping[str, Sequence[str]]) -> str | None:
    ...
    :meth:`~netviz.web.session.EditingSession.write_file`.
    """
...
def _requested_path(path: str) -> str:
    ...
    :func:`~netviz.web.session.relative_path`, which is the only thing that
...
def _namespace(payload: dict[str, Any]) -> str:
    ...
    than a client bug: the root namespace is spelled ``""`` throughout netviz,
...
def _point(value: Any) -> tuple[float, float] | None:
    """A paste anchor, in netviz's own coordinates, or ``None``.

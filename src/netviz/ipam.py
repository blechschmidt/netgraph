...
``netviz list subnets`` answers "which prefixes exist?". This module answers
...
Everything here is a pure function of :func:`netviz.subnets.subnets_of` and
an :class:`~netviz.loader.inventory.Inventory`; nothing formats, so the CLI
...
#: :func:`netviz.validate.validate` sorted them into, so ``netviz ipam`` and
#: ``netviz validate`` list the same problems in the same sequence.
#:
...
class Utilisation:
    ...
    def sort_key(self) -> tuple[str, int, int, int]:
        """Instance, family, network address, prefix length — as :mod:`netviz.subnets`."""
        return (
            self.vrf,
...
def conflicts(inventory: Inventory, config: ValidationConfig | None = None) -> tuple[Finding, ...]:
    """The address-plan findings of :func:`netviz.validate.validate`.

    Filtered rather than recomputed, so a suppression in ``netviz.toml`` or a
    ``netviz/ignore`` annotation silences a finding here exactly as it does
    in ``netviz validate``, and a rule re-graded to ``info`` is reported at
...
@dataclass(frozen=True, slots=True)
class Report:
    """Everything ``netviz ipam`` prints, before it is rendered."""

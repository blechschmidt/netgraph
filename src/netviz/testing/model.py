"""What ``netviz test`` produces: one verdict per assertion, grouped by suite.
...
class Verdict:
    ...
    def classname(self) -> str:
        ...
        return f"netviz.test.{self.suite}"
...
class TestReport:
    ...
    #: netviz's version, quoted so a stored report says what produced it.
    version: str = ""
    #: Names given on the command line that matched no suite. Reported rather
    #: than ignored: ``netviz test typo`` running zero assertions and exiting

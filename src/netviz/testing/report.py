...
:class:`~netviz.testing.model.TestReport` and none of them decides anything:
...
from netviz.diagnostics import JUnitCase, as_junit, dump_json
from netviz.fsio import display_path
from netviz.loader.inventory import short_name
from netviz.testing.model import FAILED, PASSED, SKIPPED, SuiteResult, TestReport, Verdict
...
def as_junit_report(report: TestReport) -> str:
    ...
    return as_junit(
        "netviz test",
        list(_cases(report)),
        properties={"inventory": _root_text(report.root), "netviz": report.version},
...
def _root_text(root: Path) -> str:
    ...
    ``-i`` resolves to an absolute path, and an absolute path in a JUnit
    ``<property>`` is the one part of the document that differs between two
    machines looking at the same tree -- so a report generated on a laptop and
    one generated by CI disagree about a run that was otherwise identical. The
    ...
    :func:`netviz.drift.report._root_text`: :func:`~netviz.fsio.display_path`.
    """

...
:class:`~netviz.testing.model.TestReport`. Nothing here re-reads the inventory
or re-resolves a reference: every claim is checked against the graphs
:func:`~netviz.render.graph.build_graph` already builds and the routes
:mod:`netviz.trace` already finds, so a failing test and a rendered diagram
...
from netviz.impact.engine import anchors_for, single_points
from netviz.impact.graphs import LAYERS, POWER, LayerView, views
from netviz.loader.inventory import Inventory, short_name
from netviz.models import Assertion, AssertionLayer, AssertionType, TestSuite
from netviz.power import PowerPlan, power_plan
from netviz.render.graph import FilterSpec, Graph, Layer, Node, build_graph
...
from netviz.trace import DEFAULT_MAX_HOPS, TracedPath, TraceError, TraceResult, trace
    ...
    *,
    ...
    Args:
        ...
        names: Globs narrowing which suites run, matched against the
            ...
            :attr:`~netviz.testing.model.TestReport.unmatched` rather than
...
def _selected(context: _Context, assertion: Assertion) -> tuple[str, ...]:
    ...
    out of the same :func:`~netviz.testing.selectors.select_nodes` the renderer

...
``--profile``
    ...
    ``netviz.toml``, each described by the settings it overrides. A profile
...
``netviz completion powershell`` works at all.
"""
...
from netviz.errors import NetvizError, count_text
...
#: The shells netviz ships a completion script for. Click generates the first
...
class PowerShellComplete(ShellComplete):
    ...
    netviz has to supply itself. It is *only* the transport: every candidate
    still comes from the completers in this module, so ``--profile <Tab>`` reads
    the same ``netviz.toml`` on Windows as it does anywhere else.
    ...
    therefore what makes ``_NETVIZ_COMPLETE=powershell_complete`` do anything.
    """
...
def complete_export_format(
    ...
    """``netviz export`` formats, described by the export registry itself."""
    return _items(
...
def complete_test_suite(
    ...
    For ``netviz test [SUITE]...``. A suite is not an element, so it is not in
    the element index that :func:`complete_element` reads.
    """
    from netviz.loader import load_tree  # imported late: completion must start fast
...
def complete_profile(
    ...
    """The profiles declared by the inventory's ``netviz.toml``.
    ...
    """
    from netviz.config import load_config  # imported late: completion must start fast
    from netviz.settings import profile_summaries
...
def complete_fix(
    ...
    """
    from netviz.fixes import fixable_rules, spec_for

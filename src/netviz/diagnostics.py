"""Machine-readable reports for ``netviz validate``.
...
inventory can have — a :class:`~netviz.loader.inventory.LoadError` from the
loader and a :class:`~netviz.validate.Finding` from the semantic engine — into
...
``sarif``
    ...
    rule in :data:`netviz.rules.RULES` is described once in the tool driver,
...
by command — ``netviz drift`` wants one per element, not one per difference —
...
inventory root's own location (:attr:`Report.prefix`). ``netviz -i
...
from netviz.errors import format_path
from netviz.loader.inventory import Inventory, LoadError, namespace_of
from netviz.rules import RULES, Rule, Severity
...
class Diagnostic:
    ...
    rule: str
    #: Severity after configuration, ``netviz/ignore`` and ``--strict``.
    severity: Severity
    ...
    def from_record(cls, record: Mapping[str, Any]) -> Diagnostic:
        ...
        pull-request bot of :mod:`netviz.review` subtracts the base branch's
    ...
    def from_finding(cls, finding: Finding, inventory: Inventory) -> Diagnostic:
        ...
        The location comes from :attr:`~netviz.validate.Finding.site` when the
...
def _pointer(field_path: Sequence[str | int]) -> str | None:
    ...
    key netviz itself would reject.
    """
    if not field_path:
        return None
...
def build_report(
    ...
    *,
    ...
    Args:
        ...
        findings: What :func:`netviz.validate.validate` returned.
...
def _root_of(inventory: Inventory) -> Path:
    ...
    ``netviz -i switch.yaml`` loads a single file, and the loader records that
...
def as_json(report: Report) -> dict[str, Any]:
    ...
    return {
        ...
        "tool": {"name": "netviz", "version": report.version},
        "inventory": {
...
def _sarif_result(
    ...
    result["partialFingerprints"] = {"netvizFinding/v1": fingerprint(diagnostic)}
...
def fingerprint(diagnostic: Diagnostic) -> str:
    ...
    **not** from the line: inserting a document above a broken one must not
    ...
    :mod:`netviz.review` subtracts a base branch's diagnostics from a head's
...
def as_junit(
    ...
    *,
    ...
    this shape. ``time`` is deliberately absent: netviz reports whether an
...
def dump_json(payload: Any) -> str:
    ...
    Every structured document netviz writes goes through here, so two of them

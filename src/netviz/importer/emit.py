"""Turning a :class:`~netviz.importer.draft.Draft` into YAML somebody wants to edit.
...
reason :mod:`netviz.scaffold` does: a dumper cannot interleave comments, and
...
from netviz.errors import clip_text
from netviz.fsio import safe_file_stem
from netviz.importer.draft import Draft, DraftCable, DraftDevice, DraftInterface
from netviz.models import API_VERSION
from netviz.scaffold import SCHEMA_FILE_NAME

__all__ = ["CABLES_FILE", "DEVICES_DIR", "render_cables", "render_device", "render_draft", "scalar"]

#: Where devices and cables go, matching the tree ``netviz init`` writes.
...
#: escaped :func:`~netviz.importer.draft.comment_text` cannot turn a document
#: into a wall of text.
...
_DEVICE_HEADER: Final[tuple[str, ...]] = (
    "Written by 'netviz import'. This is a first draft to edit and commit, not an "
    "artefact to regenerate.",
    "Every value below was reported by the device. A line marked 'inferred:' is "
    "netviz's reading of what was reported and is worth checking first; nothing was "
...
_CABLE_HEADER: Final[tuple[str, ...]] = (
    "Written by 'netviz import'. One document per adjacency, separated by '---'.",
...
def _device_paths(devices: Sequence[DraftDevice]) -> Iterator[tuple[DraftDevice, str]]:
    ...
    ``SW1`` and ``sw1`` are two elements to netviz and one file name on macOS
    ...
    :func:`~netviz.fsio.safe_file_stem` handles those, and it does so on every

...
This is the part of ``netviz import`` that touches the outside world; every
...
collection command themselves and hands netviz what it printed. That is a
...
comes from ``NAME=PATH``, from ``--host``, from the input's own ``netviz-``
banner when netviz generated it, or from the file name — in that order, most
...
to the configuration sniffer, which reads the banner of a file netviz wrote
and otherwise matches a line only one of the seven grammars can have
(:func:`netviz.importer.config.sniff`). The CSV is what is left: it is the one
format with no shape of its own, so it is the fallback rather than a match.
Sniffing is what makes ``netviz import collected/*`` work on a directory
...
from netviz.errors import NetvizError, clip_text, echo_value
from netviz.fsio import write_text
from netviz.importer.config import CONFIG_DIALECT_NAMES, CONFIG_READERS, banner_element
from netviz.importer.config import sniff as sniff_config
from netviz.importer.csvlinks import read_csv_links
from netviz.importer.draft import Draft
from netviz.importer.emit import render_draft
...
#: :mod:`netviz.export.config` also writes. The split matters: a capture
#: describes a running kernel, a configuration describes what somebody asked
#: for, and :mod:`netviz.drift.coverage` treats their silences differently.
...
class ImportSourceError(NetvizError):
    """Raised when an input cannot be read, or cannot be read as what it claims.

    Shares its exit status with :class:`~netviz.errors.LoaderError`: from the
    user's side both mean "netviz could not parse the thing you pointed it at".
    """
...
def dialect_of(entry: ImportInput, requested: str = "auto") -> str:
    ...
    from the file's own banner when netviz wrote it and from a line only one
    grammar can have otherwise (:func:`netviz.importer.config.sniff`). What is
    ...
    Raises:
        ImportSourceError: The input is JSON but no capture netviz reads.
    """
    if requested != "auto":
        return requested
...
def _host_for(entry: ImportInput, dialect: str) -> tuple[str, bool]:
    ...
    2. **The file's own banner.** Anything ``netviz export`` wrote carries
       ``netviz-element:`` (:mod:`netviz.export.config.header`), which names
    ...
    if dialect in CONFIG_READERS:
        raise ImportSourceError(
            ...
            "after the device. A configuration 'netviz export' wrote names it in its header"
        )
...
def write_files(files: dict[str, str], target: Path, *, force: bool = False) -> list[Path]:
    ...
    for relative, content in files.items():
        ...
        try:
            ...
            # LF on every platform; see netviz.fsio. An imported tree is the

"""The intermediate inventory ``netviz import`` builds before writing anything.
...
* **Honesty.** A field is either observed or absent. Where netviz had to
...
:mod:`netviz.models` objects. Its job is to describe a document that a human
is about to edit, including the parts netviz could not fill in; a model would
...
class DraftInterface:
    ...
    name: str
    #: One of the :class:`~netviz.models.interface.InterfaceType` values.
    type: str = "ethernet"
...
class DraftDevice:
    ...
    #: netviz reads reports "this is a workstation".
    ...
    def refine_kind(self, kind: str, comment: str | None) -> None:
        ...
        if kind != "computer":
            self.note(
                f"another input reported this device as a {kind}; netviz kept "
...
@dataclass(slots=True)
class Draft:
    """Everything one ``netviz import`` run observed, before it is a tree."""
    ...
    #: :func:`~netviz.importer.run.build_draft`.
    #:
    #: The importer itself has no use for this — a document is written from what
    #: was observed, not from what did the observing. ``netviz drift`` does:
        ...
        if existing is None:
            existing = self.devices[name] = DraftDevice(
                ...
                kind_comment=(
                    ...
                    "'computer' is netviz's neutral default — correct it by hand"
                ),
            )
        return existing
    ...
    def prune(self) -> None:
        ...
        neighbour whose port id netviz could not read is named without a single

...
document before netviz wrote one, so reading it back is mostly structural:
``ethernets``/``wifis``/``bonds``/``bridges``/``vlans``/``tunnels`` are the six
sections :mod:`netviz.export.config.netplan` writes the six interface types
...
:class:`~netviz.importer.draft.DraftInterface` has fields for only half of it.
...
address on every host in the tree. :mod:`netviz.importer.iproute` drops the
...
:meth:`~netviz.importer.draft.Draft.note` keeps only the first of two
identical lines, so a file with forty ``dhcp4:`` keys is one line in the run
report. A section netplan has and netviz has no interface type for —
...
#: The netviz interface type of each netplan section, in the order
#: :mod:`netviz.export.config.netplan` writes them — members before the
...
_DROPPED: Final[tuple[tuple[tuple[str, ...], str], ...]] = (
    (
        ("routes",),
        "netplan 'routes:' say where traffic leaves by; 'netviz import' writes interfaces "
    ...
    (
        ("nameservers",),
        "netplan 'nameservers:' configure host name resolution; netviz describes links and "
...
_DROPPED_TUNNEL: Final[tuple[tuple[tuple[str, ...], str], ...]] = (
    (
        ...
        "netviz models a tunnel as its own document naming both ends (docs/schema.md §14), "
        "so they were not imported",
    ),
    (
        ("peers", "key"),
        "a netplan tunnel's 'peers:'/'key:' name the far ends and hold key material; netviz "
...
def _network(text: str, *, source: str, draft: Draft) -> dict[str, Any] | None:
    ...
    ``yaml.safe_load`` rather than :mod:`netviz.loader.documents`: that loader
...
def _apply_link(interface: DraftInterface, mapping: dict[str, Any]) -> None:
    ...
    if isinstance(mac, str) and _looks_like_mac(mac):
        ...
        interface.comments.append(
            ...
            "rather than being one; netviz records only a fixed MAC"
        )
    ...
    if mode is False or (isinstance(mode, str) and mode.strip().lower() == "off"):
        ...
        interface.comments.append(
            f"netplan states 'activation-mode: {comment_text(str(mode))}' here; netviz "
...
def _note_unread_sections(network: dict[str, Any], *, source: str, draft: Draft) -> None:
    ...
    for section in network:
        ...
        if section in _RENDERER_KEYS:
            continue
        draft.note(
            f"{source}: the netplan section {section!r} maps onto no netviz interface type, "
...
def _looks_like_mac(value: str) -> bool:
    """Is this the six-octet form netviz records, rather than a netplan policy?"""

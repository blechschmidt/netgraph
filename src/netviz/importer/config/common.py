...
from netviz.banner import DIALECT_KEY, ELEMENT_KEY, parse_banner
...
def read_int(value: str, *, low: int | None = None, high: int | None = None) -> int | None:
    ...
    docstring <netviz.importer.config>` says why), so every conversion in every
...
#: :data:`netviz.export.config.CONFIG_DIALECTS` lists them. Spelled here rather
...
_SIGNATURES: Final[tuple[tuple[str, re.Pattern[str]], ...]] = (
    ...
    # ``table inet netviz {`` or ``table ip filter {``: the one construct only
...
def sniff(text: str) -> str | None:
    ...
    The banner wins when there is one: a file netviz wrote says what it is, and
...
def _banner(text: str) -> Mapping[str, str]:
    """The ``netviz-*`` keys of ``text``, whichever comment marker it uses."""
...
def stanzas(text: str) -> Iterator[tuple[str, list[str]]]:
    ...
    shape for different reasons -- netviz chose it, Debian inherited it -- and
...
def fold_into(draft: Draft, host: str, source: str) -> None:
    ...
    which :mod:`netviz.drift.coverage` reads as "no dialect saw this", making

...
:mod:`netviz.export.config.frr` writes routing — static routes, a VRF, an OSPF
area, a BGP autonomous system — and the handful of things a routing daemon says
about an interface somebody else built. A :class:`~netviz.importer.draft.Draft`
...
bond0`` in an ``frr.conf`` reads exactly like ``interface eno1``. netviz's
...
:meth:`~netviz.importer.draft.DraftInterface.merge` lets a specific type
...
is not in the file — :func:`netviz.export.config.frr._interface_block` omits an
...
asked. That is why :mod:`netviz.drift.coverage` grants ``frr`` no capability:
...
:mod:`netviz.importer.iproute` gives: ``lo`` terminates no cable and appears in
no topology. Only ``lo`` — netviz's ``loopback`` type is for a router loopback
...
from netviz.importer.draft import Draft, DraftDevice, DraftInterface, comment_text
from netviz.importer.names import interface_name

__all__ = ["read_frr"]

#: The kernel loopback, and only it. ``lo0`` and ``Loopback0`` are *declared*
#: router loopbacks -- an operator wrote them down, netviz's ``loopback`` type
...
def _read_hostname(words: list[str], *, source: str, host: str, draft: Draft) -> None:
    ...
    draft.note(
        ...
        f"{host!r}; netviz kept the host you named -- re-run with --host "
...
def _open_interface(
    ...
    if original is not None:
        interface.comments.append(
            ...
            "here because a netviz interface name may only hold letters, digits, '.', '/' "
            "and '-'"
        )
    interface.comments.append(
        "inferred: FRR configures interfaces but creates none, so this file never says what "
        "the link is; 'ethernet' is netviz's neutral type -- correct it if this is a bridge, "
...
def _report(seen: _Seen, *, source: str, draft: Draft) -> None:
    ...
    if seen.maskless:
        draft.note(
            ...
            f"prefix length; netviz writes addresses in CIDR form and will not guess a mask, "
            f"so they were not imported"
        )
    if seen.unrecognised:
        draft.note(
            f"{source}: {len(seen.unrecognised)} FRR directive(s) netviz does not read "

...
:mod:`netviz.export.config.wireguard`, which writes one file per WireGuard
...
* The ``netviz-element`` banner names the *device* this configuration belongs
...
netdev is exactly netviz's ``tunnel`` type, so unlike
:mod:`netviz.importer.config.frr` there is no fallback involved. What the file
holds beyond an address and an MTU has no home in a draft interface, and the
peers are the important case: netviz models a tunnel as its own document naming
...
:mod:`netviz.drift.coverage` grants this dialect no capability at all.
...
from netviz.importer.config.common import fold_into, read_int
from netviz.importer.draft import Draft, DraftInterface, comment_text
...
#:
#: A key that is here is a key netviz understands and has decided about, which
#: is a different statement from the one made about a key that is not.
_KEY_NOTES: Final[dict[str, str]] = {
    "PublicKey": (
        "wg-quick identifies a peer by its public key and netviz identifies one by element "
    ...
    "PersistentKeepalive": (
        "a keepalive changes what the tunnel does on the wire and no netviz field states an "
        "interval"
    ),
    "ListenPort": (
    ...
    "PrivateKey": (
        "netviz stores no key material (docs/schema.md section 14.2); the value was not read "
...
def _uncommented(raw: str) -> str:
    ...
    whitespace is stripped too, because that is what netviz's own emitter
...
def _interface_of(source: str, *, draft: Draft) -> DraftInterface | None:
    ...
    interface.comments.append(
        "this is one end of a WireGuard tunnel; netviz models the tunnel as its own document "
...
def _report(seen: _Seen, *, source: str, draft: Draft) -> None:
    ...
    if seen.peers:
        draft.note(
            f"{source}: the file names {seen.peers} WireGuard peer(s); netviz models a "
    ...
    if seen.sections:
        draft.note(
            f"{source}: {len(seen.sections)} section(s) netviz does not read "
            f"({_listed(seen.sections)}) were left alone"
        )
    if seen.unknown:
        draft.note(
            f"{source}: {len(seen.unknown)} setting(s) netviz does not read "
            f"({_listed(seen.unknown)}) were left alone; only the values a netviz interface "

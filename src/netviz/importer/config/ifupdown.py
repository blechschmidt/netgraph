...
The mirror of :mod:`netviz.export.config.ifupdown`, and the reader most likely
to be handed something netviz did not write: an operator's file holds hooks,
...
The stanza splitter is :func:`~netviz.importer.config.common.stanzas`, which is
...
:class:`~netviz.importer.draft.DraftInterface`, and which address list an
...
length. An address with neither is reported and dropped: a prefix netviz made
up would be compared against the inventory as though the host had stated it, and
would be reported as drift on the strength of netviz's own guess.
...
interface, with the stacking reported instead of stated: netviz's model
...
topology and carries only host-scope addresses; netviz's ``loopback`` type is
...
from a name that looks like ``wlp1s0`` would be netviz guessing at hardware
from a string.
...
than as routes; ``mapping`` picks a logical stanza by running a script netviz
cannot evaluate; and ``source``/``source-directory`` cannot be followed at all,
because netviz reads the one file it was given and not a filesystem — that
...
from netviz.importer.config.common import fold_into, read_int, stanzas
...
#: :mod:`netviz.export.config.ifupdown` checks against, so the round trip of a
...
#: The families ``iface`` may declare that netviz models. ifupdown also knows
...
def _read_stanza(state: _Pass, header: str, body: list[str]) -> None:
    ...
    if keyword in _START_KEYWORDS:
        ...
        state.note(
            ...
            "script at boot; netviz cannot evaluate one, so it was not imported"
        )
    else:
        state.note(
            f"{keyword!r} opens a stanza netviz does not read; only 'auto', "
...
def _read_iface(state: _Pass, words: list[str], body: list[str]) -> None:
    ...
    if family is not None and family not in _FAMILIES:
        state.skipped.add(interface_name(base)[0] or base)
        state.note(
            f"{raw!r} is configured for the {family!r} address family, which netviz does "
...
def _read_options(state: _Pass, interface: DraftInterface, body: list[str]) -> None:
    ...
    for line in body:
        ...
        if keyword == "address":
            ...
            state.note(
                "'gateway' states a default route; 'netviz import' writes interfaces and "
    ...
    for value in values:
        ...
        if prefix is None:
            state.note(
                ...
                "netmask; netviz does not invent a prefix length, because one it made up "
...
def _read_members(state: _Pass, interface: DraftInterface, value: str, *, kind: str) -> None:
    ...
    for word in value.split():
        ...
        # interface called 'none' would be one netviz invented.
        if word == _NO_MEMBERS:
            continue
    ...
    if interface.vlan is None:
        interface.vlan = DraftVlan(
            ...
            comment=(
                ...
                f"the digits the interface name ends in, and netviz read {vid} the same way"
            ),
        )
...
def _note_include(state: _Pass, keyword: str, arguments: list[str]) -> None:
    ...
    state.note(
        f"the file pulls in {target!r} with '{keyword}'; netviz reads the file it was given "

...
makes a netviz ``cable`` a cable rather than a line on a picture — or invent
...
from netviz.importer.draft import Draft, DraftCable
...
def _endpoint(device: str, port: str, *, number: int, source: str) -> tuple[str, str]:
    ...
    if name is None or interface is None:
        ...
        raise CsvError(
            ...
            "holds no character a netviz name may use"
        )

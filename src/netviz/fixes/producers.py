...
offered and neither is picked. ``netviz validate --fix`` applies a rule with
...
particular tree. That is :mod:`netviz.fixes.run`'s job — it applies a fix,
...
from netviz.edit.paths import format_field_path
...
def _fix_stale_geometry(finding: Finding, inventory: Inventory) -> Sequence[Fix]:
    ...
    What ``netviz layout --prune`` does for a whole drawing, for the single
...
FIXES: Final[tuple[FixSpec, ...]] = (
    ...
    FixSpec(
        "W138",
        "Drops the stale entry from the layout document, as 'netviz layout --prune' would",
        _fix_stale_geometry,
    ),
    FixSpec(
        "W140",

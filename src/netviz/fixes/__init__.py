...
``netviz validate`` says what is wrong. For a good part of the catalogue it
...
:mod:`netviz.edit` can already express every one of those changes. This package
is the join: a table of rule to *fix producer*, a producer being a pure function
from a :class:`~netviz.validate.Finding` and the
:class:`~netviz.loader.Inventory` it was found in to the operations that repair
it.

    from netviz.fixes import fixes_for, repair
...
* ``netviz validate --fix`` applies every unambiguous repair and prints what
...
:mod:`netviz.fixes.producers` decides *what* a repair is, and never picks
between two of them: a rule that admits more than one plausible reading offers
all of them and lets a person choose. :mod:`netviz.fixes.run` decides whether a
...
from netviz.fixes.model import Choice, Fix, FixProducer, FixSpec
from netviz.fixes.producers import (
    FIXES,
    fixable_rules,
    fixes_for,
    producer_for,

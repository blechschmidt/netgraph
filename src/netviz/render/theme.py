...
:mod:`netviz.models.theme` says what a theme *document* may contain. This
...
2. **The theme's rules**, most specific first. Specificity is the number of
   conditions a selector states (:attr:`~netviz.models.theme.ThemeSelector.clauses`)
...
4. **The built-in palette** (:mod:`netviz.render.palette`), which supplies a
...
takes, for the same reason. A ``netviz.toml`` may set the default under
...
#: came from ``netviz.toml``. Spelled out so "no theme" is sayable, exactly as
#: ``--icons none`` is.
NO_THEME: Final = "none"

#: Extensions a theme file may have. A theme is YAML, like everything else a
#: user writes for netviz.
THEME_SUFFIXES: Final[tuple[str, ...]] = (".yaml", ".yml")

#: Directory holding the themes that ship with netviz, one file each.
...
class StyleTarget:
    ...
    Flattened out of :class:`~netviz.render.graph.Node` and
    :class:`~netviz.render.graph.Edge` so that selector matching has one shape
...
class Theme:
    ...
    def extend(self, other: Theme | None) -> Theme:
        ...
        marked. That is what lets a ``netviz.toml``'s inline ``[theme]`` table
...
def _load_file(path: Path) -> Theme:
    ...
    from netviz.loader.documents import YamlSyntaxError, decode_text, parse_documents
    from netviz.models.document import parse_theme
...
def resolve_theme(named: str | Theme | None, *, inline: Theme | None = None) -> Theme | None:
    ...
    ``inline`` is the ``[theme]`` table of ``netviz.toml``. It is appended
...
def rules_from(entries: Iterable[Any], *, name: str) -> Theme:
    ...
    Routed through :func:`~netviz.models.document.parse_theme` rather than
    validating the rules directly, so a bad colour in ``netviz.toml`` is
    ...
    """
    from netviz.models.document import parse_theme
    from netviz.models.theme import default_envelope

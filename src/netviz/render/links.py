...
end that already knows the URL of every element — ``netviz report``, whose
diagrams link each device to its own page in the bundle — and needs nothing
expanded. Both satisfy :class:`Linker`, which is all
:mod:`netviz.render.dot` asks for, so the backend has one code path for
...
netviz knows: every element carries the document, and usually the line, it was
...
class Linker(Protocol):
    """What :mod:`netviz.render.dot` asks for the URL of one element.
...
class LinkMap:
    ...
    ``netviz report`` writes a page per device and wants the diagrams to link

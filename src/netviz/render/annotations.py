...
(:mod:`netviz.models.annotation`). What they do change is the picture, and
...
**Which members survive.** :func:`~netviz.annotations.area_members` resolves an
...
graph carries that answer (:attr:`~netviz.render.graph.Graph.annotation_targets`),
...
its leader line, which is exactly what :func:`~netviz.annotations.note_anchor`
...
link is drawn — with the colours read out of :mod:`netviz.render.palette`,
...
from netviz.models.annotation import Area, AreaSpec, Legend, Note, NoteSpec
...
#: :mod:`netviz.render.ids`: the prefix guarantees the id starts with a letter,
...
class LegendSwatch:
    ...
    label: str
    color: str
    #: One of :data:`~netviz.models.annotation.SWATCH_SHAPES`.
    shape: str = "box"
    description: str = ""
...
class AnnotationViews:
    ...
    which is the same order :class:`~netviz.annotations.AnnotationSet` iterates
    ...
    def area_of(self, fqn: str) -> AreaView | None:
        ...
        A node belongs to at most one container in every backend netviz has —
...
def _id(prefix: str, fqn: str, taken: set[str]) -> str:
    ...
    Slugged by :func:`~netviz.render.ids.slug`, so the alphabet and the
...
def member_hull(area: AreaView, positions: Iterable[tuple[float, float]]) -> Box | None:
    ...
    engine places nothing and netviz has to compute the frame itself. ``None``

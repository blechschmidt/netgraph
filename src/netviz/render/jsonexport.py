...
a consumer that draws the graph itself, rather than reading a picture netviz
...
(:func:`~netviz.render.annotations.parse_markup`) — so that a consumer draws
the same bold word netviz draws without reimplementing the parser, and a
...
from netviz.layout.routing import Route
from netviz.models import API_VERSION
from netviz.power import Feed, PowerNode
from netviz.render.aggregate import AggregateView, BundleView
from netviz.render.annotations import AreaView, LegendView, NoteView, annotation_views
from netviz.render.diffview import DiffOverlay, Mark
...
from netviz.render.routes import default_routing, route_plan
from netviz.render.styles import PLAIN, ResolvedStyle, StyleMap
...
def to_json(graph: Graph, options: RenderOptions | None = None, *, indent: int = 2) -> str:
    ...
    ``kind`` pair every netviz document carries, so a consumer can pin what it
    parses::
...
def graph_to_dict(graph: Graph, options: RenderOptions | None = None) -> dict[str, Any]:
    ...
    # to draw what netviz drew without reimplementing the ladder of §22,
...
def _layout(graph: Graph) -> dict[str, Any] | None:
    ...
    coordinate in this document is exactly where netviz would draw it, and a
    client can lay nothing out at all. ``partial`` means some are, and the rest
    are the client's problem — the same problem netviz hands to Graphviz.
...
def annotations_payload(
    ...
    Resolved by :func:`~netviz.render.annotations.annotation_views`, the same
...
def _node(
    ...
    if node.netns is not None:
        ...
        if node.address != node.fqn:
            ...
            # the one node whose identity is not an address ``netviz edit``
            # accepts. See :attr:`~netviz.render.graph.Node.address`.
...
def _aggregate(view: AggregateView) -> dict[str, Any]:
    ...
    look them up in ``netviz render -f json`` without ``--collapse``, or to
...
def _link_layout(
    ...
    :mod:`netviz.layout.avoid` added on top of them so the line misses the
    ...
    chosen one rather than having to guess. ``route`` is the line netviz
...
def _diff_mark(overlay: DiffOverlay | None, ident: str) -> dict[str, Any] | None:
    """What ``netviz diff`` says about one node or link, or ``None``.

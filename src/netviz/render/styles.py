...
:mod:`netviz.models.style` is what a document may say. :mod:`netviz.render.theme`
...
It does not decide emphasis. A ``--highlight`` and a ``netviz diff`` overlay
...
:mod:`netviz.drawio.styles` are where a vocabulary name becomes a backend's
...
from netviz.models.style import NO_ICON, hex_colour
from netviz.render.graph import Edge, EdgeKind, Graph, Node
from netviz.render.icons import IconTheme, suffix_order
from netviz.render.palette import edge_style_for, node_style_for
from netviz.render.theme import StyleTarget, Theme
...
#: The shape a name netviz does not know falls back to.
...
#: The same shape as :data:`netviz.config._EMPTY_SEVERITIES`.
...
class ResolvedStyle:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        JSON draws what netviz drew without reimplementing the ladder.
        """
        values = {
            "fill": self.fill,
            "stroke": self.stroke,
...
def resolve_style(
    ...
    *,
    ...
    Args:
        target: What is being drawn; see :class:`~netviz.render.theme.StyleTarget`.
        ...
        output: The output format, which decides whether a vector or a raster
            icon is preferred (see :func:`~netviz.render.icons.suffix_order`).

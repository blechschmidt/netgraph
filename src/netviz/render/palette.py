...
* :mod:`netviz.render.dot` draws the graph, and picks a Graphviz shape, a fill
  and a line style from here;
* :mod:`netviz.render.annotations` builds an ``auto: layers`` legend (§21)
  from exactly the same entries, so a swatch cannot claim a colour the drawing
  does not use;
* :mod:`netviz.drawio.styles` exports the same encoding as mxGraph styles;
* :mod:`netviz.render.mermaid` restates it as ``classDef`` rules, which is the
...
from netviz.render.aggregate import AGGREGATE_KIND
...
#: The cluster frame netviz draws itself when the layout engine will not.
...
#: than it buys. :data:`~netviz.render.graph.SUBNET_KIND` is not hardware — it
#: is a prefix the addresses imply — so it gets a palette entry of its own and
#: is drawn with rounded corners (see ``netviz.render.dot._node_style``).
...
#: export (:mod:`netviz.drawio.styles`) cannot drift away from the picture
#: netviz itself draws.
...
#: What a link netviz has no palette entry for is drawn as.

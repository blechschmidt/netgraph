...
inventory than pop a native tooltip over it, and ``netviz web`` cannot be
...
(``netviz/render/assets/page.css``, ``page.js`` and the ``detail.js`` the web
...
Everything on the page is something netviz already produces:

* the picture is the SVG :func:`~netviz.render.dot.to_image` lays out, with
  ``--element-ids`` forced on (the ids are how a shape and a record find each
  other) and made embeddable by :mod:`netviz.render.fragment`;
* the records are :func:`~netviz.render.details.build_details`, which are the
  ``-f json`` export plus an element id and a links cross-reference, so the
  detail panel cannot disagree with ``netviz render -f json``;
* how a record is *drawn* is ``detail.js``, shared with ``netviz web``.
...
:mod:`netviz.render.fragment`: the repeated font attributes and the repeated
``--icons`` payload, which is now one shared :class:`~netviz.render.fragment.IconLibrary`
...
``detail.js`` and ``netviz web`` are untouched by this.

Annotations
-----------

A note, an area and a legend (§21) are *drawn*, so they reach the page the way
everything else does: inside the SVG that :func:`~netviz.render.dot.to_image`
...
``netviz render -f json`` writes — so that a panel showing what a note says,
...
from netviz.render.details import build_details, printable
from netviz.render.dot import to_image
from netviz.render.fragment import IconLibrary, fragment
...
DATA_ELEMENT_ID: Final = "netviz-data"
...
#: renderer's own assets directory is also where ``netviz web`` reads
...
def _clean(value: Any) -> Any:
    ...
    (:func:`~netviz.render.details.printable`): a C0 control, and the
...
def policy(style: str, scripts: Iterable[str] = ()) -> str:
    ...
    # A page with no script at all -- every page ``netviz report`` writes --
...
@cache
def asset_text(names: tuple[str, ...], *, package: str = "netviz.render") -> str:
    """The named files in ``<package>/assets``, concatenated.

    Cached: ``netviz watch -f html`` re-renders on every save, and the files
    do not change between two of them. ``package`` is a parameter because
    ``netviz report`` inlines its own style sheet the same way, out of its own

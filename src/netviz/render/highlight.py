...
``netviz path A B --highlight`` answers a question about *two* elements and
...
as :class:`~netviz.render.options.RenderOptions` promises — so it belongs
here rather than in :class:`~netviz.render.graph.FilterSpec`, which decides
what exists.
...
already gave them — :attr:`Node.fqn <netviz.render.graph.Node.fqn>` and
:attr:`Edge.id <netviz.render.graph.Edge.id>` — so a producer never has to
...
class Highlight:
    ...
    <netviz.render.options.RenderOptions.highlight>`, where ``None`` is the
    ...
    #: :attr:`Edge.id <netviz.render.graph.Edge.id>` of each link to emphasise.

...
from netviz.render.dot import to_dot, to_image
from netviz.render.graph import Graph
from netviz.render.html import html_document, to_html
from netviz.render.jsonexport import to_json
from netviz.render.mermaid import mermaid_advisories, to_mermaid
...
#: serves one at all (``netviz watch -f html --serve``).
#:
...
class Renderer:
    ...
    #: (:data:`~netviz.httpserve.DEFAULT_CSP`) is too strict for it, or
    ...
    #: (:attr:`~netviz.render.graph.Layer.RACK`)? An elevation is a *grid* —
    ...
    #: (:mod:`netviz.render.highlight`)? Emphasis is *visual weight* — a bold
    #: outline, a dimmed fill — so it needs a backend that decides how things
    #: look. Mermaid and JSON have no such vocabulary, so ``netviz path
    #: --highlight`` does not offer them rather than silently ignoring the flag.
    supports_highlight: bool = False
    #: Can this backend draw a :class:`~netviz.render.diffview.DiffOverlay`?
    ...
    #: beside one, so ``netviz diff -f mermaid`` says so rather than emitting
...
#: Every format ``netviz render -f`` accepts, keyed by name, in help order.
...
def supports_highlight(format: str) -> bool:
    """Would ``format`` draw a :class:`~netviz.render.highlight.Highlight`?
...
def supports_diff(format: str) -> bool:
    """Would ``format`` draw a :class:`~netviz.render.diffview.DiffOverlay`?

...
One caveat about the output, which :func:`netviz.render.dot.to_image`'s tests
...
#: with a letter — a fully-qualified name need not — and keeps netviz's ids out
...
class ElementIds:
    ...
    (:func:`~netviz.render.details.build_details`) cannot disagree about which
    ...
    #: One id per edge, indexed as :attr:`~netviz.render.graph.Graph.edges` is.
    edges: tuple[str, ...] = ()

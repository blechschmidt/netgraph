...
Every filter netviz had before this module removes *detail by removing
...
:class:`~netviz.render.graph.Graph` *before* any renderer sees it, so DOT,
...
:mod:`netviz.render.ids` and a tooltip from :mod:`netviz.render.details`
...
``netviz init`` scaffolds — is not a level a reader distinguishes, because
...
from netviz.models import Adapter, Device, InterfaceType, PatchPanel
from netviz.render.graph import Edge, EdgeKind, Graph, Node, NodeType
...
#: §3, :data:`~netviz.render.graph.SUBNET_KIND` and
#: :data:`~netviz.render.graph.TUNNEL_KIND` are all taken, so this cannot
...
#: :data:`~netviz.render.graph.SUBNET_ID_PREFIX` gives.
...
class AggregateView:
    ...
    #: address, then prefix length — the order :mod:`netviz.subnets` uses.
    subnets: tuple[str, ...] = ()
...
class BundleView:
    ...
    #: The member links, in graph order. Whole :class:`~netviz.render.graph.Edge`
...
class AggregateSpec:
    ...
    Unlike :class:`~netviz.render.graph.FilterSpec`, none of this removes
...
def collapse_namespaces(graph: Graph, namespaces: Iterable[str]) -> Graph:
    ...
    Returns:
        ...
        :attr:`~netviz.render.graph.NodeType.AGGREGATE`. Links that crossed a
        boundary keep their identity and attach to the collapsed node; links
        wholly inside one are dropped from :attr:`~netviz.render.graph.Graph.edges`
...
def _prefixes(nodes: Iterable[Node]) -> tuple[str, ...]:
    ...
    :attr:`~netviz.render.graph.Node.routable_addresses`, so a namespace does
...
def _aggregate_ports(graph: Graph) -> Mapping[str, Mapping[str, str]]:
    ...
    :class:`~netviz.render.graph.PortView`, because membership is a fact about
    the *aggregate* interface — ``members: [Gi1/0/1, Gi1/0/2]`` — and a port does
    not know it is in one.
    """
    mapping: dict[str, dict[str, str]] = {}
    for fqn, node in graph.nodes.items():
        # Everything else a node can stand for owns no interfaces at all — a
        # PDU (§17.1), a user or a group (§19.1) — so there is nothing on it that
        # could be aggregated, and no ``interfaces`` attribute to ask.
        element = node.element
        if not isinstance(element, (Device, Adapter, PatchPanel)):
            continue
        ports = {

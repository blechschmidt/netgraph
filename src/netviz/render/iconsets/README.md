...
fall back to netviz's plain Graphviz shapes.
...
file and SVG output gets the vector one. See `netviz/render/icons.py`.
...
subnet is a cloud. Each carries the same accent colour netviz gives that kind
in its plain shapes, so a diagram stays colour-coded either way.

**The artwork is netviz's own** and is covered by the project's MIT licence.
...
netviz render --icons ./cisco-official -f svg -o topology.svg
```

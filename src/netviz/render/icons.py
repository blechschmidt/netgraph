...
One theme ships with netviz, :data:`CISCO`, drawn in the network-diagram idiom
Cisco made standard — a router is a cylinder with arrows on it, a switch a flat
slab, a subnet a cloud. The art is netviz's own (MIT, like the rest of the
...
* Its **SVG** output can reference an SVG icon directly, so netviz prefers the
...
* Plain ``-f dot`` output is a file someone else will run through Graphviz with
  a ``-T`` netviz cannot see, so it takes the portable choice — raster first.
...
#: :data:`~netviz.render.aggregate.AGGREGATE_KIND`: a collapsed namespace is
...
#: open padlock would put netviz's guess about a security property into a
...
#: Directory holding the themes that ship with netviz, one subdirectory each.
...
#: The theme that ships with netviz.
CISCO: Final = IconTheme(
    name="cisco",
    directory=_BUNDLED_DIR / "cisco",
    description="Cisco-style network topology icons, drawn for netviz",
)

...
``netviz plan`` already answers *what changed* — element by element, field by
...
* **The plan.** Everything finer — that an element was *updated* rather than
  ...
  device under a new name — comes from :class:`~netviz.plan.model.Plan` and
  from nowhere else. :mod:`netviz.plan` stays the single notion of a
...
from netviz.loader import short_name
...
#: should send the reader to ``netviz plan``, not grow a paragraph.
...
class DiffOverlay:
    ...
    :attr:`Node.fqn <netviz.render.graph.Node.fqn>` and :attr:`Edge.id
    <netviz.render.graph.Edge.id>` — for the same reason
    :class:`~netviz.render.highlight.Highlight` is: a producer never has to
    ...
    #: :attr:`Edge.id <netviz.render.graph.Edge.id>` to what happened to it.
    edges: Mapping[str, Mark] = field(default_factory=dict)
    #: Node or edge id to the field paths that moved, as ``netviz plan``
    ...
    #: The changeset itself, as :meth:`Plan.to_dict <netviz.plan.model.Plan>`
    ...
    #: a :class:`~netviz.plan.model.Plan` so that :mod:`netviz.render` keeps
    #: no dependency on :mod:`netviz.plan`.
    ...
    def narrowed(self, addresses: Iterable[str]) -> DiffOverlay:
        ...
        What ``netviz diff --target`` is: the reader asked about four devices

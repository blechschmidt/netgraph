...
``netviz render -f svg`` produces a standalone document: an XML declaration, a
...
* the native tooltips and the links are kept or dropped by the caller, because
  the two embedders disagree: ``netviz web`` replaces both with an info box
...
* every ``id`` can be given a prefix, because a page holding more than one
  ...
  :mod:`netviz.render.html` — would otherwise hold each element id several
...
#: :func:`netviz.render.dot._promote_tooltips` moves it into a ``<title>``;
...
#: (:func:`netviz.render.dot._embed_icons` inlines each file as a ``data:``
...
#: ``netviz report`` carry: ``../devices/sw-core.html`` and nothing cleverer.
...
class IconLibrary:
    ...
    :mod:`netviz.render.html`), which is what makes the icon payload a
    ...
    *,
    ...
    Args:
        ...
        local_links: Also keep anchors pointing at another file of the same
            ...
            ``netviz report`` diagram reaches its own page; it stays off by

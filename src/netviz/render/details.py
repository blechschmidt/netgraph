...
shape carries (:mod:`netviz.render.ids`), so a consumer can answer "what is
this?" for whatever is under the pointer.

Two consumers, one builder
--------------------------

:func:`build_details` produces the records. ``netviz web`` serves them to its
info boxes as JSON; :mod:`netviz.render.dot` turns each one into a line or
...
Every field comes from :func:`~netviz.render.jsonexport.graph_to_dict`,
...
``netviz render -f json``, and the person comparing the two would have no way
to tell which one was lying.

Display options
---------------

The records are narrowed by the same :class:`~netviz.render.options.RenderOptions`
    ...
    *,
    ...
    Args:
        ...
        ids: The identity assigned to this graph. Pass the same
            :class:`~netviz.render.ids.ElementIds` the rendering was made
...
def printable(text: str) -> str:
    ...
    (:func:`netviz.render.dot._dot_string`) and this module's own text — so

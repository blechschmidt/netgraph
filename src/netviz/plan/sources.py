...
Both sides of a diff are an :class:`~netviz.loader.Inventory`, and this module
...
from netviz.plan.model import StateRef
...
class PlanSourceError(NetvizError):
...
class MissingInventory(PlanSourceError):
    ...
    repository grew its ``netviz/`` folder at some commit, and every revision
...
def check_revision(rev: str) -> str:
    ...
    revision: ``netviz web`` takes one from a query string, and a page this
...
def git_ref(root: Path, ref: str) -> Iterator[Path]:
    ...
    with TemporaryDirectory(prefix="netviz-plan-") as directory:
...
def git(arguments: list[str], *, cwd: Path, failure: str, stdin: bytes | None = None) -> bytes:
    ...
    The only place in netviz that starts a git process for reading history,

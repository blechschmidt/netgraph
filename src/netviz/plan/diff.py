...
lives in :mod:`netviz.plan.sources`, :mod:`netviz.plan.live` and the two
commands.

The engine does four things, in this order:

1. Address every element on both sides (:mod:`netviz.plan.address`).
2. Pair up the ones that are only on one side, structurally, so a rename is
   reported as a rename (:mod:`netviz.plan.identity`).
3. Diff the bodies of everything that survived the pairing
   (:mod:`netviz.plan.document`).
4. Put the result in dependency order (:mod:`netviz.plan.order`).
"""
...
from netviz.models import ElementBase
...
from netviz.plan.document import Diffable, body_of, diff_documents, document_of
from netviz.plan.identity import detect_renames
from netviz.plan.model import Action, Change, Plan, StateRef
...
def diff(
    ...
    *,
    ...
    Returns:
        An ordered :class:`~netviz.plan.model.Plan`. Empty when the two states

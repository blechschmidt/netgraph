...
``netviz plan`` a dozen times a day and wants one number:

    Plan: + 3 to add, ~ 5 to change, - 1 to destroy.

The **per-element detail** is for the review that has to happen before
``netviz apply``, and is written on the assumption that the reader knows the
...
from netviz.plan.model import ACTION_SIGILS, Action, Change, FieldChange, Plan
from netviz.plan.paths import MISSING
...
def write_plan(console: Console, plan: Plan, *, verbose: bool = False) -> None:
    ...
    """
    console.print(f"netviz plan: {plan.source.description} → {plan.target.description}")
    console.print()
    if plan.empty:

...
same network differently produce an empty plan, which is what makes ``netviz
plan --from HEAD`` usable on a tree somebody has just run ``netviz fmt`` over.
...
from netviz.models import AnnotationBase, ElementBase, Layout
from netviz.plan.model import FieldChange
from netviz.plan.paths import MISSING, Selector, Step

"""``netviz report``: the as-built document an engineer is asked to hand over.
...
Nothing is derived twice. The tables are :mod:`netviz.listing` (what
``netviz list`` prints), the address plan is :mod:`netviz.ipam`, the cable
schedule is :func:`netviz.export.cables.schedule` (what
``netviz export cable-list`` writes), the power schedule is
:mod:`netviz.power`, the findings are :mod:`netviz.validate` through
:mod:`netviz.diagnostics`, and the diagrams are the graphs
:mod:`netviz.render` draws. A per-site view is those same functions over a
narrowed inventory (:func:`netviz.loader.inventory.subset`), which is what makes
...
from netviz.report.bundle import Bundle
...
from netviz.report.diagrams import LAYER_TITLES, Diagrams
from netviz.report.layout import DEVICES_DIR, DIAGRAMS_DIR, SITES_DIR, Layout, anchor, page_slug
from netviz.report.model import FORMATS, Cell, Column, Diagram, Meta, Page, Report, Section, Table
from netviz.report.options import Options
from netviz.report.pages import build
from netviz.report.stamp import (
    EPOCH_ENV_VAR,
    NO_TIMESTAMP,
    Revision,
    git_revision,
...
from netviz.report.write import JSON_FILE, render_pages, stylesheet
...
def generate(
    ...
    *,
    ...
    Raises:
        NetvizError: A template is missing or failed to render.
    """

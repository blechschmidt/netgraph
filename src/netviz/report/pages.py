...
:class:`~netviz.report.model.Section`, every section is fields, diagrams and
tables, and every table came from a derivation netviz already had — see
:mod:`netviz.report.collect`.
...
here (:func:`~netviz.report.layout.anchor`) and are part of what the tests
...
from netviz.export.context import ExportContext, ExportOptions, location_of
from netviz.export.manifest import Recorder
from netviz.ipam import build_report as build_ipam_report
from netviz.loader.inventory import Inventory, short_name, subset
from netviz.models import Element, Interface, Pdu, format_watts
from netviz.models.netns import ROOT_NETNS, NetnsDefinition
from netviz.render.graph import Edge, EdgeKind, Graph, Layer, Node
from netviz.report.collect import (
    DATA_LAYER,
    ROOT_SITE,
    Context,
    Scope,
...
from netviz.report.diagrams import LAYER_TITLES
from netviz.report.layout import anchor
from netviz.report.model import BLANK, Cell, Column, Field, Page, Report, Section, Table
from netviz.subnets import Subnet, subnets_of
...
_FINDINGS_NOTE: Final = (
    "These are the findings 'netviz validate' reports for this inventory. A report is "
...
def _addressing_section(context: Context, scope: Scope) -> Section:
    """The address plan, with the utilisation ``netviz ipam`` reports."""
    ...
    return Section(
        ...
        notes=(
            (
                ...
                "findings table, and 'netviz ipam --conflicts' explains each one.",
            )
            if report.findings
            else ()
        ),
...
def _vlan_matrix(context: Context, scope: Scope, prefixes: Sequence[Subnet]) -> Table:
    ...
    # First non-empty name wins, exactly as ``netviz list vlans`` decides it:
    ...
    tables = [
        Table(
            ...
            note=(
                ...
                "leg of it this is. 'netviz export cable-list' writes the same rows as CSV, "
...
def _panel_maps(context: Context, scope: Scope) -> tuple[Table, ...]:
    ...
    (:attr:`~netviz.models.patchpanel.PatchPanelSpec.interfaces`) are the only
...
def _power_section(context: Context, scope: Scope) -> Section:
    """The PDU load schedule, from :func:`netviz.power.power_plan`."""
    return Section(
        key="power",
        title="Power",
...
def _accounts_section(context: Context, scope: Scope) -> Section:
    """Who the network is for, from :func:`netviz.identity.identity_plan` (§19).

...
module works out both, and hands :mod:`netviz.report.pages` a
...
Everything is derived from what netviz already computes:

* the tables are :mod:`netviz.listing`, the same functions ``netviz list``
  prints, over a narrowed inventory (:func:`~netviz.loader.inventory.subset`);
* the address plan is :func:`netviz.ipam.build_report`;
* the cable schedule is :func:`netviz.export.cables.schedule`;
* the power schedule is :func:`netviz.power.power_plan`;
* the diagrams are :func:`~netviz.render.graph.build_graph` at each layer.
...
(:func:`~netviz.render.aggregate.common_prefix`) — and ``--group-depth``
...
from netviz.listing import Listing
from netviz.loader.inventory import Inventory, namespace_of, short_name, subset
from netviz.models import Adapter, Device, PatchPanel, Pdu
from netviz.power import PowerPlan, power_plan
from netviz.render.aggregate import common_prefix
from netviz.render.graph import Edge, Graph, Layer, Node, build_graph
from netviz.report.diagrams import Diagrams
from netviz.report.layout import Layout
from netviz.report.model import BLANK, Cell, Column, Meta, Table
from netviz.report.options import Options
...
def _members_of(inventory: Inventory, group: str, *, level: int) -> frozenset[str]:
    ...
    :func:`~netviz.loader.inventory.subset` only where everything they join
...
def table_from_listing(
    ...
    *,
    ...
    """One :mod:`netviz.listing` table, with its subject column linked.

...
:func:`netviz.layout.routing.route` decides one leg at a time and looks at
...
bends go*; :func:`~netviz.layout.routing.route` still draws the line through
...
:class:`~netviz.edit.operations.SetLinkGeometry` stores and the same list a
...
:func:`netviz.layout.routing.route`'s local Z or L: the diagram is no worse
...
from netviz.layout.routing import Anchor
...
#: the same point. The same value :mod:`netviz.layout.routing` uses, for the
#: same reason.
EPSILON: Final = 1e-9
...
class Obstacle:
    ...
    :func:`netviz.layout.routing.route` clips those stubs off afterwards.
    """
    ...
    def waypoints(
        ...
        *,
        ...
        Returns:
            ...
            :func:`netviz.layout.routing.route`, or ``None`` when the line the
        ...
        # node centres :func:`~netviz.layout.routing.route` supplies anyway.
...
def _local(
    ...
    """The line :func:`netviz.layout.routing.route` already draws for one leg.

...
the *graph*, not here — by ``netviz layout``, which builds one — or against
...
from netviz.layout.geometry import Box, Geometry, LinkGeometry, Placement, Routing
...
def resolve_geometry(inventory: Inventory, view: str) -> Geometry:
    ...
    Args:
        ...
        view: The layer name, as :class:`~netviz.render.graph.Layer` spells
            it (``l1``, ``l3``, ``routing``, ...).

    Returns:
        One :class:`~netviz.layout.geometry.Geometry`. Empty — and cheap —

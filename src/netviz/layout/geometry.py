...
:class:`~netviz.models.layout.Layout` is what a *document* says.
...
file, resolves a name or runs Graphviz — see :mod:`netviz.layout.resolve` and
:mod:`netviz.layout.seed` for those.
...
#: not set one (``[editor] grid`` in ``netviz.toml``). Twenty is half of
...
class Geometry:
    ...
    #: The view it belongs to, as :class:`~netviz.render.graph.Layer` spells it.
    view: str = ""

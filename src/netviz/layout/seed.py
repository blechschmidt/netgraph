"""``netviz layout``: seeding an arrangement, dropping it, and pruning it.
...
Every write goes through :mod:`netviz.edit`, so the comments and the
...
from netviz.layout.document import geometry_sections, inline_entry
...
from netviz.layout.graphviz import Drawing, DrawingError, parse_drawing
from netviz.layout.resolve import conflicts_in, resolve_key
from netviz.loader.inventory import Inventory, namespace_of
from netviz.models.layout import LAYOUT_VIEWS, ViewGeometry
...
#: hierarchical layout netviz renders with by default and the right answer for
...
def seed_geometry(
    ...
    * some of it is — the stored positions are kept and only the rest are
      ...
      (:func:`~netviz.render.dot.complete_layout`). Adding a switch and
    ...
    *second* render onwards would be a poor thing to promise. So the candidate
    ...
    Args:
        graph: The :class:`~netviz.render.graph.Graph` to lay out, carrying
            whatever arrangement it already has.
        options: The :class:`~netviz.render.options.RenderOptions` the diagram
        ...
        with_waypoints: Is the caller going to *store* the routes this produces?
            ...
            shape it runs into, and netviz cannot measure a label, so the box
...
def _geometry_of(
    ...
    *,
    ...
    # refreshes it. See :mod:`netviz.layout.routing`.
    nodes = {
...
def _interior(spline: Sequence[tuple[float, float]]) -> tuple[tuple[float, float], ...]:
    ...
    :func:`netviz.layout.routing.route`.
    """
...
@dataclass(frozen=True, slots=True)
class LayoutReport:
    """What ``netviz layout`` found, before it changed anything."""

"""Command line entry point for netviz.
...
``completion``
    ...
    behind it live in :mod:`netviz.completion`; they are attached to the
...
progress notes go to stderr; ``netviz render -f json | jq`` and
``netviz validate > report.txt`` both do what they look like they do.
...
from netviz.console import Align, Console
...
from netviz.converge import REPORT_FORMATS as CONVERGE_FORMATS
from netviz.converge import converge as converge_network
from netviz.converge import write_scripts as write_converge_scripts
from netviz.diagnostics import FORMATS as DIAGNOSTIC_FORMATS
from netviz.diagnostics import Diagnostic, dump_json
from netviz.diagnostics import build_report as build_diagnostics
from netviz.diagnostics import render_report as render_diagnostics
from netviz.diff import draw
...
from netviz.drawio import Level as DrawioLevel
from netviz.drift import FORMATS as DRIFT_FORMATS
from netviz.drift import CompareSpec, DriftReport, check_drift, render_drift
from netviz.drift import write_text as write_drift
...
from netviz.export.config.write import stale_files, write_config
from netviz.fixes import FIXES, FixReport, repair, spec_for
from netviz.fsio import display_path, write_text
from netviz.history import Commit, Frame, HistoryError, Timeline
from netviz.impact import DEFAULT_LIMIT, ImpactError, render_impact
from netviz.impact import LAYERS as IMPACT_LAYERS
from netviz.impact import REPORT_FORMATS as IMPACT_REPORT_FORMATS
from netviz.impact import simulate as run_impact
...
from netviz.ipam import Report as IpamReport
...
from netviz.listing import LISTINGS
from netviz.listing import SUBJECTS as LISTING_SUBJECTS
from netviz.listing import utilisation as utilisation_listing
...
from netviz.query import DOMAINS, Domain, QueryError, QueryResult, attribute_names, evaluate
from netviz.query import parse as parse_query
...
from netviz.report import EPOCH_ENV_VAR, Bundle, git_revision, resolve_timestamp
from netviz.report import FORMATS as REPORT_FORMATS
from netviz.report import JSON_FILE as REPORT_JSON_FILE
from netviz.report import Options as ReportOptions
from netviz.report import generate as generate_report
from netviz.review import Diagram as ReviewDiagram
from netviz.review import Review, Verdict, build_review, render_comment, summarise
from netviz.rules import RULES, Severity, resolve_rule_id, rule_for
from netviz.scaffold import SCHEMA_FILE_NAME, build_scaffold, write_scaffold
...
from netviz.subnets import IPNetwork, subnets_of
from netviz.testing import FORMATS as TEST_FORMATS
from netviz.testing import render_test_report, run_tests, suite_location
from netviz.trace import DEFAULT_MAX_HOPS, TraceError, TraceResult, render_trace, trace
from netviz.trace import REPORT_FORMATS as TRACE_FORMATS
from netviz.validate import Finding
from netviz.validate import validate as run_validation
from netviz.version import as_dict as version_as_dict
from netviz.version import collect as collect_version
from netviz.version import format_text as format_version
...
from netviz.web import DEFAULT_PORT as WEB_PORT
from netviz.web import WebServer
from netviz.web.session import EditingSession, TreeWatcher

if TYPE_CHECKING:
    # ``netviz.fmt`` pulls in ruamel.yaml, which costs roughly 30 ms of import
    ...
    from netviz.fmt import Mode, Summary
...
#: Exit status when a plan is not empty: 'netviz converge plan' exits 0 when
...
class AppContext:
    ...
    no_cache: bool = False
    #: ``netviz.toml``, read at most once per run. A command may ask for it
    ...
    def cache(self) -> DocumentCache | None:
        ...
        ``--no-cache``, the :data:`~netviz.loader.DISABLE_ENV_VAR` environment
        variable, or ``[cache] enabled = false`` in ``netviz.toml``.

        A ``netviz.toml`` that cannot be read at all is *not* one of them. Some
    ...
    def config(self) -> Config:
        """Read ``netviz.toml`` from the inventory root, if there is one.
...
def _show_version(ctx: click.Context, param: click.Parameter, value: bool) -> None:
    ...
    so ``netviz --version`` answers from a directory that holds no inventory.
    """
...
@click.option(
    ...
    help=(
        "Show the netviz, Python and Graphviz versions in use, and exit. "
        "'netviz version --json' is the same report, machine-readably."
    ),
)
@click.option(
    "-i",
...
@click.option(
    ...
    help=(
        ...
        "off for a whole environment. See 'netviz cache info'."
    ),
)
@click.pass_context
def cli(
...
def _path_inside(file: Path, root: Path) -> str:
    ...
    directory, which is what :func:`~netviz.fsio.display_path` answers; the
...
def _next_steps(path: Path, *, with_schema: bool, minimal: bool = False) -> Iterator[str]:
    ...
    ``cd`` is printed only when it is needed: the netviz commands below it are
    run from the inventory root, which is already the shell's directory when
    ``init`` was given no argument. ``netviz test`` is offered only for the
    ...
    if path.resolve() != Path.cwd():
        yield f"  cd {path}"
    yield "  netviz validate"
    if not minimal:
        yield "  netviz test"
    yield "  netviz render -f svg -o network.svg"
    if with_schema:
        yield ""
        yield (
...
#: The sub-command ``netviz import`` runs when the first argument is not the
#: name of one. It is what the command has always been, and keeping it reachable
#: without typing it is not nostalgia: ``netviz import caps/*.json`` is in
...
class _ImportGroup(click.Group):
    """``netviz import`` — a group that still answers to its old signature.
    ...
    sub-command by handing the whole line to :data:`LEGACY_IMPORT`. ``netviz
    import drawio FILE`` reaches the new command, ``netviz import caps/*.json``
...
def import_group() -> None:
    ...
    \b
    netviz import captures/*.json          from what live devices printed
    netviz import drawio diagram.drawio    from a diagram somebody edited

    The first form is the original one and needs no sub-command: anything that
    is not the name of one is read as a capture file, so 'netviz import
...
@click.option(
    ...
    help=(
        ...
        "the same dialects 'netviz export' writes."
    ),
)
@click.option(
    "--host",
...
def import_captures_command(
    ...
    read — and writes a devices/ and cables/ tree in the layout 'netviz init'
    ...
    \b
    netviz import --from lldp   -o net  captures/*.json
    netviz import --from iproute --host pc1 -o net  link.json addr.json
    ip -j addr show | netviz import --host pc1 -o net --dry-run -
    """
...
def _report_import_validation(console: Console, inventory: Inventory, *, root: Path) -> None:
    ...
    rule should not produce a report contradicting its own ``netviz.toml``.
...
#: what lets the report separate "expected, and yours to fill in" from "netviz
...
@click.option(
    ...
    help=(
        "Which view the diagram draws. Read from the file for anything netviz exported; "
        "needed only for a diagram netviz did not write."
    ),
)
@click.option(
...
def import_drawio_command(
    ...
    The file is reconciled by the identity netviz stamped into each cell when
    ...
    result is a 'netviz plan' changeset, shown and confirmed before any file
    is touched.

    A diagram netviz did not export carries no identity, so nothing can be
    reconciled against it. It is read anyway and reported cell by cell, which
    is the honest answer: netviz will not invent hardware from a rectangle.
...
def _diagram_layer(diagram: Diagram, view: str | None) -> Layer:
    ...
    netviz never exported, which says nothing about what it draws.
...
@click.option(
    ...
    show_default=True,
    help=(
        "Input dialect, as for 'netviz import'. 'auto' sniffs each input on its own: "
        "lldp is 'lldpctl -f json', iproute is 'ip -j link show' or 'ip -j addr show', "
        "csv is 'device,port,device,port' cabling rows, and netplan, networkd, ifupdown, "
        "frr, nftables, wireguard and interfaces are the running configuration in the same "
        "dialects 'netviz export' writes."
    ),
)
@click.option(
    "--host",
...
@click.option(
    ...
    metavar="PATTERN",
    help=(
        "Leave out interfaces whose name matches this glob, as 'netviz import --exclude' "
...
def drift_command(
    ...
    Reads the same captures 'netviz import' does — no host is contacted and no
    ...
    \b
    netviz -i net drift --from lldp captures/*.json
    ip -j addr show | netviz -i net drift --host pc1 -
    netviz -i net drift --fail-on drift -F junit caps/* > drift.xml
    """
    inventory = app.load()
    console = app.console()
...
@click.option(
    ...
    help=(
        ...
        "Repeatable. 'netviz rules --fixable' lists the keys."
    ),
)
@click.pass_obj
def validate_command(
    ...
    summary to stderr, so ``netviz validate -F sarif > report.sarif`` writes a
    ...
    \b
        netviz validate --fix              repair what can be repaired
        netviz validate --fix --dry-run    ... and show the diff instead
        netviz validate --fix --choose W114=list   pick between two repairs
...
def _run_validation(
    ...
    *,
    ...
    """Apply the command-line overrides on top of ``netviz.toml`` and validate."""
...
def fmt_command(
    ...
    ``.netvizignore`` and the dot- and underscore-prefix rules apply exactly
...
def _format_stdin(console: Console) -> None:
    ...
    """
    from netviz.fmt import STDIN_NAME, FormatSyntaxError, format_source
...
def _report_formatting(console: Console, summary: Summary, *, mode: Mode) -> None:
    ...
    ``netviz fmt --diff | git apply`` and ``netviz fmt --check | xargs``
    both work.
    """
    from netviz.fmt import Mode
...
def edit_command(ctx: click.Context) -> None:
    ...
    Every subcommand is one operation from ``netviz.edit``. It is applied to an
    ...
    an undo stack, run 'netviz web ./inventory --write'.
    """
    if ctx.invoked_subcommand is None:
...
def edit_apply_command(
    ...
    'netviz plan' are described in terms of. Each object carries an "op" naming
...
def edit_set_command(
    ...
    """Set a field on an element: netviz edit set core-sw spec.model 'C9300'.
...
def edit_duplicate_command(
    ...
    'netviz edit copy' with no --to, under the name a diagram editor gives it:
    it is the same operation, and Ctrl-D in 'netviz web' does exactly this.
    """
    _run_copy(
        app,
        [address],
...
def _run_copy(
    ...
    *,
    ...
    :func:`~netviz.edit.clipboard.copy_plan` against the *inventory*, and only
...
def edit_delete_command(
    ...
    ``netviz layout --prune`` used to have to clean up afterwards.
    """
    _run_edit(
        app,
...
def _edit_config(app: AppContext) -> Config | None:
    """``netviz.toml``, when there is one worth reading.
...
@click.option(
    ...
    help=(
        ...
        "agreed with what the network reports. Reads the same inputs 'netviz import' and "
        "'netviz drift' do."
    ),
)
@click.option(
    ...
    default="auto",
    show_default=True,
    help="Input dialect for --from-live, as 'netviz drift --from' takes it.",
)
@click.option(
    "--host",
    metavar="NAME",
...
@click.option(
    ...
    default=None,
    help=(
        "Write the plan to FILE so 'netviz apply FILE' executes exactly what was reviewed. "
...
def plan_command(
    ...
    by field — that 'netviz apply' executes back onto the *files*.

    Nothing is written unless -out is given, and nothing is ever sent to a
    device: applying to the live network is out of scope.

    \b
    netviz -i net plan --from HEAD
    netviz -i net plan --to ../proposed --json
    netviz -i net plan --from-live captures/*.json -out drift.plan
    netviz -i net plan --fail-on changes --from origin/main
    """
    console = app.console()
    ...
    # (``netviz plan | head``) must not be able to lose the file that the
    # subsequent ``netviz apply`` is about to be given.
    ...
    if out_path is not None:
        stream.info(f"plan written to {out_path}; apply it with 'netviz apply {out_path}'")
...
def _live_plan(
    ...
    *,
    ...
    # count in its place -- the same shape 'netviz drift' reports it in.
...
def apply_command(
    ...
    Each entry becomes a 'netviz edit' operation, so comments, key order and
    ...
    netviz -i net apply drift.plan
    netviz -i net apply drift.plan --target 'device.core/*' --auto-approve
    """
    console = app.console()
    root = app.inventory if app.inventory.is_dir() else app.inventory.parent
    ...
    try:
        ...
        console.error(f"{plan_file} is not a netviz plan: {exc}")
        raise click.exceptions.Exit(EXIT_INVALID) from exc
...
def _state_matches(console: Console, plan: Plan, session: EditSession, *, plan_file: Path) -> bool:
    ...
    console.error(
        ...
        f"changed since. Re-run 'netviz plan' and review the new plan."
    )
...
def converge_group() -> None:
    ...
    'netviz drift' says how the live network differs from the inventory and
    'netviz export config' says what a device would run if it agreed. This
    ...
    netviz never applies any of it. There is no transport here and no flag
...
@click.option(
    ...
    default="auto",
    show_default=True,
    help="Input dialect, exactly as 'netviz drift --from' takes it.",
)
@click.option(
    "--host",
    metavar="NAME",
...
@click.option(
    ...
    help=(
        ...
        "netviz's own imperative grammar and covers every device kind; the other five "
...
def converge_plan_command(
    ...
    Reads the same captures 'netviz drift' does — no host is contacted and no
    ...
    are pending, mirroring 'netviz plan'.

    \b
    netviz -i net converge plan caps/*.json
    netviz -i net converge plan --dialect netplan -o scripts/ caps/*.json
    netviz -i net converge plan --rollback -o rollback/ caps/*.json
    """
    inventory = app.load()
    console = app.console()
    ...
    if out_dir is not None:
        ...
        commentary.warn(
            ...
            "carries every inverse command, and 'netviz converge plan -F json' shows them"
        )
...
_FILTER_OPTIONS: Final[tuple[Callable[[Any], Any], ...]] = (
    ...
    click.option(
        ...
        help=(
            ...
            "equivalent query and are combined with it; 'netviz query --explain' "
...
_DISPLAY_OPTIONS: Final[tuple[Callable[[Any], Any], ...]] = (
    ...
    click.option(
        ...
        help=(
            ...
            "by the Graphviz backends, the JSON export and the editor. 'netviz layout "
...
#: reads render defaults out of ``netviz.toml``; see :mod:`netviz.settings`
...
def _layout_flags(command: _Command) -> _Command:
    ...
    them. The configuration options are here so ``netviz.toml``'s ``[render]``
...
def _filter_spec(params: Mapping[str, Any]) -> FilterSpec:
    ...
    :attr:`FilterSpec.selected` is :func:`netviz.query.apply.resolve`, called
    from the one place that has both — see :mod:`netviz.query.apply` for why
    the order is fixed.
    """
    return FilterSpec(
...
def _describe_formats() -> str:
    ...
    :data:`~netviz.render.RENDERERS` documents itself here, and cannot drift
...
# --------------------------------------------------------------------------- #
# netviz.toml: render defaults and named profiles
# --------------------------------------------------------------------------- #
...
def _apply_settings(ctx: click.Context) -> tuple[Resolution, ...]:
    ...
    Returns:
        One :class:`~netviz.settings.Resolution` per setting the command
...
def _print_settings(
    ...
    console.info(f"settings for 'netviz {command}'")
    console.info(
...
@click.option(
    ...
    help=(
        ...
        "writing anything. The plan is checked against the tree exactly as 'netviz apply' "
        "checks it."
    ),
)
@click.option(
...
def diff_command(
    ...
    **_options: Any,
    ...
    everything untouched is faded. It is 'netviz plan' as a picture: the same
    ...
    \b
    netviz -i net diff --against HEAD -f svg -o change.svg
    netviz -i net diff --from ../live --to . -f html -o review.html
    netviz -i net diff --plan drift.plan -f json | jq .changeset
    """
    app: AppContext = ctx.obj
    params = ctx.params
    ...
    if plan.empty:
        ...
        console.warn(
            ...
            f"{layer}; try another --layer, or read 'netviz plan'"
        )
    else:
...
def _planned_states(
    ...
    The plan is executed into an :class:`~netviz.edit.EditSession` that is
    never committed, so what is drawn on the right is the *same* text
    ``netviz apply`` would write, produced by the same operations. Drawing a
...
def _empty_graph_reason(layer: Layer, spec: FilterSpec) -> str:
    ...
    if layer is Layer.POWER:
        return (
            ...
            "'power.inputs', or run 'netviz list power' to see what is recorded"
        )
    ...
    # what a freshly scaffolded 'netviz init --minimal' looks like.
...
def _report_routing(
    ...
    :func:`~netviz.render.dot.routing_advisories`.
    """
...
def _unknown_element(exc: UnknownElementError) -> click.BadParameter:
    ...
    hint = (
        ...
        if exc.candidates
        else " Run 'netviz list devices' to see what is declared."
    )
...
def _report_collapse(console: Console, graph: Graph, spec: AggregateSpec) -> None:
    ...
    if not folded:
        console.warn(
            ...
            "Run 'netviz list devices' to see the fully-qualified names"
            if spec.collapse
...
def _write_output(
    ...
    and everything :mod:`netviz.export` emits is text by construction. The
...
@click.option(
    ...
    type=click.Path(dir_okay=False, writable=True, path_type=Path),
    default=None,
    help="Also write the changeset as JSON, the document 'netviz plan --json' writes.",
)
@click.option(
    "--summary-out",
    metavar="FILE",
...
@click.option(
    ...
    metavar="[LABEL=]PATH",
    help=(
        "A rendering of the visual diff to link, as 'netviz diff -o' wrote it. The label "
...
def review_command(
    ...
    composite action in .github/actions/netviz-review; see docs/ci.md.

    \b
    netviz -i net review --from origin/main
    netviz -i net review --from HEAD --diagram diff.svg -o comment.md
    netviz -i net review --from origin/main --sarif-out head.sarif --fail-on never
    """
    console = app.console()
...
@click.option(
    ...
    default=20,
    show_default=True,
    help="List at most this many commits, newest first. 'netviz log -n 1' is the last change.",
)
@click.option(
    "--max-revisions",
    "max_revisions",
    type=click.IntRange(min=1),
    default=None,
    show_default="[history] max-revisions in netviz.toml, or 100",
    help=(
...
def log_command(
    ...
    computed by the same code 'netviz plan' and 'netviz diff' use.
    ...
    \b
    netviz -i net log
    netviz -i net log --from v1.0 --to v2.0 --json
    netviz -i net log -n 1 --no-summary
    """
    console = app.console()
...
@click.option(
    ...
    help=(
        ...
        "layout netviz draws with; circo suits a ring, fdp and neato a flat mesh."
    ),
)
@click.option(
    "--write",
...
def layout_command(
    ...
    **_options: Any,
    ...
    \b
        netviz layout                     what is arranged, and what is stale
        ...
        netviz layout --prune             drop geometry for deleted elements
        netviz layout --clear             go back to laying it out from scratch

    Writes go through the same path as 'netviz edit', so comments and
    ...
    options you render with -- or put them in netviz.toml, which this command
    reads too.
    """
    app: AppContext = ctx.obj
    params = ctx.params
...
def _print_layout_report(console: Console, report: LayoutReport) -> None:
    ...
    if not report.documents:
        console.info("no layout document in this inventory; 'netviz layout --write' seeds one")
    else:
    ...
    if report.stale:
        ...
        console.warn(
            ...
            "Run 'netviz layout --prune' to drop them."
        )
...
def path_command(
    ...
    **_options: Any,
    ...
    \b
    netviz path pc-north-01 srv-south-01
    netviz path 10.1.10.51 10.2.20.11 --all
    netviz path sw-hq:Ethernet49/1 sw-hq:Ethernet50/1 --vlan 10
    netviz path rtr-hq rtr-branch-b --highlight -f svg -o path.svg
...
@click.option(
    ...
    help=(
        ...
        "Repeatable. Each end takes the spellings 'netviz path' takes."
    ),
)
@click.option(
    "--spof",
...
@click.option(
    ...
    help=(
        ...
        "'netviz/redundancy' annotation, and report E047, E048 and W141 through the "
...
def impact_command(
    ...
    \b
    netviz impact --spof
    netviz impact --fail device/sw1 --fail cable/rack1-a3
    netviz impact --fail pdu/pdu-r1-a --path pc-alice=srv-web
    netviz impact --redundancy -F github
    ...
    'netviz/redundancy' annotation are graded by E047, E048 and W141, which is
...
def test_command(
    ...
    \b
    netviz test
    netviz test connectivity segmentation
    netviz test -F junit -o test-results.xml
    ...
    the same graphs 'netviz render' draws and the same search 'netviz path'
...
@click.option(
    ...
    # (see netviz.watch.loop) and ``show_default=True`` would make --help, and
...
def watch_command(
    ...
    **_options: Any,
    ...
    Every change triggers the same load, validate and render that `netviz
...
def web_command(
    ...
    and redo -- every change through the same layer `netviz edit` writes with,
    ...
    SOURCE it opens on the example topology `netviz init` writes. Note that a
    stream has no folders and therefore no namespaces.

    Either way the diagram is the same one `netviz render` draws, and hovering
    ...
    Render defaults and --profile are read from the netviz.toml of the
...
def _web_source(console: Console, source: Path | None) -> str:
    ...
    or nothing. A pipe wins over the rest, because ``netviz render -f dot |
    ...`` taught users that netviz reads stdin when it is not a terminal, and
...
def _example_stream() -> str:
    ...
    The same documents ``netviz init`` writes, so the first thing a reader of
...
#: What ``netviz query`` can print. ``elements`` is the default because a
...
def _filter_spec_without_query(params: Mapping[str, Any]) -> FilterSpec:
    ...
    ``netviz query 'X' --kind switch`` is "among the switches, which match X" —
...
def _explain_query(console: Console, spec: FilterSpec, expression: str | None) -> None:
    ...
    Generated from :mod:`netviz.query.attributes` rather than written out, so
...
#: in :mod:`netviz.query.parser`, kept short enough to read in a terminal;
...
def lsp_command(app: AppContext, stdio: bool, watch: bool, log_path: Path | None) -> None:
    ...
    and Neovim, and ``schema/netviz.schema.json`` covers the editors that want
    ...
    """
    from netviz.lsp import Connection, serve
...
def list_command(app: AppContext, what: str, output_format: str, select: str | None) -> None:
    ...
    \b
    netviz list devices --select 'label.role = access'
    netviz list subnets --select 'namespace under sites/north'
    """
    console = app.console()
    inventory = app.load()
    _warn_about_load_errors(console, inventory)
...
#: Output formats of ``netviz ipam``. ``table`` reads; the other two pipe.
...
def ipam_command(
    ...
    followed by the conflicts :mod:`netviz.validate` finds in the address
    plan — the same findings, at the same severities, that ``netviz validate``
...
def _report_next_free(
    ...
    ``netviz ipam --next-free 10.0.0.0/8 --size 26`` is meant to be read by
    ...
    if block is None:
        console.error(
            ...
            f"run 'netviz ipam --free {prefix}' to see what is left"
        )
        raise click.exceptions.Exit(EXIT_INVALID)
        ...
        if conflicts_only:
            ...
            if report.findings:
                console.info(
                    ...
                    f"table, so run 'netviz ipam --conflicts --format csv' for them"
                )
    else:
        if not conflicts_only:
...
def _print_utilisation_table(
    ...
    The columns and the cells are :func:`netviz.listing.utilisation` — the same
...
def _resolve_labels(
    ...
    ``netviz_`` namespace — because a *static* value for one of those would
    ...
    for item in value:
        ...
        if not is_assignable_label(key):
            raise click.BadParameter(
                ...
                f"'netviz_' namespace are computed per element and would be overwritten "
...
#: conventional ones (RFC 1912 §2.2) rather than anything netviz invents;
...
_DRAWIO_OPTIONS: Final[tuple[Callable[[Any], Any], ...]] = (
    ...
    click.option(
        ...
        metavar="NAME|PATH",
        help=(
            "Apply a stylesheet to the exported diagram (§22). mxGraph spells netviz's "
    ...
    click.option(
        ...
        help=(
            ...
            "native draw.io shapes, which the reader can edit and 'netviz import drawio' "
...
def _describe_exports() -> str:
    ...
    :data:`~netviz.export.EXPORTERS` documents itself here and cannot drift
...
def export_command(
    ...
    **_options: Any,
    ...
    \b
    netviz -i net export hosts -o /etc/hosts.d/net
    netviz -i net export netplan --out build/config
    netviz -i net export netplan --name pc-desk | ssh pc-desk 'cat >/etc/netplan/10.yaml'
    """
    app: AppContext = ctx.obj
    params = ctx.params
...
#: a decision :func:`~netviz.report.layers_for` makes from the documents.
...
def show_command(
    ...
    the output is what netviz actually works with rather than what was typed.
    ...
    \b
    netviz show --select 'kind = router and has vrf'
    """
    console = app.console()
...
def _show_selection(
    ...
    *,
    ...
    ``netviz fmt`` and the loader both read back; JSON gets an array, because
...
def _as_written(inventory: Inventory, fqn: str) -> Any:
    ...
    raised, exactly as ``netviz validate`` would raise it.
...
def config_command() -> None:
    """Inspect the per-inventory netviz.toml."""
...
def config_show_command(app: AppContext, command: str, profile: str | None) -> None:
    ...
    'netviz COMMAND': every value is the profile's, the [render] table's, or
    netviz's own. Add --show-config to the command itself to see a particular
...
def version_command(as_json: bool) -> None:
    """Print the netviz, Python and Graphviz versions in use.

    'netviz --version' prints the same text. This command exists for the
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="List only the rules 'netviz validate --fix' can repair, and what each repair does.",
)
@click.pass_obj
...
def schema_command(kind: str | None, emit_all: bool, output: Path | None) -> None:
    """Print the JSON Schema (2020-12) for netviz documents.

    Point an editor at it and a typo'd key is underlined as you type it rather
    than when you next run 'netviz validate'; docs/schema.md has the
    ...
    if output is None:
        ...
        return
    output.parent.mkdir(parents=True, exist_ok=True)
    # ``write_text`` from netviz.fsio rather than Path's: a JSON Schema is
...
def completion_command(shell: str) -> None:
    ...
    \b
    bash:  netviz completion bash > ~/.local/share/bash-completion/completions/netviz
    zsh:   netviz completion zsh  > ~/.zfunc/_netviz   # with ~/.zfunc on $fpath
    fish:  netviz completion fish > ~/.config/fish/completions/netviz.fish
    ...
    inventory-aware: 'netviz show <TAB>' and '--neighbors-of <TAB>' offer the
...
def _warn_about_load_errors(console: Console, inventory: Inventory) -> None:
    ...
    if count:
        console.warn(
            ...
            f"missing from this output; run 'netviz validate' for details"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    Translates :class:`~netviz.errors.NetvizError` into a concise stderr
...
if __name__ == "__main__":  # pragma: no cover - exercised via ``python -m netviz``

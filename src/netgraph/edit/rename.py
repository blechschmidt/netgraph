...
Renaming an element is never one document either. ``netgraph edit rename A B``
...
drawn wherever the engine puts it, and ``netgraph layout --prune`` then drops
...
the same code: :func:`~netgraph.edit.cascade.placed_element` says which element
a layout key depends on, and
:func:`~netgraph.edit.cascade.annotation_references` walks a note's anchor and
...
:func:`~netgraph.edit.references.reference_text` already makes for a reference —
...
from netgraph.edit.cascade import TUNNEL_ID_PREFIX, annotation_references, placed_element
from netgraph.edit.paths import FieldPath
from netgraph.edit.references import NameIndex, reference_text

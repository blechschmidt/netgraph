...
One :class:`~netgraph.edit.operations.SetGeometry` per layout document that
...
Coordinates are netgraph's throughout: points, ``y`` upwards, a position being
...
from netgraph.layout.geometry import DEFAULT_GRID, Placement, round_coordinate
...
#: larger than this is a request to re-run ``netgraph layout``, not to nudge.
    ...
    *,
    ...
    if len(wanted) > MAX_SELECTION:
        raise EditError(
            ...
            f"(the limit is {MAX_SELECTION}); re-run 'netgraph layout --write' instead"
        )

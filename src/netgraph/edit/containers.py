...
move; it *is* ``netgraph edit move``, with the document rewritten into that
directory, ``metadata.namespace`` following the folder it now sits in, and every
reference to it re-spelled by :mod:`netgraph.edit.references`.
...
:class:`~netgraph.loader.inventory.Inventory` plus the tree's file facts, with no
access to the tree itself. Nothing here writes; the operations it returns go
through :func:`~netgraph.edit.apply.apply_operation` like any others, which is
what keeps the browser's drop and ``netgraph edit move`` the same code path
...
this would roll such a batch back anyway (:class:`~netgraph.edit.session.
...
module's.** :func:`~netgraph.edit.placement.choose_file` picks the file, so a
switch dropped into a rack gets ``r1/sw-north-acc-01.yaml`` and a cable joins
``r1/cables.yaml`` — the same answer ``netgraph edit create`` would give, which
...
from netgraph.edit.errors import EditError, PlacementError
from netgraph.edit.operations import MoveElement, Operation
from netgraph.edit.placement import FileFacts, check_file, choose_file
from netgraph.loader.inventory import Inventory, namespace_of, qualify
...
#: that would rewrite a thousand files is a ``netgraph edit`` script somebody
#: should be able to read before running, and the same bound
#: :mod:`netgraph.edit.clipboard` puts on a paste for the same reason.
MAX_MOVES: Final = 500
    ...
    *,
    ...
    Args:
        ...
        files: What each file already holds, from
            :meth:`~netgraph.edit.tree.EditableTree.facts`, so the placement
    ...
    if len(sources) > MAX_MOVES:
        raise EditError(
            ...
            f"(the limit is {MAX_MOVES}); move them with 'netgraph edit move'"
        )

...
rule that already exists somewhere else in netgraph:

**Elements** — :mod:`~netgraph.edit.references` says which references are
...
``netgraph layout --prune``, which drops every key the current *drawing* lacks
...
from netgraph.edit.paths import FieldPath
from netgraph.edit.references import NameIndex, Reference, ReferenceRole, dependents_of
from netgraph.layout.resolve import resolve_key
from netgraph.loader.inventory import Inventory, namespace_of, qualify
from netgraph.models.annotation import Annotation, Area, Note

#: Prefix of a tunnel node's id, from :mod:`netgraph.render.graph`. Spelled out
...
def plan_cascade(
    ...
    Returns:
        A plan. Computing it changes nothing; :mod:`~netgraph.edit.apply` is
...
def annotation_references(annotation: Annotation) -> Iterator[tuple[str, FieldPath]]:
    ...
    The same walk :func:`netgraph.validate._annotation_references` makes — this
...
def placed_element(key: str, *, inventory: Inventory, namespace: str) -> str:
    ...
    question for the layout engine, which is what ``netgraph layout --prune``

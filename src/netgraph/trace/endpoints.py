...
``netgraph path A B`` accepts three spellings, and which one was meant is
...
from netgraph.models import Adapter, Device, Tunnel
from netgraph.subnets import is_routable_address
from netgraph.trace.model import Endpoint, TraceError
...
def _by_address(
    ...
    *,
    ...
    inventory and mean nothing (:func:`~netgraph.subnets.is_routable_address`).
    """
    ...
    if not matches:
        hint = (
            ...
            else " Run 'netgraph list subnets' to see what the inventory is addressed in."
        )
        raise TraceError(

"""Turning command-line arguments into a :class:`~netgraph.drift.model.DriftReport`.

Four steps, in one function, so the command in :mod:`netgraph.cli` stays about
options and exit codes:

1. read the inputs (:func:`netgraph.importer.read_inputs`) — files, or ``-`` for
   standard input, exactly as ``netgraph import`` reads them;
2. parse them into a draft (:func:`netgraph.importer.build_draft`) with the
   importer's own dialect readers;
3. work out what those dialects could see (:func:`netgraph.drift.coverage_of`);
4. compare the draft with the declared tree (:func:`netgraph.drift.compare`).

Nothing here opens a socket or reads a credential, for the same reason ``import``
does not: the operator runs the collection command and hands netgraph what it
printed, so the check works from a laptop with no route to the network it is
checking, and netgraph never holds a switch password.
"""
...
def check_drift(
    ...
    *,
    ...
    Args:
        ...
        dialect: One of :data:`netgraph.importer.DIALECTS`.

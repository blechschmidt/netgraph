"""``netgraph drift``: the inventory as an assertion about the live network.

``netgraph import`` turns output collected from real devices into inventory
...
one either, though ``ip -j addr show`` does; no dialect netgraph reads prints
...
the rest of the network has been unplugged. :mod:`netgraph.drift.coverage` is
where that judgement is made and is the module to read first.

The pipeline is :mod:`~netgraph.drift.run` → :mod:`~netgraph.drift.compare` →
:mod:`~netgraph.drift.report`, over the types in :mod:`~netgraph.drift.model`.
...
from netgraph.drift.compare import CompareSpec, compare
from netgraph.drift.coverage import CAPABILITIES, Capability, Coverage, coverage_of
...
from netgraph.drift.report import FORMATS, as_json, as_junit_report, render_drift, write_text
from netgraph.drift.run import check_drift

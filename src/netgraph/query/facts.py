...
answer comes out of the :class:`~netgraph.render.graph.Graph` the caller already
has, which is what makes a query cheap enough to run on every keystroke in the
editor's search box.

The subject of the element domain is a :class:`~netgraph.render.graph.Node`, so
...
from netgraph.render.graph import Edge, Graph, NetnsView, Node, PortView, SecurityView
...
def element_values(facts: Facts, node: Node, attribute: str, qualifier: str) -> tuple[str, ...]:
    ...
    :mod:`netgraph.query.attributes`, so aliases have already been resolved;

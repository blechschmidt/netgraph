...
node per device. Written the way every other netgraph document is written —
...
parser does not care, flow style being YAML like any other, and ``netgraph fmt``
...
document goes out through PyYAML (:func:`netgraph.edit.apply._emit`), and an

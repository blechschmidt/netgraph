...
— which for netgraph means a YAML document, loaded, validated and edited the
same way everything else is.

The pieces:

:mod:`netgraph.models.layout`
...
:mod:`netgraph.layout.graphviz`
    Reading coordinates back out of Graphviz, and putting a partially-pinned
    engine run back on the stored coordinate system.
:mod:`netgraph.layout.seed`
    ``netgraph layout``: running the auto-layout once and persisting the result,
...
from netgraph.layout.graphviz import Drawing, Transform, fit_transform, parse_drawing, realign
from netgraph.layout.resolve import Conflict, conflicts_in, resolve_geometry, resolve_key

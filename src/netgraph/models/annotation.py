...
diagramming tool has them, and until now netgraph had no way to write one down.
...
**None of the three is an element.** Like :mod:`~netgraph.models.layout` they
are *sidecars*: they declare no network fact, own no interface, terminate no
cable, are never listed by ``netgraph list`` and never appear in the graph. That
...
make ``netgraph validate`` fail, cannot move a hop in ``netgraph path``, cannot
appear in the configuration ``netgraph export config`` generates, and cannot
show up as an infrastructure change in ``netgraph plan``. ``tests/test_annotations.py``
...
from netgraph.models.element import AREA_KIND, LEGEND_KIND, NOTE_KIND
from netgraph.models.layout import LAYOUT_VIEWS, Coordinate, Extent
from netgraph.models.metadata import Metadata
from netgraph.models.scalars import ApiVersion, ElementRef
...
#: So :mod:`netgraph.edit.apply` refuses a write that trips :data:`VALUE_RULE`
...
#: equal, which is what keeps ``netgraph plan`` quiet about a case change.
...
def _views(value: Any) -> Any:
    ...
    The same closed set :mod:`~netgraph.models.layout` scopes geometry by, for
...
class AnnotationGeometry(NetgraphModel):
...
class AnnotationBase(NetgraphModel):
...
class AnnotationSpecBase(NetgraphModel):
...
class NoteAnchor(NetgraphModel):
...
class AreaSelector(NetgraphModel):
...
class LegendEntry(NetgraphModel):

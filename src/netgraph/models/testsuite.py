...
nothing else, that no management address is used twice — and ``netgraph test``
grades it::
...
**A test suite is not an element.** Like :mod:`~netgraph.models.layout` and
:mod:`~netgraph.models.template` it declares no device, terminates no cable, is
never drawn as a node and is never listed by ``netgraph list``. It is indexed
...
``netgraph render --kind switch --namespace sites/north`` already parses, spelled
as one scalar because a YAML document has nowhere to put a repeated flag. It
parses to the very same :class:`~netgraph.render.graph.FilterSpec` the renderer
filters with (:mod:`netgraph.testing.selectors`), so an assertion and a diagram
...
from netgraph.models.scalars import ApiVersion, VlanId
...
class AssertionLayer(str, Enum):
    ...
    #: routed otherwise. The default, and what ``netgraph path`` does.
    ANY = "any"
...
class Assertion(NetgraphModel):
    ...
    #: or a selector matching several of them. Same spellings as ``netgraph path``.
...
class TestSuiteSpec(NetgraphModel):

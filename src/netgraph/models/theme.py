...
An element's own ``spec.style`` (:mod:`netgraph.models.style`) says how *that*
...
So it is loaded on its own, by :mod:`netgraph.render.theme`, from the path or
the bundled name ``--theme`` was given. The envelope is the netgraph one
...
Precedence is decided by :mod:`netgraph.render.theme`, not here: this module's
...
from netgraph.models.element import THEME_KIND
...
def _clause(value: Any) -> Any:
    ...
    ``netgraph.toml`` accepts for its repeatable options.
    """
    if value is None:
        return ()
...
class ThemeSelector(NetgraphModel):

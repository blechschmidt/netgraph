...
:mod:`netgraph.models.routing` does: a firewall rule is *state of a box*. It is
...
from netgraph.models.routing import AddressFamily, Fwmark, IPPrefix, RulePriority
...
#: reason :data:`~netgraph.models.routing.FWMARK_PATTERN` is.
...
class FirewallRule(NetgraphModel):
    ...
    def _check_translation(self) -> NatRule:
        ...
        if self.to_port is not None and self.type.is_source and not self.dst_ports:
            raise field_error(
                ...
                f"every packet it matches; that is a port range, not a port, and netgraph "
...
class FirewallConfig(NetgraphModel):

...
from netgraph.models import Adapter, Device, IPv4Address, IPv6Address
...
class Subnet:
    ...
    #: The prefix in ``10.0.0.0/24`` form; the identity of the subnet.
    prefix: str
    network: IPNetwork
    ...
    def sort_key(self) -> tuple[int, int, int]:
        """Order by family, then network address, then prefix length.
...
def subnets_of(inventory: Inventory) -> tuple[Subnet, ...]:
    ...
    Ordered by address family, then by network address, then by prefix length,
    so two overlapping prefixes (``10.0.0.0/16`` and ``10.0.0.0/24``) keep a
    stable relative order. Within a subnet, members follow inventory load order —
    the same order the graph lists its nodes in, so a subnet's member list and
    the nodes of a rendering never disagree about sequence.

    Returns:
        One :class:`Subnet` per distinct prefix. Loopback and link-local

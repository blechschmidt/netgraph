...
Every label is prefixed ``netgraph_``. Prometheus reserves ``__``-prefixed
...
def is_assignable_label(name: str) -> bool:
    ...
    computes per element: ``instance`` and the ``netgraph_`` namespace. A static
...
def _labels(node: Node, fqdn: str, extra: dict[str, str] | Any) -> dict[str, str]:
    ...
    Deliberately absent: the netgraph version. Every label here becomes part of

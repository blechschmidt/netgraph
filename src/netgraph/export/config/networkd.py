...
does own, and the manifest names ``netgraph export netplan`` as the dialect that
...
VRF in BGP; turning one into the other would be netgraph deciding which routes
...
from netgraph.export.config.netplan import HOST_KINDS
...
def _record_out_of_remit(plan: DevicePlan, configured: set[str], recorder: Recorder) -> None:
    ...
    if plan.device.spec.routing is not None:
        recorder.skip(
            ...
            "no adjacency. Generate it with 'netgraph export frr'",
        )
    for interface in plan.interfaces:
        subject = f"{plan.fqn}:{interface.name}"
        ...
        if wireless is not None:
            ...
            recorder.skip(
                ...
                "association, which wpa_supplicant or iwd owns. 'netgraph export netplan' "
...
def _bridge_vlan(interface: Interface) -> list[str]:
    ...
    native VLAN has no untagged domain to declare, and VLAN 1 is netgraph's
    fallback rather than the document's statement.

    Written only for a port whose ``vlan`` block is a filter
    (:func:`~netgraph.export.config.plan.restricts_vlans`). The same block on a
...
def _wireguard(plan: DevicePlan, tunnel: TunnelPlan) -> list[str]:
    ...
    section.append(f"# The private key of {plan.name}. netgraph stores no key material.")

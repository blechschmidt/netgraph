...
it is not somebody else's grammar, it is netgraph's own, and it is defined as
whatever holds one device's interface configuration completely. It therefore has
no :class:`~netgraph.export.config.model.Unsupported` list at all, and that is a
...
configuration is meant to change, and what ``netgraph drift`` reads back as a
    ...
    lines = [
        *config_header(
            ...
            notes=(
                ...
                "diff when the configuration is meant to change, and what 'netgraph drift'",
...
def _interface(plan: DevicePlan, interface: Interface) -> Iterator[str]:
    ...
    if not interface.enabled:
        ...
        # to true, so a line for it would be netgraph's default rather than the

...
device's configuration nothing else in netgraph could write — and the half where
...
The artefact is one ``etc/nftables.conf``: a single ``table inet netgraph``
...
netgraph``, preceded by a ``destroy table`` of the same name. nftables tables are
...
flushing a ruleset netgraph did not write. ``inet`` rather than ``ip`` and
...
:class:`~netgraph.export.config.model.Unsupported` rather than a rule that
...
def _record_out_of_remit(plan: DevicePlan, policy: FirewallConfig, recorder: Recorder) -> None:
    ...
    if plan.device.spec.routing_policy:
        recorder.skip(
            ...
            "iproute2's job rather than the firewall's; 'netgraph export routes' writes them, "

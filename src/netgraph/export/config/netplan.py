...
netplan is the closest thing to netgraph's own model that a Linux distribution
...
(:data:`~netgraph.export.config.model.Unsupported`), not a silent omission.

**A VRF needs a table number netgraph does not have.** netplan's ``vrfs`` section
...
from netgraph.models.interface import RadioRole, Security
...
FILENAME = "etc/netplan/10-netgraph.yaml"
...
#: netplan's ``auth.key-management`` for each security mode netgraph records.
...
def declines(plan: DevicePlan) -> str:
    ...
    return (
        ...
        f"CLI, so 'netgraph export interfaces' is what describes this device"
    )
...
def _notes(plan: DevicePlan) -> tuple[str, ...]:
    ...
    if plan.forwards_ipv4 or plan.forwards_ipv6:
        ...
        notes.append("in this file. See 'netgraph export interfaces' for the declared state.")
    return tuple(notes)
    ...
    for interface in plan.interfaces:
        ...
        if interface.type is InterfaceType.TUNNEL and interface.vlan is not None:
            recorder.skip(
                ...
                f"is lost, not a filter — see 'netgraph export interfaces'",
            )
...
def _interface(plan: DevicePlan, interface: Interface, recorder: Recorder) -> list[str] | None:
    ...
    ``None``
        ...
        a key in both cases and netgraph has nothing to put in it.
    ``[]``
...
def _wireguard(plan: DevicePlan, tunnel: TunnelPlan, recorder: Recorder) -> Iterator[str]:
    ...
    yield f'      key: "{_PLACEHOLDER}"  # private key of {plan.name}; netgraph holds no keys'
    yield "      peers:"
...
def _mtu(plan: DevicePlan, interface: Interface, recorder: Recorder) -> int | None:
    ...
    were both written down by somebody, and which was meant is not netgraph's to
    decide.
    """
    tunnel = plan.tunnel_on(interface.name)
...
def _wireless(plan: DevicePlan, interface: Interface, recorder: Recorder) -> Iterator[str]:
    ...
    in the one key netplan requires, and netgraph will not invent an SSID — so it
    ...
    if wireless is None or not wireless.bss:
        recorder.skip(
            ...
            "file without one; the inventory names no SSID on this radio, and netgraph will "
            "not invent one",
        )
        return
    yield "      access-points:"
    for bss in wireless.bss:
        ...
        if bss.security is not None:
            ...
            if bss.security is not Security.OPEN:
                yield f'            password: "{_PLACEHOLDER}"  # netgraph holds no keys'

...
Every other emitter in :mod:`netgraph.export` produces one string. A device
...
the real system paths a file belongs at (``etc/netplan/10-netgraph.yaml``), so
...
class ConfigFile:
    ...
    #: ``etc/netplan/10-netgraph.yaml``, ``etc/frr/frr.conf``. Relative rather
...
class Unsupported:
    ...
    Distinct from a :class:`~netgraph.export.manifest.Skip`, which records
...
class UnsupportedConfigError(NetgraphError):
    ...
    ``netgraph import`` already avoids for clashing files.

    Shares its exit status with :class:`~netgraph.errors.ValidationError`. From
    the operator's side both mean the same thing — the inventory says something
    netgraph will not paper over.
    """
...
class ConfigSet:
    ...
    def as_stream(self, marker: str) -> str:
        ...
        for: ``netgraph export netplan --name pc-desk`` should pipe straight

...
purpose — :mod:`netgraph.plan.diff` decides what a change is, :mod:`netgraph.plan.order`
decides what order they go in, :mod:`netgraph.plan.execute` decides how to make
...
``rename``
    One element, two names. Detected structurally (:mod:`netgraph.plan.identity`)
...
from netgraph.plan.address import Address, parse_address
from netgraph.plan.paths import MISSING, Step, format_path, parse_path
...
#: Bumped when the on-disk plan grows a field an older netgraph would misread.
...
class FieldChange:
    ...
    ``before`` and ``after`` are :data:`~netgraph.plan.paths.MISSING` when the
    ...
    #: The field, in the selector grammar of :mod:`netgraph.plan.paths`.
    path: tuple[Step, ...]
    before: Any = MISSING
    after: Any = MISSING
...
class StateRef:
    ...
    description: str
    #: Content hash of the state (:mod:`netgraph.plan.state`), when it has one.
...
class Plan:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "tool": {"name": "netgraph", "version": self.version},
...
def plan_from_dict(payload: Any) -> Plan:
    ...
    if version != PLAN_SCHEMA_VERSION:
        raise PlanFormatError(
            f"plan schema version {version!r} cannot be applied by netgraph {__version__}, "

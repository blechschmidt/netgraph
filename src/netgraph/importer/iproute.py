...
— through ``linkinfo`` — the three stacking constructs netgraph models:
...
:meth:`~netgraph.importer.draft.DraftInterface.merge`) and the argument order
does not matter.
...
no topology and carries only host-scope addresses. netgraph's ``loopback`` type
...
skipped*. netgraph models a tunnel as its own document naming both ends
...
#: ``linkinfo.info_kind`` values that map onto a netgraph interface type.
...
def _members_by_master(records: list[dict[str, Any]]) -> dict[str, list[str]]:
    ...
    on the aggregate, while netgraph declares it on the aggregate
...
def _read_link(
    ...
    *,
    ...
    if kind is not None and interface_type == "ethernet" and kind not in ("veth", ""):
        interface.comments.append(
            f"inferred: 'ip' reports this as a {kind!r} link; netgraph has no such type, "
    ...
    if interface.type in ("bridge", "lag") and not interface.members:
        draft.note(
            ...
            "netgraph requires at least one member, so it was not imported"
        )
        device.note(
...
def _interface_type(record: dict[str, Any], kind: str | None) -> str | None:
    """The netgraph ``type`` of a link record, or ``None`` when there is none."""
    if kind is not None:
...
def _note_unimportable(
    ...
    if kind in _TUNNEL_INFO_KINDS:
        draft.note(
            f"{source}: {raw!r} is a {kind} tunnel; netgraph models a tunnel as its own "
        ...
        draft.note(
            ...
            "onto no netgraph interface type, so it was not imported"
        )
...
def _apply_stacking(
    ...
    *,
    ...
    Returns ``False`` when the record is a shape netgraph cannot express, in
    ...
    if protocol is not None and protocol not in ("802.1Q", "802.1q"):
        interface.comments.append(
            f"'ip' reports this sub-interface as {protocol}; netgraph's VLAN model is "

...
* **It is symmetric, so it is self-checking.** Run on both ends of a link, LLDP
  reports the adjacency twice. :attr:`~netgraph.importer.draft.DraftCable.key`
...
LLDP says neither which interface holds it nor its prefix length, and a netgraph
...
from netgraph.importer.draft import Draft, DraftCable, DraftDevice, comment_text
from netgraph.importer.names import element_name, interface_name

__all__ = ["read_lldp"]

#: LLDP system capabilities mapped to the netgraph kind they imply, most
...
def _kind_of(chassis: dict[str, Any]) -> tuple[str | None, frozenset[str]]:
    """The netgraph kind the advertised capabilities imply, if any."""
...
def _note_rename(comments: list[str], what: str, original: str) -> None:
    if not any(comment.startswith(f"the {what} is named ") for comment in comments):
        comments.append(
            ...
            "because a netgraph name may only hold letters, digits and separators"
        )

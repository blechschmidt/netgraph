...
The mirror of :mod:`netgraph.export.config.nftables`, and the one reader here
that imports *nothing* into the draft on purpose.

**A ruleset says what a box refuses, and a draft has nowhere to put that.** A
:class:`~netgraph.importer.draft.Draft` is the vocabulary
:mod:`netgraph.importer.emit` can write: elements, interfaces, addresses, VLANs,
...
same thing :mod:`netgraph.importer.config.neutral` does for a ``route`` or a
``policy`` stanza, and for the same reason: a note naming what was found is the
difference between "netgraph does not import this" and "netgraph read your
...
So :mod:`netgraph.drift.coverage` grants this dialect nothing at all, exactly as
...
documents it came from — because that is what makes ``netgraph drift`` able to
...
#: the emitter's (:func:`netgraph.export.config.nftables._set_name`), and a set

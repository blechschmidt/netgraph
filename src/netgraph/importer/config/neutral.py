"""``--from interfaces``: netgraph's own configuration grammar, read back.

The mirror of :mod:`netgraph.export.config.neutral`, and the only pairing here
where both halves are netgraph's: the emitter's docstring *defines* the grammar
this parses, and the round trip through the two is meant to be exact for
everything a :class:`~netgraph.importer.draft.Draft` can hold. Which is the
...
device document said so, so :meth:`~netgraph.importer.draft.DraftDevice.refine_kind`
is given no comment: the ``inferred:`` note a fresh draft device carries is for a
kind netgraph reasoned its way to, and this one was read. A kind outside
:data:`~netgraph.models.DEVICE_KINDS` is reported and ignored rather than written
...
from a file netgraph itself wrote would be reporting on its own output.

**A trunk's VLAN set is expanded here.** ``vlan-tagged 10,20,100-110`` is what
:func:`~netgraph.errors.compact_ids` writes, and this reads it back: ids and
inclusive ``low-high`` ranges, separated by commas, bounded to
``1``-``4094``. :class:`~netgraph.models.scalars.VlanSet` accepts the same
...
from netgraph.importer.config.common import fold_into, read_int, read_vlan_id, stanzas
...
from netgraph.models import DEVICE_KINDS, InterfaceType, VlanMode
...
def _read_vlan_stanza(state: _Pass, words: list[str], body: list[str]) -> None:
    ...
    if body:
        state.note(
            "a VLAN's name and description have no home in the draft 'netgraph import' "
...
def _read_type(state: _Pass, interface: DraftInterface, value: str) -> None:
    if value in _INTERFACE_TYPES:
        ...
        return
    state.note(
        f"{interface.name}: {comment_text(value)!r} is not a netgraph interface type, so the "
...
def _note_stated(state: _Pass) -> None:
    ...
    if state.routes:
        state.note(
            ...
            f"[{comment_text('; '.join(state.routes))}]; 'netgraph import' builds devices, "
    ...
    if state.tunnels:
        state.note(
            ...
            f"[{comment_text('; '.join(state.tunnels))}]; netgraph models a tunnel as its own "
...
def _note_attribute(state: _Pass, attribute: str) -> None:
    ...
    if stated is None:
        state.note(
            f"{attribute!r} is not an attribute of the 'interfaces' grammar netgraph writes, "
            "so it was not imported"
        )
        return
    state.note(
        f"{family!r} states {stated}; the draft 'netgraph import' builds has no field for it, "
...
def _vlan_ids(state: _Pass, value: str) -> list[int]:
    ...
    The inverse of :func:`~netgraph.errors.compact_ids`, which is what wrote the

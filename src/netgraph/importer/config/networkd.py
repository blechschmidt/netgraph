...
states enslavement on the *member* (``[Network] Bridge=br0``) where netgraph
states membership on the aggregate, exactly as ``ip`` does with ``master`` and
:func:`netgraph.importer.iproute` inverts it. It also states a VLAN's and a
...
call :mod:`netgraph.importer.iproute` makes for the same reason.
...
:mod:`netgraph.importer.iproute`; where the parent already has a
...
interface, and :meth:`~netgraph.importer.draft.Draft.note` keeps only the first
...
property or by glob rather than by one name, a ``Kind=`` netgraph has no
...
from netgraph.importer.config.common import fold_into, read_int, read_vlan_id
...
#: The netgraph interface type of each ``[NetDev] Kind=``. The four tunnel kinds
#: are the ones :mod:`netgraph.export.config.networkd` writes; everything else a
...
_DROPPED_SECTIONS: Final[dict[str, str]] = {
    "Route": (
        "a networkd '[Route]' section says where traffic leaves by; 'netgraph import' writes "
        "interfaces and no 'spec.routes', so none was imported"
    ),
    "WireGuard": (
        "a '[WireGuard]' section holds a listening port and a private key; netgraph models a "
        "tunnel as its own document naming both ends and stores no key material "
        "(docs/schema.md §14.2), so it was not imported"
    ),
    "WireGuardPeer": (
        "a '[WireGuardPeer]' section names a far end by its public key; netgraph names both "
        "ends of a tunnel by element and interface (docs/schema.md §14), which a key does not "
        "give, so it was not imported"
    ),
    "Tunnel": (
        "a '[Tunnel]' section describes an encapsulation and its endpoints; netgraph models a "
...
_DROPPED_KEYS: Final[tuple[tuple[str, tuple[str, ...], str], ...]] = (
    (
        "Network",
        ("DNS", "Domains", "NTP"),
        "networkd 'DNS='/'Domains='/'NTP=' configure host name resolution and time; netgraph "
...
def _unit_type(unit: _Unit, raw: str, stream: _Stream) -> str | None:
    """The netgraph type of a unit, or ``None`` when netgraph has none for it.
    ...
    :meth:`~netgraph.importer.draft.DraftInterface.merge`.
    """
    ...
    if interface_type is None:
        stream.note(
            f"{raw!r} is a {comment_text(kind)!r} netdev, which maps onto no netgraph "
    ...
    if stated is not None:
        ...
        if mtu is not None:
            ...
            stream.note(
                ...
                "count; netgraph records an integer MTU, so it was not imported"
            )
...
def _bridge_vlan(unit: _Unit, stream: _Stream) -> DraftVlan | None:
    ...
    return DraftVlan(
        ...
        comment=(
            ...
            "one; netgraph's import writes the tagged set only, so state the native VLAN by "
...
def _collect_stacking(unit: _Unit, name: str, stream: _Stream) -> None:
    ...
    netgraph: ``Bridge=``/``Bond=`` name the aggregate *this* link is a member
    ...
    for child, lower in stream.parents.items():
        ...
        if interface is None:
            ...
            stream.note(
                ...
                f"{interface.type} interface, which netgraph does not stack on a parent, so "
...
def _add_interfaces(stream: _Stream, device: DraftDevice) -> None:
    ...
    The two checks are the two shapes networkd can state and netgraph cannot
    ...
    for interface in stream.interfaces.values():
        if interface.type in ("bridge", "lag") and not interface.members:
            stream.note(
                ...
                "enslaves an interface into; netgraph requires at least one member "
    ...
    for aggregate, members in stream.members.items():
        ...
        if existing is None:
            ...
            # because an operator may hand netgraph one unit file per argument

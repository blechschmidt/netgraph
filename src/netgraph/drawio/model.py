...
mxGraph cell reduced to the fields netgraph writes or reads; :class:`Diagram` is
a whole ``<mxGraphModel>`` as an ordered list of them plus the metadata cell.
The XML is :mod:`netgraph.drawio.mxfile`'s problem, and the inventory is
:mod:`netgraph.drawio.build`'s and :mod:`netgraph.drawio.reconcile`'s.
...
=================  ==========================  =========================
                   netgraph (§18, Graphviz)    draw.io (mxGraph)
=================  ==========================  =========================
``y``              upwards                     downwards
...
exported file* (``netgraph:originX``/``originY`` on the metadata cell) rather
...
from netgraph.layout.geometry import round_coordinate
...
#: :class:`~netgraph.layout.geometry.Placement` carries a size only when one was
#: stored (§18), and "as big as the label needs" has no meaning in a file that
#: is going to be opened somewhere netgraph is not.
...
class Frame:
    """The map between netgraph's coordinates and draw.io's.

    ``origin_x`` is the netgraph ``x`` that lands on draw.io's ``x = 0``, and
    ``origin_y`` the netgraph ``y`` that lands on draw.io's ``y = 0``. Both
    ...
    def to_drawio(self, x: float, y: float) -> tuple[float, float]:
        """A netgraph point as a draw.io page point."""
        return (round_coordinate(x - self.origin_x), round_coordinate(self.origin_y - y))

    def to_netgraph(self, x: float, y: float) -> tuple[float, float]:
        """A draw.io page point back in netgraph's system."""
        return (round_coordinate(x + self.origin_x), round_coordinate(self.origin_y - y))
    ...
    def box_to_netgraph(
        ...
        """The centre, in netgraph, of a draw.io box with that top-left corner."""
        return self.to_netgraph(x + width / 2, y + height / 2)
...
class Cell:
    ...
    page coordinates. ``attributes`` holds the ``netgraph:*`` custom attributes
    with the prefix stripped, so a reader asks for ``kind`` rather than for
    ``netgraph:kind``.
    """

    id: str
    #: ``None`` for a cell netgraph did not write — every cell of a hand-made
    #: diagram, and anything a draw.io user added to one of netgraph's.
    ...
    def attribute(self, name: str, default: str = "") -> str:
        """One ``netgraph:*`` attribute, or ``default`` when it is absent."""
        return self.attributes.get(name, default)

    @property
    def is_netgraph(self) -> bool:
        """Did netgraph write this cell?"""
...
class Diagram:
    ...
    #: Which netgraph view this draws, as :class:`~netgraph.render.graph.Layer`
    #: spells it. Empty for a diagram netgraph did not write.
    view: str = ""
    ...
    def of_role(self, role: CellRole) -> tuple[Cell, ...]:
        """Every netgraph cell of one role, in file order."""
        return tuple(cell for cell in self.cells if cell.role is role)

    @property
    def foreign(self) -> tuple[Cell, ...]:
        """Every cell netgraph did not write, in file order."""
        return tuple(cell for cell in self.cells if not cell.is_netgraph)

    @property
    def is_netgraph(self) -> bool:
        """Was this file produced by ``netgraph export drawio``?
        ...
        of one of netgraph's has the attributes and none of the guarantees.
        """
...
METADATA_ID: Final = "netgraph-metadata"
...
def _plain(value: float) -> str:
    ...
    Twelve significant digits; see :func:`netgraph.drawio.identity._number` for

...
bends into a line, and :mod:`netgraph.layout.avoid` knows where the bends have
...
* ``netgraph layout --route`` writes it into the inventory as waypoints, at
...
chose; ``netgraph layout --route`` is the command that makes one permanent, and
...
from netgraph.layout.avoid import DEFAULT_BUDGET, Budget, Detour, Obstacle, Router
...
class _Cached:
    ...
    #: it. A :class:`~netgraph.layout.routing.Route` is immutable, so sharing one
...
def _settled(
    ...
    *,
    ...
    clean link costs a :func:`~netgraph.layout.routing.route` and a scan of the

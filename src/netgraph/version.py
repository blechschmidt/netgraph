"""What ``netgraph --version`` reports, and the machine-readable form of it.
...
* **netgraph** itself, from the installed distribution metadata.
* **Python** — version, implementation and the interpreter path, because a
  ``netgraph`` on ``PATH`` frequently belongs to an environment the reporter did
  not mean to be in.
* **Graphviz** — the ``dot`` that :func:`netgraph.render.dot.find_dot` would
...
* **The YAML parser actually selected** — libyaml or pure Python. This is the one
  ...
  loader behaviour (see :mod:`netgraph.loader.documents`).
...
#: The distributions worth naming in a bug report: netgraph's own runtime
...
class VersionReport:
    ...
    #: The installed netgraph distribution, or ``0.0.0.dev0`` from a source tree
    ...
    graphviz: str | None
    #: The executable :func:`~netgraph.render.dot.find_dot` chose, if any.
    ...
    #: ``libyaml`` or ``python`` -- which parser :mod:`netgraph.loader.documents`
    #: actually selected, honouring ``NETGRAPH_YAML_LOADER``.
    yaml_parser: str
...
def _yaml_parser() -> str:
    """Which loader ``netgraph.loader.documents`` bound, as one word.
...
def collect() -> VersionReport:
    ...
    # Local import: ``netgraph.render.dot`` pulls in Jinja2 and the template
...
def format_text(report: VersionReport) -> str:
    ...
    The first line is ``netgraph <version>`` and nothing else, so that the
    thousand scripts which do ``netgraph --version | cut -d' ' -f2`` keep
    working across this change.
    """
    dependencies = ", ".join(f"{name} {value}" for name, value in report.dependencies.items())
    lines = [
        f"netgraph {report.netgraph}",

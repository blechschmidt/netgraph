...
This is the module the whole command exists for. ``netgraph drift`` produces a
:class:`~netgraph.drift.model.Change` per disagreement; this turns each one into
what would close it, in the vocabulary of :mod:`netgraph.converge.intent`.
...
:class:`~netgraph.converge.model.Provenance` of the finding it came from. There
...
command. Only interfaces netgraph could have *created* -- VLAN sub-interfaces,
...
a rack. It stays in the plan as a :attr:`~netgraph.converge.intent.IntentKind.
...
from netgraph.converge.intent import Intent, IntentKind, article, order_intents
from netgraph.converge.model import Provenance
from netgraph.drift.model import Change, Direction, DriftReport
...
def observed_devices(inventory: Inventory, draft: Draft) -> Mapping[str, DraftDevice]:
    ...
    Resolution is :meth:`~netgraph.loader.inventory.Inventory.lookup`, exactly as
    :mod:`netgraph.drift.compare` does it, so a device that drift reported under
...
def derive(inventory: Inventory, draft: Draft, report: DriftReport) -> tuple[Intent, ...]:
    ...
    Returns:
        Intents sorted by :func:`~netgraph.converge.intent.order_intents`.
        Prerequisites are *not* filled in here: they are per element and the
        builder groups by element, so
        :func:`~netgraph.converge.intent.prerequisites_of` is applied there.
    """
    ...
    if kind not in _VIRTUAL_TYPES:
        yield _manual(
            ...
            note=(
                ...
                "inventory does not declare. netgraph will not shut or remove a physical port it "
...
def _vlan_prerequisites(
    ...
    not netgraph's to invent, and a plan that made one up would be configuring a
...
def _manual(change: Change, origin: Provenance, *, note: str = "", summary: str = "") -> Intent:
    """A finding netgraph will not write a command for."""
    return Intent(
...
def _manual_summary(change: Change) -> str:
    ...
    Short, and about the *thing* rather than about netgraph: an operator
    scanning a plan wants to see "move a cable" and "declare a device", not six
    variations on "netgraph cannot do this".
    """
...
def _manual_note(change: Change) -> str:
    ...
    if change.scope == "device":
        if change.direction is Direction.UNDECLARED:
            return (
                ...
                "declared state to converge it on. Run 'netgraph import' to adopt it, or take "
                "it off the network"
            )
        return (
    ...
    if change.field == "type":
        return (
            ...
            "type is a re-creation, not an edit, so netgraph leaves it to a person who can "
            "see what else is on it"
        )
    return "no dialect netgraph writes has a command for this difference"

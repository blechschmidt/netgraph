...
* :mod:`netgraph.validate` — is an outlet claimed twice, does a PDU's load fit
  inside its capacity, does a PoE-powered device have a PoE uplink;
* :mod:`netgraph.render.graph` — the ``power`` layer, whose nodes are the PDUs
  and the powered elements and whose edges are the feeds;
* :mod:`netgraph.cli` — ``netgraph list power``, the per-PDU utilisation table;
* :mod:`netgraph.export.power` — the load schedule.

So the resolution happens exactly once, here, the same way
:mod:`netgraph.subnets` resolves addressing once for its three consumers. The
module is a pure function of an :class:`~netgraph.loader.inventory.Inventory`:
...
class Feed:
    ...
    #: :attr:`~netgraph.models.pdu.PduSpec.input_feed` of the PDU. Empty for a
...
def format_utilisation_percent(value: float | None) -> str:
    ...
    :func:`netgraph.ipam.format_utilisation`, so the two utilisation tables read

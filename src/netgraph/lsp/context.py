...
analysed: it is legal, ``netgraph fmt`` writes it for short scalar sequences,
...
def _classify(
    *,
    ...
    One ambiguity is resolved against netgraph rather than against YAML. Inside
    ...
    sequences netgraph's schema fills with mappings are written ``- name: eth0``

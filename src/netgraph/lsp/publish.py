"""``netgraph validate``'s findings, in the shape ``textDocument/publishDiagnostics`` takes.

Nothing is re-derived here. The rules, the severities, the messages and the
grading a ``netgraph.toml`` asks for all come from
:func:`~netgraph.diagnostics.build_report`, which is what the CLI, the SARIF
...
* **A range.** The report carries the line and column of the offending value;
  ...
  and stops at it (:mod:`netgraph.lsp.locate`).
...
from netgraph.lsp.text import Encoding, Range
...
DIAGNOSTIC_SOURCE: Final = "netgraph"

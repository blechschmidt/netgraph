...
``netgraph schema`` already emits the whole contract — every key of every kind,
...
``allOf`` with ``if``/``then``
    ...
    left to ``netgraph validate``, which states them far better than a
...
class SchemaIndex:
    ...
    def api_version(self) -> str:
        ...
        return str(version) if version is not None else "netgraph.dev/v1alpha1"

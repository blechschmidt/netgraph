 ...
 * through netgraph.edit exactly as `netgraph edit create` does -- a tour that
 ...
 * second, writable session over the copy (netgraph/web/tour.py), and while the
...
var netgraphTour = (function () {
  ...
  function steps() {
    ...
    return [
      ...
      },
      {
        ...
        body: function () {
          return "One batch through the same write path <code>netgraph edit create</code> "
            + "uses. Watch the file list on the left: a document has to be written "
            + "for the shape to exist, and netgraph chooses where by looking at where "
      ...
      },
      {
        ...
        body: function () {
          return "The picture and the files are one thing, in both directions: edit "
            ...
            + "palette does is a <code>netgraph edit</code> command you can run from a "
  ...
  function apply(list, said) {
    return netgraphSession.ops(list, said).then(function (result) {
      batches += 1;
      return result;
    });
  }
  ...
  function undoAll() {
    ...
    return netgraphSession.step("undo").then(function (moved) {
  ...
  function paint() {
    ...
    highlight(step.spot);
    netgraphA11y.announce(step.title + ". " + panel.querySelector("#tour-body").textContent, false);
  }
    ...
    Promise.resolve()
      ...
      .then(function () {
        ...
        if (step.after) {
          ...
          netgraphA11y.announce(step.after(), false);
        }
      })
  ...
  function finish() {
    ...
    write(null);
    highlight(null);
    if (entry) { netgraphKeys.closeOverlay(entry); entry = null; }
  ...
  function invite() {
    ...
    var shut = function () {
      markSeen();
      netgraphKeys.closeOverlay(handle);
    };
    ...
    handle = netgraphKeys.overlay(node, {
  ...
  function resume() {
    ...
    index = 0;
    batches = 0;
    entry = netgraphKeys.overlay(panel, {
  ...
  function defineCommands(bridge) {
    host = bridge;
    netgraphKeys.define("tour", {
      run: function () {
        if (running) { netgraphA11y.announce("the tour is already running", false); return; }
        start();
      }
    });
  }

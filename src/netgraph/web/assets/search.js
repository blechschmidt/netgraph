 ...
 * (`netgraph.query`), exactly the string `netgraph query` answers, exactly the
 ...
 *   `FilterSpec` `netgraph render --select` narrows one with. That is a
 ...
 */
window.netgraphSearch = (function () {
  "use strict";
  ...
  function matchedIds() {
    ...
    answer.addresses.forEach(function (address) {
      window.netgraphSelect.shapesOf(address).forEach(function (id) { found[id] = true; });
    });
    return found;
  }
   ...
   */
  function shapes() {
    return window.netgraphCull.ids();
  }
  ...
  function chooseMatches() {
    ...
    var addresses = (answer.addresses || []).filter(function (address) {
      ...
      return window.netgraphSelect.shapesOf(address).length > 0;
    });
    if (!addresses.length) {
    ...
    window.netgraphSelect.set(addresses);
  ...
  function defineCommands() {
    ...
    K.define("search.select", {
      run: function () {
        ...
        if (count) {
          window.netgraphA11y.announce(count + " selected from the query", false);
        }
      },
      enabled: function () {
    ...
    K.define("search.clear", {
      run: function () {
        if (!clear()) { window.netgraphA11y.announce("the search box is already empty", false); }
      },

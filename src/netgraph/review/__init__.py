...
:mod:`netgraph.review.model`
    The data. A :class:`~netgraph.review.model.Review` is a changeset
    (:class:`~netgraph.plan.model.Plan`), the head inventory's diagnostics, the
    base inventory's diagnostics, and enough context to link a finding to a line
    on GitHub. :func:`~netgraph.review.model.compute_delta` subtracts one side
    from the other using :func:`netgraph.diagnostics.fingerprint`, which is the
    same identity code scanning tracks alerts by — so the bot and the alert list
    cannot disagree about which problem is new.

:mod:`netgraph.review.comment`
    The formatting. :func:`~netgraph.review.comment.render_comment` is a pure
    function from a :class:`~netgraph.review.model.Review` to the Markdown body
...
from netgraph.review.comment import MARKER_PREFIX, marker, render_comment
from netgraph.review.model import (
...
from netgraph.review.run import build_review

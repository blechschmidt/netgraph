...
``netgraph render`` narrows a diagram with repeated flags — ``--kind switch
...
:class:`~netgraph.render.graph.FilterSpec` the renderer filters with. That is
...
def parse_selector(text: str) -> FilterSpec:
    ...
    Returns:
        The :class:`~netgraph.render.graph.FilterSpec` it denotes.

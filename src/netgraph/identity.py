...
* ``netgraph list users`` and ``netgraph list groups``,
...
That is the same arrangement :mod:`netgraph.power` has for outlet feeds, for the
same reason.
...
from netgraph.models import GROUP_KIND, USER_KIND, Group, User
...
def identity_plan(inventory: Inventory) -> IdentityPlan:
    ...
    (:meth:`~netgraph.loader.Inventory.lookup`): outwards from the group's own

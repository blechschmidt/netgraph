...
(:mod:`netgraph.impact.engine`) and everything that consumes one — the text
...
class ImpactError(NetgraphError):
    ...
    #: 7 is taken by :class:`~netgraph.trace.model.TraceError` and 6 by
    #: :class:`~netgraph.httpserve.ServeError`; the CLI turns this one into a
...
class Spof:
    ...
    layer: str
    #: ``node`` or ``link`` (:mod:`netgraph.connectivity`).
    kind: str
    ...
    #: the report actually prints — see :func:`netgraph.impact.engine.single_points`.
    isolates: tuple[str, ...] = ()
...
@dataclass(frozen=True, slots=True)
class ImpactReport:
    """Everything one ``netgraph impact`` run found."""
    ...
    anchors: tuple[str, ...] = ()
    anchor_source: str = ""
    #: One per requested layer, in :data:`~netgraph.impact.graphs.LAYERS` order.
    ...
    def as_dict(self, *, version: str) -> dict[str, Any]:
        ...
        return {
            "schemaVersion": 1,
            "tool": {"name": "netgraph", "version": version},
...
def _finding_record(finding: Finding) -> Mapping[str, Any]:
    ...
    Deliberately a subset of :meth:`netgraph.diagnostics.Diagnostic.as_record`
    rather than a shape of its own: a consumer that already reads netgraph

...
under ``netgraph/report/templates``, and ``--template DIR`` puts a directory in
...
:mod:`netgraph.report.model` and nothing else — because the moment a template
...
:mod:`netgraph.report.stamp` explains how to pin it).

**Self-contained HTML.** Each page inlines the one style sheet
(``assets/report.css``) through the same
:func:`~netgraph.render.html.asset_text` the interactive renderer uses, and
carries the matching :func:`~netgraph.render.html.policy` in a ``<meta>``: no
...
from netgraph.render.html import asset_text, policy
from netgraph.report.layout import Layout
from netgraph.report.model import Cell, Column, Page, Report
...
def stylesheet() -> str:
    ...
    return asset_text(_STYLES, package="netgraph.report")
...
def render_pages(
    ...
    Raises:
        NetgraphError: A template is missing or does not render.
    """
    ...
    for page in report.pages:
        ...
        try:
            ...
            raise NetgraphError(f"cannot load the report template {name!r}: {exc}") from exc
        try:
            ...
            raise NetgraphError(f"the report template {name!r} failed: {exc}") from exc
...
def _json_bytes(report: Report) -> bytes:
    ...
    :meth:`~netgraph.report.model.Report.to_json` writes, which is the order a
...
def markdown_link(value: Cell, link: Callable[..., str]) -> str:
    """One :class:`~netgraph.report.model.Cell` as Markdown, linked if it can be.
...
def _environment(templates: Path | None) -> Environment:
    ...
    :mod:`netgraph.render.fragment` has sanitised it.
    """
    loaders: list[FileSystemLoader | PackageLoader] = [
        PackageLoader("netgraph.report", "templates")
    ]
    if templates is not None:
        if not templates.is_dir():
            raise NetgraphError(f"--template {templates} is not a directory")

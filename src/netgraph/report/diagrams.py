...
Nothing here lays a graph out: :func:`~netgraph.render.dot.to_image` does, which
is the same call ``netgraph render -f svg`` makes, so a diagram in a report is the
...
drawing through :mod:`netgraph.render.fragment`, because a page that fetches
...
:class:`~netgraph.render.links.LinkMap` rather than derived from a template.

**What to say when there is no drawing.** Graphviz is optional in netgraph, and
...
class Diagrams:
    ...
    :func:`netgraph.report.generate` merges it into the bundle at the end.
    """
    ...
    def draw(self, graph: Graph, *, scope: str, page: str) -> Diagram:
        """One layer of one scope, as a :class:`~netgraph.report.model.Diagram`.
        ...
        if self.inline:
            ...
            # netgraph.render.fragment. Everything else the templates print is

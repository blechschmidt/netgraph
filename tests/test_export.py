"""``netviz export`` — the artefacts, their escaping, and their manifests.
...
:class:`~netviz.render.graph.FilterSpec` a render uses, so an export scoped to
...
from netviz.export.manifest import Manifest, Reason, Recorder
from netviz.export.names import (
    MAX_DNS_LABEL,
    ansible_identifier,
    csv_cell,
    dns_labels,
...
def run_export(
    ...
    *,
    ...
    The same sequence :func:`netviz.cli.export_command` runs, minus the
...
#: The version netviz stamps into a provenance banner, wherever it appears.
_VERSION = re.compile(re.escape(GENERATOR))
_VERSION_PLACEHOLDER = "netviz <version>"


def normalise(payload: str) -> str:
    """``payload`` with the generator's version replaced by a placeholder.

    The version is genuinely useful *in* an artefact — "which netviz wrote
...
def test_hosts_carries_a_generated_by_header(inventories: dict[str, Inventory]) -> None:
    payload = run_export(inventories["home-lab"], "hosts").payload
    assert payload.startswith("# Generated by 'netviz export hosts'")
    assert "netviz " in payload.splitlines()[1]
...
def test_host_variables_carry_the_interfaces_and_vlans_a_template_needs(
    ...
    names = [interface["name"] for interface in switch["netviz_interfaces"]]
    assert names, "a switch with no interfaces reached the inventory"
    assert switch["netviz_vlan_ids"] == sorted(switch["netviz_vlan_ids"])
    assert any("vlans" in interface for interface in switch["netviz_interfaces"])
    assert [entry["id"] for entry in switch["netviz_vlans"]] == sorted(
        entry["id"] for entry in switch["netviz_vlans"]
    )
...
def test_interfaces_keep_declaration_order(inventories: dict[str, Inventory]) -> None:
    ...
    document = json.loads(run_export(inventories["home-lab"], "ansible-inventory").payload)
    interfaces = document["_meta"]["hostvars"]["pc-desk.hosts"]["netviz_interfaces"]
...
def test_prometheus_labels_carry_namespace_kind_vendor_and_site(
    ...
    assert labels["netviz_namespace"] == "network"
    assert labels["netviz_kind"] == "switch"
    assert labels["netviz_site"] == "hq"
    assert labels["netviz_rack"] == "r1"
...
def test_the_netviz_version_is_not_a_prometheus_label(
    inventories: dict[str, Inventory],
) -> None:
...
def test_hostile_free_text_round_trips_through_the_ansible_json(hostile: Inventory) -> None:
    ...
    assert '"quoted"' in host["netviz_description"]
    assert "\n" in host["netviz_description"]
    assert host["netviz_vendor"].startswith("Ünicorn")
...
def test_hostile_free_text_round_trips_through_the_prometheus_json(hostile: Inventory) -> None:
    ...
    sites = {group["labels"]["netviz_site"] for group in groups}
...
def test_the_manifest_is_a_versioned_document() -> None:
    ...
    assert record["apiVersion"] == "netviz.dev/v1alpha1"
...
def test_the_cli_writes_the_artefact_to_stdout_and_the_manifest_to_stderr(
    ...
    """``netviz export ... | consumer`` must not be polluted by commentary."""
...
def test_completion_offers_every_format() -> None:
    from netviz.completion import complete_export_format
...
def test_the_registry_reports_the_conventional_suffix_of_each_format() -> None:
    from netviz.export import suffix_for
...
@pytest.mark.parametrize("name", ["instance", "netviz_kind", "netviz_anything"])
...
def test_one_address_on_two_interfaces_is_published_once(tmp_path: Path) -> None:
    ...
    addresses = document["_meta"]["hostvars"]["dup"]["netviz_addresses"]

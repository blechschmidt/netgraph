...
|---|---|---|---|
| `e001-unknown-endpoint.yaml` | `E001` | `NG-C002` | A cable names `pc-ghost`, which is not declared. |
| `e002-double-termination.yaml` | `E002` | `NG-C005` | Two cables land on `pc-a:eth0`. |
| `e003-duplicate-mac.yaml` | `E003` | `NG-I008` | Two unrelated ports share `00:1e:8c:00:00:01`. |
| `e004-duplicate-ip.yaml` | `E004` | `NG-A004` | `10.0.0.1/24` is claimed by two untagged ports. |
| `e005-vlan-mismatch.yaml` | `E005` | `NG-C011` | Access VLAN 10 is cabled to access VLAN 20. |
| `e006-adapter-capacity.yaml` | `E006` | `NG-X008` | `ports: 1`, two downstream interfaces. |
| `w101-unaddressed-interface.yaml` | `W101` | `NG-I013` | `pc-a:eth1` has no address and no `vlan` block. |
| `w102-mtu-mismatch.yaml` | `W102` | `NG-C010` | MTU 1500 cabled to MTU 9000. |
| `w103-orphan-device.yaml` | `W103` | `NG-C016` | A lone device, no cables at all. |
| `w104-ip-on-access-port.yaml` | `W104` | `NG-V009` | A layer-2 switch holds an IP on a bridge port. |
| `w105-lonely-subnet.yaml` | `W105` | `NG-A008` | `pc-a:eth1` is the only interface in `192.168.99.0/24`. |
| `w106-subnet-address-clash.yaml` | `W106` | `NG-A009` | `10.0.0.1/24` is claimed by two hosts, in VLAN 10 and VLAN 20. |
| `e007-stacking-cycle.yaml` | `E007` | `NG-I004` | Two `vlan` interfaces are each other's `parent`. |
| `e008-doubly-aggregated-member.yaml` | `E008` | `NG-I005` | `eth1` is a member of both `bond0` and `bond1`. |
| `e009-subinterface-vlan.yaml` | `E009` | `NG-V005` | `eth0.20` needs VLAN 20; `eth0` trunks only VLAN 10. |
| `e010-multicast-mac.yaml` | `E010` | `NG-I009` | `01:00:5e:00:00:01` has the multicast bit set. |
| `w107-addresses-on-lag-member.yaml` | `W107` | `NG-I006` | The address sits on `eth0` rather than on `bond0`. |
| `w108-mac-on-loopback.yaml` | `W108` | `NG-I007` | `lo` declares a `mac`. |
| `w109-no-cableable-interface.yaml` | `W109` | `NG-I012` | `pc-a` owns nothing but a loopback. |
| `w110-reserved-address.yaml` | `W110` | `NG-A005` | `10.0.0.0/24` is the prefix's own network address. |
| `w111-overlapping-prefixes.yaml` | `W111` | `NG-A006` | `eth0` and `eth1` are both in `10.0.0.0/24`. |
| `w112-loopback-prefix.yaml` | `W112` | `NG-A007` | A routed loopback carries `192.0.2.1/30`, not a `/32`. |
| `w113-undeclared-vlan.yaml` | `W113` | `NG-V004` | An access port is in VLAN 20; `vlans` declares only 10. |
| `w114-native-vlan-not-trunked.yaml` | `W114` | `NG-V006` | `native_vlan: 20` with `trunk_vlans: [10]`. |
| `w115-trunk-all-to-host.yaml` | `W115` | `NG-V007` | `trunk_vlans: all` is cabled to a workstation. |
| `w116-lag-member-vlan.yaml` | `W116` | `NG-V008` | `eth0` says VLAN 20, `bond0` says VLAN 10. |
| `i001-locally-administered-mac.yaml` | `I001` | `NG-I010` | `02:00:00:00:00:01` is not a vendor-assigned address. |
| `e011-wireless-medium.yaml` | `E011` | `NG-C006` | A copper cable ends on a `type: wifi` port. |
| `e012-uncableable-endpoint.yaml` | `E012` | `NG-C009` | The cable lands on `pc-a:lo`, a loopback. |
| `e013-upstream-cabled-and-attached.yaml` | `E013` | `NG-X005` | `attached_to: laptop` *and* a cable to `laptop:eth0`. |
| `e014-attachment-cycle.yaml` | `E014` | `NG-X006` | `adp-a` and `adp-b` are attached to each other. |
| `e015-unknown-attachment.yaml` | `E015` | `NG-X001` | `attached_to: ghost-host`, which is not declared. |
| `w117-self-link.yaml` | `W117` | `NG-C004` | `cbl-loop` joins `sw-a:port1` to `sw-a:port2`. |
| `w118-speed-mismatch.yaml` | `W118` | `NG-C008` | A 5Gbps upstream port on a 1Gbps cable. |
| `w119-lag-endpoint.yaml` | `W119` | `NG-C012` | The cable ends on `bond0`, not on a member. |
| `w120-half-duplex.yaml` | `W120` | `NG-C013` | `duplex: half` between two computers. |
| `w121-disconnected-topology.yaml` | `W121` | `NG-C014` | Two cabled pairs that never meet. |
| `w122-hub-subnets.yaml` | `W122` | `NG-H005` | Two hosts on one hub, in `10.0.0.0/30` and `10.0.1.0/30`. |
...
directory as a single inventory would collide on `NG-N002` and drown the rule

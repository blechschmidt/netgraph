...
Every change was made with `netgraph edit`, so the files are exactly what the
...
`cbl-sw-nas`, and `netgraph diff` uses it; the golden pins the coordinates.
...
is the state a tree is *in* between deleting a device and running `netgraph
